...
`verificacao-fantasma` é a classe mais recorrente do [`docs/livro-razao.md`](docs/livro-razao.md), com o dobro da segunda colocada, e junto com `teste-que-nao-prende` tem sempre a mesma raiz: **uma checagem que não checa**. As formas já pagas:
...
- **Login é opcional; servidor não é, MAS a regra não é mais uniforme entre as páginas, e a assimetria é decisão de 2026-08-24.** O app roda anônimo, e o boot do MAPA continua fail-fast em `GET /api/config`: sem backend alcançável, tela "EBGeo indisponível" (`showUnavailableScreen`, com `return` antes de `initServices()`). `atlas.html` deixou de ser: `loadLocalAtlases` nunca tocou a rede, então a seção "Neste computador" desenha com o servidor fora e só a metade de servidor vira um bloco de indisponibilidade. Criar, renomear, copiar e excluir atlas local funcionam offline; **só ABRIR precisa do servidor**, porque abrir navega para o mapa, e o texto do bloco diz isso em voz alta. `frontend/src/js/config.js` é só o *shape* que o servidor hidrata; **não há fallback estático**. Anônimo ≠ offline, e "fail-fast" agora precisa dizer em qual página.
- **Permissão tem DOIS eixos ortogonais, e eles compartilham exatamente UMA palavra: `admin`.** O eixo **POR ATLAS** tem CINCO níveis e **é** uma escada: `read < comment < write < manage < owner` (Leitor, Comentarista, Editor, Gestor, Dono). Sempre gate pela hierarquia: no cliente por `checkPermission(GuardAction.X)`, no servidor por `requireAtlasPermission`. Lista fechada tipo `perm === 'write' || perm === 'owner'` exclui o `manage` em silêncio e já causou bug real, duas vezes, nos dois pacotes. Operação de store que escreve em atlas remoto sem consultar o guard enfileira trabalho que morre do outro lado, sem erro para o usuário.
  ...
  **Os DOIS eixos têm rede.** O eixo GLOBAL é varrido por `backend/tests/unit/papel-global-censo.test.js`, que reprova o sítio não classificado; o eixo POR ATLAS ganhou censo nos dois pacotes (`frontend/tests/unit/permissao-de-atlas-censo.test.js` e `backend/tests/unit/permissao-de-atlas-censo.test.js`), e o que eles proíbem é MECÂNICO: uma linha que cite dois ou mais valores distintos do vocabulário do eixo ligados por `||`, `&&`, literal de array ou `.includes(` é acusada, e só `frontend/src/js/projects/permission-levels.js` pode escrever uma. A forma pega nos dois sentidos, e é por isso que ela é a proibição certa: a lista do topo (`owner || manager || admin`) exclui em silêncio quem nascer no meio, e a do fundo (`viewer || commenter`) falha ABERTO para o papel que o servidor inventar depois deste build. **Três ressalvas que o verde não conta.** A primeira: o `admin` fica fora do gatilho da varredura, porque é a palavra que os dois eixos compartilham e varrê-la arrastaria todo sítio de papel global (ela entra na contagem de tokens, então `role === 'owner' || role === 'admin'` continua sendo acusado). A segunda: no servidor a classe `LISTA_FECHADA_VIVA` do censo está declarada e VAZIA, com o zero asserido em caso próprio, então não há lista fechada viva a procurar lá.

  **A terceira é a que importa para quem for escrever gate novo: a regra é sobre a LINHA, então uma cadeia de igualdades em linhas SEPARADAS é a mesma exclusão escrita na vertical e passa inteira.** Foi assim que `assertOperationAllowed` (`backend/src/modules/sync/sync.service.js`), o gate por operação da escrita, ficou até 2026-09-13 comparando por igualdade a `read` numa linha e a `comment` na seguinte: um degrau novo entre os dois caía fora das duas e recebia escrita PLENA, e nenhum censo o acusava. Ele hoje compara por posto, e a forma escrita é sempre a POSITIVA (`!(nivel >= piso)`), nunca a negativa, porque `PERMISSION_LEVELS` de valor desconhecido é `undefined` e toda comparação contra ele é falsa: a negativa transforma o desconhecido em passagem e a positiva o transforma em recusa. As duas leem igual e decidem o contrário. Guarda: `backend/tests/integration/gate-de-sync-falha-fechado.repro.test.js`.
...
- **Telemetria é rastro consultável, nunca stack de terceiro, e ela tem QUATRO vocabulários espelhados.** PostHog, Sentry e GlitchTip foram avaliados e recusados em 2026-09-01 (rede fechada, um desenvolvedor, dado sensível em canvas); as ideias entraram no stack da casa e o detalhe mora em [`docs/wiki/observabilidade.md`](docs/wiki/observabilidade.md). O que morde quem escreve sem ler: a origem do erro, o estado do defeito, o catálogo de eventos de uso e o ambiente do navegador (famílias de navegador e de sistema, e os campos do bloco da máquina) vivem cada um num folha de ZERO imports em cada pacote (`origens-de-erro.js`, `estados-de-defeito.js`, `eventos-de-uso.js`, `ambiente-do-navegador.js`) MAIS CHECK no banco, presos por testes de espelho, e valor novo entra nos três no mesmo commit. O bloco da máquina (tela, placa, memória, cota) fica só na ocorrência de defeito, lida pelo administrador; o lote de uso leva só família, versão principal e sistema, desde 2026-09-23. `registrarUso` só aceita `EventoDeUso.X` e `PropDeUso.Y` (censo estrutural reprova literal nos DOIS argumentos), e `relatarErro` é a única porta de relato de erro. Relato, migalha e lote de uso NUNCA carregam conteúdo de usuário, e a exceção declarada é a mensagem de console escrita pelo programador. As rotas anônimas de telemetria têm limitador próprio ANTES do `validate` e identidade só do token. O estado `regrediu` nunca se põe à mão: só o upsert o produz. E quando o Postgres cai, o arquivo é a testemunha e as leituras de diagnóstico que leem disco continuam respondendo com o bloco de banco declarado cego, nunca zerado.
...
npm run diag -- resumo --desde 24h  # UMA tela com cinco blocos, lidos do arquivo E do
                      ...
                      #   SESSAO QUE MEXE NO BACKEND ABRE LENDO `npm run diag -- resumo --json`:
                      #   e a unica forma de a informacao ser usada sem ninguem lembrar de usa-la.
...
**O app tem CINCO páginas desde 2026-09-15, não uma:** `index.html` (mapa), `atlas.html` (seletor de atlas), `admin.html` (Administração), `calibracao.html` (calibração 360, gateada por `isAdmin()` **ou** `isProducer()`, porque calibrar é manter o que a OM produziu) e `tutorial.html` (o tutorial, que era a página estática `public/docs/doc.html` até aquela data). As quatro que não são o mapa bootam sem `@store` e sem `initServices()`, e importar delas o barrel `@utils` ou `@modals` arrasta a store inteira de volta pelo caminho transitivo. **Elas NÃO são iguais entre si, e a quinta é a diferença que se lê errado:** o tutorial é a única página que não abre banco, não tem sessão e não fala com o backend, então ele fica fora do tab-lock, do portão de migração, da telemetria e da presença, enquanto as outras TRÊS participam do tab-lock (e é por isso que `frontend/src/js/utilities/tab-lock.js` se importa **direto**, nunca pelo barrel). O que o mantém assim é guarda, não disciplina: [`frontend/tests/unit/portao-de-migracao-nas-quatro-paginas.test.js`](frontend/tests/unit/portao-de-migracao-nas-quatro-paginas.test.js) reprova se ele citar uma porta do acervo, e o nome daquele arquivo continua dizendo QUATRO porque quatro é o número de páginas que tocam o disco. Projetos e Administração compartilham a barra superior via `createAppBar` (`ui/app-bar.js`), porque `AccountControl` é `IControl` e só existe dentro de um mapa. Detalhe e medições em [`.claude/rules/paginas-e-chunks.md`](.claude/rules/paginas-e-chunks.md).
...
**As instruções de agente têm teto, e a história não mora nelas.** Este arquivo, [`.claude/rules/architecture.md`](.claude/rules/architecture.md) e [`.claude/rules/testing.md`](.claude/rules/testing.md) carregam em toda sessão e em todo subagente; os demais arquivos de `.claude/rules/` têm `paths:` e entram só quando um arquivo da área deles é LIDO. Regra de uma área nasce num arquivo com `paths:`, nunca no núcleo. Ao escrever ou corrigir uma regra, escreva o fato vigente, o porquê em uma ou duas frases e o guarda que o prende; o que a linha dizia antes, quando e quem mediu vai para o livro-razão, porque repetir a versão errada a devolve ao contexto. O que prende isso é [`frontend/tests/unit/instrucoes-do-agente-teto.test.js`](frontend/tests/unit/instrucoes-do-agente-teto.test.js): teto do que carrega sempre, teto por arquivo com escopo e todo padrão de `paths:` casando arquivo versionado.
