// Path: src/utils/mapa-de-fonte.js
/**
 * @fileoverview Um decodificador de source map v3, escrito à mão, para desminificar pilha.
 *
 * ELE MORAVA EM `scripts/diag/`, e mudou de casa em 2026-09-02 porque ganhou um SEGUNDO
 * chamador: `GET /api/v1/diag/defeitos/:id/pilha` desminifica do lado do servidor, para o
 * agente que opera de fora do host e não tem os `.map` na máquina dele. Peça compartilhada
 * entre o comando e uma rota não pode morar em `scripts/`, senão `src/` passa a depender de
 * um diretório que o deploy poderia podar sem ninguém notar.
 *
 ...
 * comprando roda num caminho que ninguém alcança sem pedir por ele (um comando que um humano
 * dispara à mão, e uma rota de administrador que ninguém abre em laço). Uma dependência nova
 * entra no `npm ci` do deploy, no `npm audit` e na superfície do processo que atende sync e
 * `GET /api/config`. O algoritmo inteiro é o VLQ base64 abaixo mais uma busca binária: são
 ...
 * (`colunaDeQuadro`, em `src/modules/diag/pilha.service.js`), porque só lá se sabe que o
 * número veio de um rastro de V8. Uma coluna errada por um não levanta erro nenhum: ela
 * devolve o segmento anterior quando cai exatamente na fronteira de um, e o resultado é um
 * nome de função plausível e errado, que é o pior desfecho que esta peça pode ter.
 */
