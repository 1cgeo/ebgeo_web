...
function mintToken({ orgId, orgRole = 'viewer', role = 'user', sub = randomUUID() }) {
  return jwt.sign(
    { sub, username: `cov_${sub.slice(0, 8)}`, role, organization_id: orgId, org_role: orgRole },
    JWT_SECRET,
...
describe('StreetView 360 — coverage of genuinely-untested behavior', () => {
  ...
  before(async () => {
    ...
    otherOrgEditorToken = mintToken({ orgId: otherOrgId, orgRole: 'editor' });
  ...
  it('image route: the owning org and a global admin CAN read the disabled-project image (200, real bytes)', async () => {
    const member = await supertest(app)
      .get(url(`/photos/${disabledPhotoId}/image?quality=full`))
      .set(...authHdr(otherOrgViewerToken))
      .expect(200);
    assert.equal(member.headers['content-type'], 'image/webp');
    assert.ok(Buffer.from(member.body).equals(disFullBuf), 'owning org gets the real disabled blob');
  ...
  it('image route: the SERVED ETag round-trips to a 304 with an EMPTY body', async () => {
    ...
    assert.ok(
      second.body === undefined || (Buffer.isBuffer(second.body) && second.body.length === 0) ||
        (typeof second.body === 'object' && Object.keys(second.body).length === 0),
