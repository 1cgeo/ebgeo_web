...
describe('Durable delivery receipts', () => {
  ...
  const op = (type, entity, data) => ({ protocolVersion: 2,
  ...
  it('cleanup preserves deduplication and a retry preserves the newer value', async () => {
    ...
    const old = { ...op('update', id, null), baseVersion: created.results[0].entityVersion,
      patch: [{ op: 'set', path: ['properties', 'name'], value: 'old' }] };
    const changed = await push(old);
    const latest = await push({ ...op('update', id, null), baseVersion: changed.results[0].entityVersion,
  ...
  it('a refused operation remains refused when its missing parent is later created', async () => {
    ...
    assert.equal(first.results[0].status, 'conflict');
