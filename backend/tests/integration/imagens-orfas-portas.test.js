...
describe('coleta de imagem órfã: rota e comando', () => {
  let app, db, admin, comum, tokAdmin, tokComum, chave;
  ...
  describe('rota /api/v1/diag/imagens-orfas', () => {
    ...
    it('conta comum recebe 403, e chave de API de administrador também', async () => {
      const comumRes = await supertest(app).get('/api/v1/diag/imagens-orfas').set('Authorization', `Bearer ${tokComum}`);
      assert.equal(comumRes.status, 403);
      const { atlas, id } = await atlasComOrfa();
      const porChave = await supertest(app).post('/api/v1/diag/imagens-orfas').set('x-api-key', chave)
        .send({ acao: 'apagar', atlasId: atlas.id });
      assert.equal(porChave.status, 403, JSON.stringify(porChave.body));
      assert.ok(await existe(id), 'a chave não apagou nada');
      const semSessao = await supertest(app).get('/api/v1/diag/imagens-orfas');
