...
const CENSO_ROTA = [
  ...
  json('src/modules/streetview360/sv360.routes.js', 'GET /photos/:uuid/tiles.json'),
  bytes('src/modules/streetview360/sv360.routes.js', 'GET /photos/:uuid/tiles/:level/:x/:y', 'src/modules/streetview360/sv360.controller.js'),
...
const CENSO_EMISSOR = [
  ...
  { arquivo: 'src/modules/nomes/assets3d.controller.js', texto: 'return res.status(416).setHeader(\'Content-Range\', `bytes */${buf.length}`).end();', n: 1,
    classe: E_SEM_CORPO, motivo: `${SEM_CORPO} O 416 da camada por MODELO usa o tamanho do buffer ja lido: o formato .3dtiles nao guarda coluna de tamanho, entao nao ha O(1) para ele.` },
  ...
  { arquivo: 'src/modules/nomes/assets3d.controller.js', texto: 'return res.status(416).setHeader(\'Content-Range\', `bytes */${meta.size_bytes}`).end();', n: 1,
    classe: E_SEM_CORPO, motivo: SEM_CORPO },
  { arquivo: 'src/modules/nomes/assets3d.controller.js', texto: 'return res.end(buf.subarray(range.start, range.end + 1));', n: 2,
    classe: E_BYTES, motivo: `${BYTES_DE_ARQUIVO} SAO DUAS desde a absorcao do acervo 3D convertido: a mesma forma serve o store plano e o .3dtiles por modelo, e as duas contam porque apagar uma nao muda a outra.` },
  { arquivo: 'src/modules/nomes/assets3d.controller.js', texto: 'return res.end(buf);', n: 2,
    classe: E_BYTES, motivo: `${BYTES_DE_ARQUIVO} Idem: uma por store de BLOB.` },
  ...
  { arquivo: 'src/modules/streetview360/sv360.controller.js', texto: 'if (req.headers[\'if-none-match\'] === etag) return res.status(304).end();', n: 3,
    classe: E_SEM_CORPO, motivo: SEM_CORPO },
