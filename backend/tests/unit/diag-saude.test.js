...
//       e a hora de ler log é a hora em que o banco pode não estar de pé), pela MEDIANA e não
//       pela média: um buraco de seis horas puxaria a média e o relatório concluiria que nada
//       faltou. A saída diz qual intervalo usou e de onde ele veio;
//   (2) zero amostra NÃO é "nenhuma queda", é instrumento que não produziu nada; uma amostra
//       só não permite afirmar coisa alguma sobre buraco, nem com `--intervalo` informado;
...
//   - troque a mediana por média em `resumirAmostras` e cai "a mediana ignora o buraco";
...
import {
  parseIntervalo, resumirAmostras, assinaturaDeErro, agruparErros,
...
describe('diag — a série de amostras: intervalo e buracos', () => {
  ...
  it('a deriva do timer não vira buraco (59s e 61s num intervalo de 60s)', () => {
    ...
    // 59 s, e não 60: a mediana é por POSTO (`percentil`, nearest-rank), então o valor
    // devolvido é uma distância que de fato ocorreu, nunca uma interpolação entre duas.
  ...
  it('o buraco sai com início, fim, duração e quantas faltaram', () => {
    ...
    assert.deepEqual(r.buracos[0], {
      inicio: 2 * MIN, fim: 5 * MIN, duracaoMs: 3 * MIN, faltantes: 2,
    });
  ...
  it('a MEDIANA ignora o buraco; a média o transformaria em "nada faltou"', () => {
    // Quatro amostras de minuto em minuto e então seis horas de silêncio. As distâncias são
    // [60s, 60s, 60s, 6h]: mediana 60s, média 1h31min. Com a média, o buraco de seis horas
    // valeria quatro intervalos e o relatório diria que faltaram TRÊS amostras.
