...
const DB_NAME = process.env.DEV_DB_NAME || 'ebgeo';
...
const APP_ADMIN_URL = `postgresql://${DB_USER}:${DB_PASSWORD}@${DB_HOST}:${DB_PORT}/postgres`;
...
async function ensureDatabase() {
  ...
  try {
    ...
    const su = pgp(SUPERUSER_URL);
    try {
      await su.none(`ALTER DATABASE ${DB_NAME} OWNER TO ${DB_USER}`);
...
async function ensureSchemaOwner() {
  const su = pgp(SUPERUSER_URL.replace(/\/postgres$/, `/${DB_NAME}`));
  try {
    await su.none(`ALTER SCHEMA public OWNER TO ${DB_USER}`);
...
async function reassignPublicObjects() {
  ...
  try {
    ...
    for (const o of objs) {
      await su.none(`ALTER ${o.kind} public."${o.name}" OWNER TO ${DB_USER}`);
    }
