...
como destrutivo. **A lista está VAZIA hoje**, por construção: num schema escrito no estado final nada é criado para
ser derrubado depois. Ela já esvaziou duas vezes e voltou a ter linha uma, quando uma migração
forward-only alargou os dois CHECK de `audit_trail` — hoje eles nascem largos na própria baseline.
Com a lista vazia, quem discrimina é o teste de controle negativo, que roda os mesmos padrões contra
SQL que os contém. **Forward-only vale a partir do momento em que a migração sai daqui**: reescrever
uma baseline só é honesto enquanto nenhum banco fora do branch a aplicou, o que hoje é o caso porque
não há produção; no dia em que houver, alargar um CHECK volta a exigir arquivo novo e linha nesta
lista.
