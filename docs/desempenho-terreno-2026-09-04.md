...
## Estado dos itens em 2026-09-05

Os itens acima foram medidos ou decididos no dia seguinte, e este é o estado de cada um:

1. **6.7.0**: feito (`caf9385e`).
2. **Agrupar as camadas do aplicativo**: medido como variante da bancada da `integracao_backend` (`terreno-camadas-agrupadas`, 3 rodadas, CPU livre e quatro vezes mais lenta, 50 feições selecionadas): 9 pilhas de render-to-texture para 1 e draw calls de 3 a 6 vezes menores nas duas cargas; com CPU livre o render p50 melhora 0,05 a 0,5 ms e a cadência do zoom chega a piorar; com a CPU quatro vezes mais lenta a cadência cai 11 a 19 ms. Custo visual medido: 0,39% do quadro muda de ordenação, todo na área das alças. Depende da máquina alvo, e a decisão é do chefe.
3. **Descer o hillshade**: continua valendo aqui, com a base vetorial de 159 camadas, onde o `beforeId: 'analysis-separator'` o põe acima dos 36 rótulos. Na `integracao_backend` a variante saiu inerte porque a base do stack de desenvolvimento é um raster de uma camada e o `beforeId` já o põe no alvo. Decisão cartográfica, ainda aberta.
4. **Duas fontes de DEM**: medido (`terreno-dem-unico`): a segunda fonte custa 42 tiles de DEM a mais por carga (67 contra 25) e mais que dobra o pedido durante o gesto; partilhar a fonte compra 0,3 a 0,5 ms por quadro em rotação, pan e zoom, sem sobreposição de amplitude, e a fase que se move é `_updateSources`. O MapLibre avisa contra partilhar (qualidade do hillshade), e o aviso não se verifica na máquina de medição, porque o DEM de demonstração configurado nela cobre um grau quadrado nos Alpes e responde 404 sobre a vista: o relevo é plano em toda a grade. Por isso nenhuma das medidas de terreno feitas com o DEM de demonstração vale como referência de relevo; valem para o pipeline de render-to-texture, que é o que se moveu.
5. **`sourceTileLodParams`**: fica `null`, e a `integracao_backend` o serve nulo por decisão do chefe.
6. **Upstream**: em aberto.
7. **Base padrão com terreno** (a pergunta da seção seguinte): a raster cobre só o recorte sul e não cabe como `DEFAULT_LAYER`; entrou um mecanismo opcional, `map2d.terrainPreferredBasemap` (nulo por padrão, `bounds` opcionais), que troca a base sem persistir ao ligar o terreno e restaura ao desligar se o usuário não trocou (`39919e4b`; na `integracao_backend` `5f70085a`, com a chave servida pelo backend). A implantação que tem o mbtiles aponta a base; sem a chave o comportamento é o de sempre.
