...
### 2026-09-24: a presença tem controle de fluxo por destinatário, lido do pong do ping de protocolo (decisão do dono)

- **Contexto:** num link de 40 kbps, três colegas movendo o mouse enchiam o downlink do par lento com lotes de cursor, e cada edição esperava atrás deles. Medido com o navegador estrangulado por CDP a 5000 B/s: presença a 3090 B/s (62% do link nominal, o efetivo inteiro) e quatro edições chegando ao par em 1,3, 2,2, 3,4 e 4,9 s, crescendo. O servidor só descartava presença com `bufferedAmount` acima de 1 MiB, e a direção sugerida era coalescer acima de um limiar menor de buffer.
- **Decisão (dono, 2026-09-24):** controle de fluxo POR DESTINATÁRIO lido do pong do ping de protocolo (`backend/src/modules/collab/collab.fluxo.js`). No máximo um quadro de presença em voo por socket, seguido de um ping com sequência; até o pong, a presença nova para aquele destinatário é retida e coalescida (a última por remetente; por remetente e superfície na seleção); depois do pong, a próxima espera três vezes o atraso acima do melhor RTT daquele socket, com teto de 5 s. Operação de sync e quadro de controle nunca passam por ali: não são retidos, coalescidos nem descartados. O ping do heartbeat leva a mesma sequência. Válvula `WS_PRESENCE_FLOW=0`.
- **Alternativa rejeitada, medida:** o limiar de `bufferedAmount`. Com o navegador estrangulado e quadros de 1 KB a 20 KB/s, o buffer do socket no servidor ficou em ZERO a rodada inteira enquanto os quadros chegavam 14 s atrasados; os bytes esperam no kernel e em quem estiver no meio (o nginx em produção), nunca no buffer do Node. Um limiar ali não dispara nunca. Também rejeitado, por ora: reduzir a taxa na origem conforme o flush (o cursor do próprio par lento ocupa cerca de 17% do uplink durante um push grande), e ligar o monitor adaptativo `connection-quality`, que depende de o cliente medir RTT.
- **O preço, declarado:** cada quadro de presença leva um ping de 2 bytes atrás, o que dobra as escritas de socket da presença (não medido na bancada de 400); num link de latência alta e banda boa (satélite) a presença fica limitada a um quadro por RTT; e um intermediário que engolisse quadros de controle deixaria a presença a um quadro por expiração do marcador (30 s).
- **Depois:** presença a 1778 a 1856 B/s e as edições chegando ao par lento em no máximo 0,72 s, em quatro rodadas; o par rápido recebe tudo na hora antes e depois.
- **Guardas:** `backend/tests/unit/presenca-fluxo-por-destinatario.test.js` (op nunca retida, nem descartada ou coalescida em 200 intercalações sorteadas) e `frontend/tests/e2e-ui/presenca-nao-disputa-com-sync.spec.js`, que reprova com a válvula desligada.
- **Status:** aceita pelo dono em 2026-09-24.
...
### 2026-09-25: os painéis laterais escondem a edição também pela trava; desenhar e recusar fica para o menu por mapa (decisão do dono)

- **Contexto:** a regra de 2026-08-24 separa as duas negações: o POSTO some, e o ESTADO (mapa travado, atlas local, offline) desenha o comando e recusa o clique nomeando o estado. Desde a medição de 2026-09-16 que criou `semEdicaoSync` (`frontend/src/js/store/edicao-indisponivel.js`), o painel de feição, a tabela de atributos e a lista de camadas escondiam a edição nos DOIS eixos, e essa divisão não estava escrita em lugar nenhum. Na resolução dos branches da campanha, em 2026-09-25, as notas do mapa passaram a seguir esses painéis (`7229712e`), e o wip do painel de processamento seguia a regra escrita: duas superfícies laterais com comportamentos opostos.
- **Decisão (dono, 2026-09-25):** os painéis laterais escondem o comando de edição nos dois eixos e acompanham a mudança ao vivo por `assinarEdicaoIndisponivel`; quando o painel diz o motivo, diz em texto, sem botão inerte. Desenhar e recusar continua sendo a regra do menu por mapa (`frontend/src/js/sidebar/tabs/map-menu-actions.js`), onde o cadeado fica ao lado dos comandos.
...
### 2026-09-25: a coleta de imagem órfã do servidor entra no lançamento, manual e sem agendamento (decisão do dono)

- **Contexto:** nenhum blob de `images` saía do disco. A única remoção é o `DELETE_IMAGE` físico da rota de exclusão de imagem, que nenhum cliente chama, e o atlas nunca é apagado de verdade (a lixeira é uma coluna de data), então o CASCADE não dispara. O disco só crescia, e `docs/wiki/imagens-atlas.md` declarava isso como limite conhecido. O dono pediu a coleta em 2026-09-24, na campanha de lançamento, com a regra "na dúvida, não apaga"; em 2026-09-25 decidiu que ela entra no lançamento sem agendamento.
- **Decisão (dono, 2026-09-24 e 2026-09-25):** o servidor ganha uma coleta de imagem órfã com três modos. A SIMULAÇÃO é o padrão e roda numa transação somente leitura; `marcar` escreve a marca de quando a imagem perdeu a última citação; `apagar` marca e apaga as elegíveis. Uma imagem só sai quando está sem citação há 30 dias CONTÍNUOS (a marca `images.sem_referencia_desde`, que o gatilho da migração zera em toda escrita que a cite, antes ou depois) e foi criada há mais de 30. A citação é o token de UUID em qualquer coluna de texto das doze tabelas de conteúdo, em qualquer atlas; atlas na lixeira não perde nada; linha excluída e operação aplicada citam por 30 dias. As portas são `npm run diag -- orfas` (com `--marcar` ou `--apagar`, e `--como` de uma conta administradora) e `GET`/`POST /api/v1/diag/imagens-orfas`, atrás de `requireAdmin`. A remoção toma o lock do log do atlas e deixa uma linha `IMAGE_ORPHAN_PURGE` por atlas. **Nada roda sozinho** (2026-09-25): não há cron nem timer, e marcar ou apagar é ato do administrador do sistema.
- **Por quê:** errar para mais (sobra blob) é barato e errar para menos apaga foto viva, e o servidor não distingue uma órfã de uma imagem cuja op ainda está numa fila offline, porque o blob sobe antes da op que o cita. Casar pelo token e não pelo campo cobre a superfície nova que ninguém lembrar de acrescentar. O gatilho é o que torna os dias contínuos: uma rodada manual e rara não vê a citação que aparece e some entre duas rodadas.
- **Alternativas rejeitadas:** apagar o blob junto com a feição (a resolução é LWW e o desfazer é local, então uma exclusão vinda de um par destruiria bytes que outro par ainda cita); varrer só os campos conhecidos (a foto por referência e a figura no HTML de uma nota ou de um slide ficariam fora até alguém lembrar); agendar já no lançamento (a proposta era um `marcar` semanal e um `apagar` mensal pelo cron do host, e o dono a deixou fora). A coleta do banco LOCAL do navegador fica fora: o desfazer é local e em memória, a figura com subida pendente mora no mesmo banco, e o blob de um atlas não montado não tem leitor para confirmar referência.
- **O preço, declarado:** uma op segurada offline por mais de 30 dias depois de a figura subir chega citando um blob já apagado, e a feição aparece sem figura. Enquanto existir imagem marcada, o gatilho varre o texto de cada linha escrita nas tabelas-fonte (medido em 2026-09-25: cerca de 10 ms por MB). Arquivo em disco sem linha não é alcançado.
- **Guardas:** `backend/tests/integration/imagens-orfas.test.js` (cada fonte com o controle negativo embutido, carência de 29 e 31 dias, imagem jovem, lixeira, os dois lados do gatilho, e a simulação em que o Postgres recusa escrever), `backend/tests/integration/imagens-orfas-portas.test.js` (só a sessão de administrador passa; a chave do próprio administrador, o produtor e o credenciado recebem 403) e `backend/tests/integration/imagens-orfas-censo.test.js` (toda tabela do banco migrado classificada, o gatilho em toda fonte e em nenhuma outra, e os coletores de import e de export cobertos).
- **Status:** aceita pelo dono em 2026-09-24 (a coleta) e em 2026-09-25 (sem agendamento).

### 2026-09-25: o atlas de servidor abre e sincroniza sem tempo real quando o WebSocket não passa (decisão do dono)

- **Contexto:** atrás de um proxy que não repassa o `Upgrade` (a produção tem três proxies no caminho, `TRUST_PROXY_HOPS` 3), o atlas de servidor não abria: a abertura esperava o quadro `connected` do socket e a aba voltava ao seletor a cada tentativa (medido em 2026-09-23). E o envio só rodava com o socket de pé, embora todo byte dele já fosse HTTP. A campanha de rede de 2026-09-24 deixou o conserto começado; o dono decidiu em 2026-09-25 que ele entra no lançamento.
- **Decisão:** um quinto estado de conexão, `HTTP_ONLY` ("sem tempo real"), que o motor de sync põe quando o socket falha depois de o servidor já ter respondido por HTTP (o handshake fechado antes do `connected`, ou 10 s sem ele). Nele o envio continua pelo mesmo `pushOperations` (o flush pergunta `canReachServer`, e não mais `isOnline`), a entrada vem de um pull periódico por cursor (`pullSync`, aplicado como o `sync_response` do socket, sem passar pelas guardas do socket), o papel por atlas vem do `user_permission` que `GET /atlas/:atlasId` passou a devolver (campo aditivo; o gate continua no servidor), e a presença não existe. O socket continua tentando em segundo plano, e o primeiro `connected` volta ao modo normal sem recarregar, com o `sync_request` partindo da versão que o pull alcançou. Um socket que cai e não volta em 10 s dispara uma sonda HTTP; a primeira resposta dela manda o socket tentar de novo na hora, e só a segunda, com o socket ainda fora, leva ao mesmo modo. Sem esse passo, o notebook que acorda depois de ficar sem rede via o aviso de "sem tempo real" só porque o recuo do socket estava em 30 s e a sonda ganhou a corrida. A pessoa é avisada uma vez por sessão, e o selo diz "Sem tempo real". As regras estão em `frontend/src/js/store/sync/sem-tempo-real.js`.
- **Os números são decisão de engenharia, à espera da confirmação do dono:** prazo do `connected`, 10 s; pull a 3 s depois de uma rodada que trouxe algo, crescendo uma vez e meia até 10 s com o atlas parado e dobrando até 30 s em falha; duas falhas seguidas passam o estado a RECONNECTING; sonda 10 s depois da queda; papel relido a cada 10 rodadas (de 30 a 100 s).
- **O que não chega sem o socket, declarado:** a presença (quem está no atlas, cursores, seleção, quem edita um briefing), os avisos que só existem como quadro ao vivo (`atlas_settings_updated`, `atlas_resources_updated`, `atlas_updated`; `sharing_updated` e `atlas_owner_changed` são cobertos pela releitura do papel), e o autor de uma edição sobrescrita. Os marcadores estruturais das exceções REST estão no log e chegam pelo pull. Um rebaixamento chega à tela com até 100 s de atraso; o servidor recusa o envio na hora.
- **O que saiu junto:** a frase "o servidor respondeu, mas a conexão em tempo real não abriu" (código `abertura-sem-tempo-real`, de 2026-09-23) e o ramo do boot que a produzia, porque a abertura deixou de falhar nesse caso.
