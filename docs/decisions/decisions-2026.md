...
- **Contexto:** a trava de camada é convenção do cliente (o servidor a guarda e não a consulta), e o caminho de criação não perguntava por ela. `activateLayer` já recusava tornar ativa uma camada travada, mas travar a camada que JÁ era a ativa deixava toda ferramenta de desenho gravando nela: medido com dois navegadores, um ponto do Dono e um do Editor chegaram ao servidor com o id da camada travada, sem aviso. As alternativas eram recusar o gesto nomeando o estado, trocar sozinho a camada ativa para a primeira destravada, ou as duas; trocar sozinho faz o desenho cair noutra camada sem a pessoa pedir.
