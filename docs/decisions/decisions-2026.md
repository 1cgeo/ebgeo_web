...
### 2026-09-24: gestos do mesmo verbo sobre feições independentes acima de 200 sobem em partes (decisão do dono, B6.1)
...
- **Decisão (dono, 2026-09-24):** o MESMO VERBO sobre FEIÇÕES DISTINTAS acima do teto sobe em partes de até 200, cada parte um lote lógico próprio. Fora de gesto, a transação cujas ops são todas de feição, de um só tipo (criar, atualizar ou excluir), cada entidade uma vez e sem intenção de mover (`isIndependentFeatureSet`, `frontend/src/js/store/sync/operation-factory.js`) ganha um `batchId` por parte; o desfazer e o refazer declaram o gesto divisível (`withGestureBatch` com `splittable`, `frontend/src/js/store/sync/gesture-batch.js`) só quando a entrada da pilha é o mesmo verbo sobre feições distintas (`isSameVerbOverDistinctFeatures`). O `batchIndex` continua de uma parte para a outra e uma transação nunca fica partida entre duas. Quando o servidor recusa uma parte, a pessoa ouve qual parte e quanto já chegou (`describeRefusedPart`). Sem mudança no servidor nem no envelope.
- **O preço, declarado:** os gestos partidos deixam de ser tudo-ou-nada no servidor: uma parte pode ser recusada (um colega travou o mapa no meio) enquanto as outras são aplicadas, e a recusada fica nas pendências. Os gestos COMPOSTOS continuam atômicos e continuam recusados acima de 200: transferir uma camada com mais de 200 feições falha inteiro (301 pendências, nada muda no servidor), assim como converter e grupo com seus membros.
