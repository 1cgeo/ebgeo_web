...
**A origem muda sem evento, então a batida REPINTA mesmo quando não lê a fila** (2026-09-23). `markStoreLocal` não emite nada, e a batida periódica chegava a `_readQueue` (`frontend/src/js/account/sync-status.control.js`) e voltava ANTES de `_render()` em atlas local e para visitante, que é o ramo que existe para poupar leitura de IndexedDB. A última pintura, feita com a origem ainda remota, ficava congelada: depois de uma abertura por `?atlas=` que falhou numa aba com a intenção "Mapa local", a aba estava num atlas local e a luz dizia `sem-conexao` (ou `recuperando`, conforme o instante da última pintura) para sempre. Pintar não lê a fila, então o ramo repinta e continua sem custo de disco. Guardas: `frontend/tests/unit/sync-status-control.test.js` e o caso 2 de `frontend/tests/e2e-ui/abertura-remota-que-falha.repro.spec.js`.
