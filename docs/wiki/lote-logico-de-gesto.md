...
Até 2026-09-24 **a recusa acima do teto era LOCAL**, sem viagem, e o gesto nunca chegava ao servidor. Não é mais assim.

**Acima do teto o lote parte em blocos ENCADEADOS, desde a decisão do dono de 2026-09-24 (B6.1)**, registrada em [`decisions-2026.md`](../decisions/decisions-2026.md). Todo lote, transação ou gesto, compostos inclusive: importar, colar, mover para camada, agrupar, transferir camada, desfazer e refazer. Cada bloco de até 200 é um lote próprio, o `batchIndex` é contínuo, e a primeira operação de cada bloco depende (`dependsOn`) da última do anterior (`createBatchOperations`). Como o carregador não entrega dependente de operação com problema e o envio manda um bloco por vez, um bloco só sai depois de o anterior ser aplicado e um bloco recusado segura os seguintes. É isso que impede um `group_feature` de chegar antes do grupo: sem o grupo o insert escreve zero linhas e volta confirmado, o sucesso mudo. O preço é o que o savepoint evitava: acima de 200 o gesto deixa de ser tudo-ou-nada no servidor, e a frase de `describeRefusedPart` diz qual parte foi recusada e quanto já chegou.
...
- **Permissão alterada NÃO vira problema durável.** O 403 do rebaixamento não está entre as recusas permanentes do flush (só 400, 413 e 422), então o flush LANÇA, nada é guardado e nada sai da fila: o trabalho espera, e sai inteiro quando a permissão volta. Guardar problema ali pediria decisão humana sobre um estado que outra pessoa desfaz. O 403 vem da ROTA (`requireAtlasPermission('comment')`, `backend/src/modules/sync/sync.routes.js`), não do `assertOperationAllowed`: são dois gates em série com o mesmo status, e só desligando os dois o caso fica vermelho.
