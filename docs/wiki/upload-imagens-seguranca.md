...
Relevante para [[atlas-import-offline]]: a primeira ocorrência de um `localId` é inserida **preservando o id** (`INSERT_IMAGE_WITH_ID`), para que a referência do blob na feição sobreviva ao import sem reescrita. Duplicata do mesmo `localId` no lote não pode reusar a PK, ganha id novo e o `mapping` colapsa last-wins. Cada tentativa escreve seus bytes em caminho exclusivo **antes** do INSERT, sem reservar conexão do banco durante a escrita. Se o INSERT falhar, só o arquivo privado daquela tentativa é removido. Assim uma falha de disco não publica linha incompleta nem apaga uma imagem já confirmada a um reenvio concorrente. Uma morte abrupta entre arquivo e INSERT ainda pode deixar bytes sem referência; esse é um resíduo de armazenamento, não uma imagem publicada sem conteúdo.

Para recuperar linhas incompletas produzidas pela ordem anterior, ou arquivos danificados, o reenvio com o mesmo id e conteúdo confere também os bytes do disco, mesmo quando o banco já guarda o hash. Se necessário, publica o arquivo reparado por renomeação de um temporário, preservando id e metadados. Conteúdo diferente continua recusado; falha de leitura ou reparação não pode responder como envio confirmado. As regressões de reparação e concorrência estão em `backend/tests/integration/imagem-reenvio-recupera-arquivo.repro.test.js`.
