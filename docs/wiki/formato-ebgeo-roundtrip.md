...
- **A Linha de Barreiras já não some na leitura nem no envio pela página de atlas; some no `.ebgeo` mandado DIRETO ao servidor, e a população disso é zero.** A ferramenta nasceu (`07b16899`) e virou Linha de Coordenação (`6106449e`) no mesmo dia, 2026-09-03, antes de qualquer implantação: nenhum usuário tem `barrier_lines` no disco, e o que segue vale para dado de teste e como higiene do formato. A ferramenta que a outra linha teve por um dia gravava no balde `barrier_lines`, e a 2.3 dela a generalizou na Linha de Coordenação sem mover nada: as feições atravessavam o disco e o `.ebgeo` sem serem desenhadas nem contadas. Desde 2026-09-07 `ensureCoordinationLines` (`frontend/src/js/store/repository.utils.js`) as ADOTA nos caminhos de leitura (IndexedDB, snapshot, `.ebgeo` importado e o leitor do envio, `buildLocalAtlasExportData`), carimbando `source: 'coordination_line'`, o código MD33 `290199` e `baseCoordinates` recuperado da geometria. O caminho que continua descartando é `frontend/src/js/projects/import-ebgeo.service.js` (o `.ebgeo` mandado direto para o servidor), que entrega os baldes CRUS a `buildFeatures` (`local-atlas-to-server.js`), onde o que não está em `VALID_FEATURE_TYPES` vira `droppedFeatures`.
