...
**Nenhuma consulta ao banco por requisição de asset.** É a restrição que dá forma ao desenho: o Cesium abre uma requisição por tile por LOD, e pôr essa explosão no pool de dez conexões a faria competir com o sync, com o socket de colaboração e com o `GET /api/config`, cuja falha impede o boot. Caminho público é resolvido só em memória; caminho privado paga UMA decisão, memoizada. Medido em `backend/tests/integration/assets3d-privado.test.js` com o contador de pool: 20 requisições públicas e 20 privadas do mesmo chamador custam zero consultas.

**Desde 2026-08-29 essa decisão não é mais só desta rota, e o nome dela diz isso.** `recursoPrivadoLiberado` (`backend/src/modules/nomes/assets3d-acesso.js`) perdeu o "asset" do nome porque nunca foi sobre asset: ela é o predicado de recurso privado, chaveada por (chamador, empréstimo, recurso), e o gate do servidor de tiles a chama também ([[tile-privado]]). Duas consequências que não se leem no arquivo: o memo é **um só** para os dois consumidores, então a janela de revogação e o teto de tamanho são compartilhados, e um segundo memo introduzido "para separar os assuntos" teria um segundo teto para dimensionar e uma segunda invalidação para esquecer; e o módulo continua chamando-se `assets3d-acesso.js`, então o caminho do arquivo diz menos que o nome da função sobre quem depende dela.
...
saía PÚBLICO e o anônimo recebia o tileset privado com um ano de cache compartilhado. O erro só
...
**As duas lições acima custaram um vazamento medido cada uma, e é por isso que elas viraram MÓDULO em vez de trecho.** `normalizarRel`, `chaveDeCasamento`, `ordenarEntradas` e `acharEntrada` saíram daqui em 2026-08-29 para `backend/src/modules/nomes/caminho-de-recurso.js`, folha de um import só, quando o índice do prefixo de tiles precisou exatamente das mesmas quatro. A saída barata teria sido copiá-las, e a cópia que não recebesse as duas lições repetiria os dois defeitos noutro prefixo, com o mesmo desfecho e sem nenhum sinal. *(Esta seção atribuía `chaveDeCasamento` a `assets3d-regime.js`; ela não mora mais lá.)*

O índice é invalidado por `invalidateAppConfigCache` (`backend/src/modules/config/config.cache.js`), que toda escrita de catálogo e de visibilidade já chamava. Ele invalida **três** derivações das mesmas linhas de catálogo (este índice, o do prefixo de tiles e o memo do predicado), e é isso que faz uma escrita direta no banco medir o cache em vez do gate: a lição está no [`livro-razão`](../livro-razao.md), e ela recorreu no mesmo dia.

**O CAMINHO NÃO REIVINDICADO É PÚBLICO AQUI, E RECUSADO NO ÍNDICE IRMÃO.** É a divergência mais importante entre os dois arquivos que compartilham as quatro funções de caminho, e ela é deliberada nos dois sentidos. Aqui o Node serve o acervo inteiro e existem arquivos legítimos que o catálogo não descreve, então recusar o não reivindicado derrubaria os modelos públicos; no prefixo de tiles o endereço é texto livre digitado pelo administrador, e publicar o não reivindicado transformaria um erro de digitação numa linha privada em vazamento mudo ([[tile-privado]]). Quem "harmonizar" os dois defaults por simetria quebra um dos dois produtos, e o defeito de cada lado é de natureza diferente: aqui ele seria visível na hora, lá seria silencioso.

**O que continua aberto, e por quê.** Só o que esta rota não vê: prefixo de catálogo servido por outro processo (a URL canônica `/3d/...` sai do nginx, ou do Vite em desenvolvimento, e nunca chega aqui), e por isso "os bytes privados estão fechados" é afirmação sobre esta rota, não sobre o servidor web. O `fileoverview` de `backend/src/modules/nomes/assets3d-regime.js` nomeia esse limite e os outros dois do índice, para que a cobertura dele não seja lida como completa. E o endereço que o NAVEGADOR busca sozinho (uma imagem, um vídeo, um loader de terceiro) não carrega cabeçalho: para um recurso privado ele depende do braço de empréstimo, isto é, de haver um atlas em foco que o empreste. **Esse último caso encolheu em 2026-08-29**, quando o login passou a emitir cookie de sessão pelo trabalho do tile: um pedido que o navegador faz sozinho para a MESMA origem passa a levar credencial, e o que sobra descoberto é o deploy cross-origin, onde `SameSite=Strict` retém o cookie. Esta linha dizia que fechá-lo "exigiria cookie de sessão emitido no login, que é o eixo de autenticação e não o desta fase"; o cookie existe, e a razão é a de [[tile-privado]], não a desta rota. **O cookie não alcança o EMPRÉSTIMO**, e é aí que o caso mais visível mora: a miniatura de catálogo do acervo adotado é arquivo desta rota, o cookie diz quem pede, e quem só enxerga o modelo pelo atlas em foco precisa do `?atlasId=` na URL da imagem. Sem ele o modelo emprestado abria e a miniatura sumia (2026-09-22); ver [[resources-catalogo]].
