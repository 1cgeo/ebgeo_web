...
O cliente, por desenho, não descarta lote rejeitado: registra o span `flush.push` como `FAILED` e relança sem dequeue, de modo que os mesmos `opIds` são reofertados no ciclo seguinte (`frontend/src/js/store/sync/sync-engine.js:271-283`). Como o auto-flush roda a cada 1,5 s e engole a exceção com um `console.warn` (`frontend/src/js/store/sync/sync-flush.js:82-83`), uma operação **permanentemente** proibida vira fila travada com retry infinito e nenhuma sinalização na UI.
