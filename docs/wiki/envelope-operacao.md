...
**5. Os dois tetos de lote são independentes.** O cliente empurra de **25 em 25** (`FLUSH_BATCH_SIZE`, `frontend/src/js/store/sync/sync-engine.js`); o backend recusa acima de 500 (`backend/src/modules/sync/sync.schemas.js`). Eram 100 até 2026-08-28, e a queda não foi economia de memória: a bancada mediu que lote grande perde nos DOIS eixos (706 ops/s em lote de 100 contra 968 em lote de 25) e ainda aproxima o 503 do advisory lock, porque a fila é `escritores x lote`. Ver [[capacidade-de-uma-instancia]]. A folga esconde o acoplamento: subir `FLUSH_BATCH_SIZE` acima de 500 faz todo push virar 422. Isso travava o sync inteiro; desde a rede de segurança de 2026-07-25 o `flush` cai em modo de isolamento e passa a empurrar **uma op por requisição** indefinidamente, o que drena a fila mas é ordens de grandeza mais lento e não emite erro nenhum. Degradação silenciosa continua sendo defeito.
