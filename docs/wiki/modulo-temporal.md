...
## Fontes
- guia *acoes-interface-multiusuario* (absorvido) (§29 e tabela de resolução de conflitos): matriz das 20 ações temporais com classificação local vs compartilhada, princípio "config compartilhada / cursor local", awareness do instante temporal, e a descrição do reagendamento que diverge do código.
- guia *ui-ux-ebgeo* (absorvido) (linhas 204-205): posicionamento do módulo `temporal/` como dimensão temporal por mapa dentro da arquitetura de UI.
