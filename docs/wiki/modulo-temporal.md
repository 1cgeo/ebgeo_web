...
- **Interruptor, cursor e reprodução (categoria 3):** `setMapTemporalView` e `toggleMapTemporal` **não perguntam nada, de propósito**. São vista da pessoa, como pan e zoom: não gravam o documento do mapa, não enfileiram op, e por isso um Leitor e um mapa travado mantêm a linha do tempo na tela. O que eles gravam é a lembrança local da pessoa, fora do atlas. Ver [[vista-da-pessoa-e-vista-salva]].
...
- **Editor da rota e arrasto do corpo:** o editor toma a descida com `preventDefault` no `pointerdown`, e isso cancela o `mousedown` de que o arrasto do corpo depende (`frontend/src/js/tool_manager/move_handler.js`). Por isso o miolo do anel da partida devolve nulo em `_queryHandle`: sem isso, o centro do símbolo nunca chegaria ao arrasto do corpo. Raios e miolo moram em `frontend/src/js/temporal/trajectory-tool/trajectory-edit-geometry.js`.
