...
2. **Config por mapa, compartilhada, LWW:** `DEFAULT_TEMPORAL_CONFIG` (`src/js/temporal/temporal.constants.js:49-56`). `modo`, `unidade` e `origem` são **lentes de exibição**: mudá-las só re-rotula a régua.
3. **Estado local por usuário, nunca persistido nem sincronizado:** cursor, `playing`, velocidade e modo revelar vivem apenas em campos do controller (`src/js/temporal/temporal-controller.js:55-67`). São análogos a pan/zoom.
...
`shiftMapTemporalTimes` passa por `guardWrite` (`src/js/store/feature.operations.js:657`), mas `setMapTemporalConfig` e `toggleMapTemporal` **não têm guarda de escrita nem de bloqueio de mapa**. A proteção contra mapa bloqueado é apenas o botão `disabled` no card (`src/js/sidebar/tabs/maps.tab.js:490-497`): qualquer outro caminho de chamada (script, atalho, código novo) contorna a restrição. Ver [[permissoes-atlas]].

## Acoplamentos que atravessam arquivos

- **Viewers 3D/360** filtram marcadores pelo cursor. Por isso o controller emite `TEMPORAL_CURSOR_CHANGED` logo após resolver o cursor no sync (`src/js/temporal/temporal-controller.js:248-252`): sem isso, ao habilitar o temporal eles filtrariam com cursor `NaN`, já que `MAP_TEMPORAL_CHANGED` dispara antes.
...
- **Importação:** dados temporais viajam como propriedades comuns, sem evento dedicado. `decimateTrajectory` descarta detalhe abaixo da resolução da régua, cujo passo mais fino é um minuto (`src/js/temporal/temporal-model.js:172-183`). Ver [[formato-ebgeo-roundtrip]].
- **Observabilidade:** `TEMPORAL_CURSOR_CHANGED` é evento por frame e é explicitamente ignorado pelo tap do ledger (`src/js/store/sync/diag/bus-tap.js:11`). Ver [[syncledger]].

> [!CONTRADICAO 2026-07-18] O JSDoc de `src/js/store/temporal.operations.js:9,25` ainda descreve a config como `{ativo, unidade, inicio, fim}`, sem `modo` e `origem`. O formato real é o de `DEFAULT_TEMPORAL_CONFIG` (`src/js/temporal/temporal.constants.js:49-56`).
...
- Código (autoritativo): `src/js/temporal/`, `src/js/store/temporal.operations.js`, `src/js/store/feature.operations.js`, `src/js/store/sync/{operation-dispatcher,remote-operation-handler}.js`, `src/js/presence/presence-bridge.js`, `src/js/layers/visibility-filter.js`.
