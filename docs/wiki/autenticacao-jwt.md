...
**O cookie deixou de ser detalhe da renovação em 2026-08-29, e o mesmo JWT passou a viajar por duas portas.** `porCookieDeSessao` (`backend/src/modules/auth/auth.controller.js`) o emite no login e no refresh, e o logout o apaga; até então o único emissor era a renovação deslizante, perto da expiração. Não há credencial nova: o cookie carrega o token que o corpo já devolve, e o corpo continua devolvendo-o porque **é o cabeçalho que escreve** (o `auth` estrito recusa principal vindo de cookie nos métodos que escrevem). O porquê da porta nova, e a amarra inteira, em [[auth-flexivel]] e [[tile-privado]].
...
- **Tokens persistem os dois em `localStorage`** sob a chave `ebgeo_auth`, degradando para memória só quando `localStorage` não existe. É deliberado (a sessão precisa sobreviver ao F5), mas quem auditar segurança deve saber que o access token está no disco do navegador, **e desde 2026-08-29 em dois lugares**: a cópia do cookie é `httpOnly` e a do `localStorage` não, e é a segunda, alcançável por script, que assina toda escrita. Endurecer o cookie não muda a exposição do access token, porque a cópia legível é a que trabalha.
...
**O teto de 72 bytes na senha é do bcrypt, não uma política escolhida** (`newPassword`, `backend/src/modules/auth/auth.schemas.js`, espelhado em `frontend/src/js/modals/signup.modal.js` e `frontend/src/js/modals/password-recovery.model.js`). O algoritmo ignora tudo além do 72º byte, em silêncio, então sem o teto duas senhas diferentes que só divergem depois dali entram como a mesma, e o produto teria aceitado uma senha longa anunciando uma força que ela não tem. A contagem é em BYTES e não em caracteres, que é onde a regra morde de verdade num produto em português: um acento ocupa dois, então o limite real varia com o texto, e uma senha de 37 caracteres pode ser recusada por um teto que anuncia 72. **Na tela de recuperação a palavra "bytes" saiu em 2026-09-20, a pedido do dono**, e a regra que a substitui é sobre ONDE cada limite fala: a dica visível diz só o piso (`PASSWORD_RULE_TEXT`), e cada teto vira uma recusa própria no momento em que morde, a dos bytes SEM número (`PASSWORD_HEAVY_TEXT`), porque o número que ela citaria é maior que a contagem que a pessoa acabou de digitar. A mesma frase ainda não foi espelhada no cadastro. O mínimo de seis caracteres continua sendo política, e continua valendo só no cadastro e na redefinição, nunca no login.
...
## Uma tabela de token, TRÊS propósitos, e o que torna isso seguro

Desde 2026-08-23 `email_verification_tokens` guarda três coisas, e o nome da tabela é histórico: confirmação de conta nova, confirmação de um endereço NOVO numa troca, e redefinição de senha. A alternativa recusada era uma tabela por fluxo, e a razão de recusá-la é a de sempre nesta casa: o resgate atômico de uso único, o prazo por linha e a contenção de falha do envio já estavam escritos e provados uma vez, e a segunda cópia é a que envelhece.

**O que compra a segurança do reúso é a coluna `purpose`, não a boa intenção do chamador.** `CLAIM_VERIFICATION_TOKEN` casa `purpose = ANY($2::text[])`, e cada rota passa a lista que lhe cabe: a confirmação aceita `verify` e `change_email` (as duas chegam pelo mesmo link `?verify=`, e o cliente só tem o token em mãos), e a redefinição aceita `reset_password` e nada mais. Sem o discriminador, um link de confirmação, que sai em todo cadastro e em todo reenvio e que fora de produção é escrito no log, seria resgatável na rota que troca a senha. O controle negativo roda nas DUAS direções em `backend/tests/integration/senha-redefinicao-por-email.test.js`, e ele afirma também que o token recusado **não** é queimado pela tentativa: se fosse, qualquer um desativaria o link de cadastro alheio apresentando-o na rota errada.

O bicondicional `email_token_new_email_check` recusa os dois estados impossíveis (endereço novo sem troca, troca sem endereço novo), do mesmo jeito que `users_producer_scope_check` faz com o escopo de produção.

## Trocar o e-mail: o endereço pretendido não mora na conta

`PUT /users/me/email` (`requestEmailChange`, `backend/src/modules/users/users.service.js`) não escreve nada na linha da conta. Ele cobra a senha atual, cunha um token com o endereço pretendido em `new_email` e manda o convite para lá; a conta só muda quando o link é aberto, e aí o endereço nasce confirmado, porque o clique É a prova de posse.

O caminho curto (gravar o endereço novo na conta e zerar `email_verified`) foi recusado por um custo concreto: um erro de digitação trancaria a pessoa fora da própria conta na hora, já que `login` recusa `email && !email_verified`, e só um admin a tiraria de lá. Duas consequências que não se deduzem: **a sessão não é tocada** em nenhum dos dois passos, ao contrário da troca de senha, porque a credencial de entrada é (usuário, senha) e nenhuma das duas se move; e **nada é reservado**, porque a unicidade é conferida no pedido e de novo no resgate, o que é o que mantém a cláusula 10.6 de [`../../CONSTITUICAO.md`](../../CONSTITUICAO.md) valendo sem custo novo.

O endereço já pertencente a OUTRA conta responde o MESMO 200, e a colisão viaja só para a caixa que a possui (`sendEmailInUseNotice`). É o não-oráculo de `register` aplicado a um chamador autenticado, e ele continua fazendo sentido: hoje nenhuma rota deste servidor responde "existe conta com este e-mail?" para uma conta comum, e um 409 aqui seria a primeira.

## Redefinição de senha por e-mail: montada só onde a mensagem chega

`POST /auth/forgot-password` e `POST /auth/reset-password` existem apenas quando `canDeliverAccountMail()` (`backend/src/utils/mailer.js`) é verdadeiro, do mesmo jeito que `/auth/register` depende de `ALLOW_SELF_REGISTRATION`. O predicado é mais largo que `isSmtpConfigured()` de propósito: fora de produção o mailer escreve o código no log, e aquele log É o canal de entrega de um stack local. Em produção sem relay as rotas simplesmente não sobem, `GET /api/config` reporta `features.password_reset_email` falso, e a tela de login oferece só o caminho do administrador, que é a regra verdadeira ali.

Quatro propriedades que a leitura do código não entrega de graça:

- **O código só é cunhado para endereço CONFIRMADO** (`FIND_RESETTABLE_USER_BY_EMAIL`). Um endereço não confirmado não foi provado de ninguém, e mandar credencial de senha para lá entregaria a conta a quem o digitou no cadastro. Quem está nesse estado é caso do administrador.
- **O prazo é em MINUTOS** (`AUTH_PASSWORD_RESET_TTL_MINUTES`, padrão 60), e não nas 48 horas da confirmação de conta: o token de confirmação ativa um endereço declarado, o de redefinição troca o único fator de autenticação da casa.
- **Redefinir corta todas as sessões** (`REVOKE_ALL_USER_TOKENS`), e a razão é mais afiada que no caso da troca voluntária: o motivo de recuperar costuma ser que outra pessoa está dentro.
- **O resíduo do anti-enumeração é o TEMPO, e ele fica aberto.** A resposta é uniforme, mas o ramo que acha a conta paga duas consultas e uma entrega SMTP, e aqui não existe o `bcrypt` que domina a requisição em `register`. O que limita isso é o teto por endereço (`forgotPasswordLimiter`), não a forma da função. É a mesma dívida que `resend-verification` já carregava.

**Não existe fila durável de entrega nem retentativa, e a substituição transacional do código cria a janela que se lê ao contrário.** Pedir recuperação substitui o código anterior dentro da transação, então na saída dela só o novo vale; se o SMTP falhar depois, a pessoa fica com um código antigo que não resgata mais e um novo que nunca chegou, e do lado dela isso se parece com "o link do e-mail parou de funcionar". A falha fica no log do servidor e a resposta externa continua uniforme, de propósito, então ninguém é avisado pela tela. A saída é sempre pedir outro código, nunca procurar o antigo, e essa é a razão de o texto da interface descrever o próximo passo em vez de afirmar que a mensagem foi enviada.

**O código viaja como CÓDIGO para colar, não como link clicável**, e isso é limite do cliente e não escolha de segurança: o boot do app consome exatamente um parâmetro de uso único (`?verify=`), em `frontend/src/js/index.js`, e não há roteamento para um segundo. Prometer um link que a página ignoraria é pior que pedir uma colagem, então o painel "Esqueci minha senha" de `frontend/src/js/modals/login.modal.js` aceita o código. Ensinar o boot a rotear um `?redefinir=` é o passo seguinte, e é ele que remove a colagem.
...
O token do link é **credencial de fator único**: `verifyEmail` o consome e marca a conta verificada sem pedir mais nada. Por isso o link só é escrito no log **fora** de produção (`exposeLink = !config.isProd`, `sendVerificationEmail` em `backend/src/utils/mailer.js`). Em dev sem SMTP aquela linha de log **é** o canal de entrega; em produção a mesma situação loga em nível `error`, sem o link, e não existe plano B pelo log: a ativação passa por um admin, que desde 2026-08-23 tem DUAS ações e não uma, e a diferença entre elas importa. Enviar `email_verified: true` APROVA o endereço que está lá; enviar `email` CORRIGE o endereço, e isso derruba a confirmação por si só (`resolveAdminEmail`, `backend/src/modules/users/users.service.js`), salvo se o mesmo pedido mandar as duas coisas. Aprovar um endereço digitado errado era a única saída que existia, e ela ativava uma conta cujo dono nunca receberia nada ([[gestao-usuarios]]). Não conte com o `redactUrl` para isso: ele delimita um invariante mais estreito do que parece, cobrindo credencial que viaja na **URL da requisição** e nada do que vai em campo de log estruturado. Desde 2026-09-19 `verify` e `atlaspublico` entraram no `SENSITIVE_QUERY_KEYS` (`backend/src/utils/redact-url.js`), ao lado das credenciais de portador, e a função também descarta o userinfo de um endereço com senha embutida; o que continua fora do alcance dela é tudo o que não seja query da URL. Ver [[api-keys]].
...
**Ele apaga o cookie de sessão, e é o único encerramento que consegue.** Os outros cinco (reuso detectado, troca da própria senha, reset por administrador, redefinição por e-mail, desativação) agem sobre o ESTADO e nem sequer têm a resposta daquele cliente em mãos; para eles quem limpa é o ramo de sessão morta de `flexibleAuth`, na requisição seguinte. Sem essa limpeza o cookie sobreviveria ao logout até o `maxAge`, e o resultado seria uma discordância silenciosa: o cliente se acha anônimo (a sessão dele é in-memory) enquanto o servidor continua vendo um principal em toda superfície que lê cookie. A armadilha de implementação está em [[auth-flexivel]]: limpar com atributos diferentes dos da emissão não apaga nada.
...
Revogação em massa dos refresh tokens acontece só em CINCO gatilhos: detecção de reuso, troca da própria senha, reset por admin, redefinição pelo link de e-mail e desativação do usuário ([[gestao-usuarios]]). Esta linha listava quatro e omitia a redefinição por e-mail, que chegou depois dela e carimba o mesmo `REVOKE_ALL_USER_TOKENS`. A contagem importa porque os cinco são também o que derruba uma chave de API viva, pelo corte de sessão ([[api-keys]]).
