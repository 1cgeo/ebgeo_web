...
`GET /api/v1/users/search` exige apenas autenticação, **sem escopo de atlas** (`backend/src/modules/users/users.routes.js`), e continua sendo global entre organizações de propósito: o caso de uso é o atlas conjunto entre unidades. O que mudou em 2026-09-14 (decisão D13, achado P8) foi a LARGURA do casamento, e é aí que está o não óbvio:

- **Posto e OM saíram do casamento e FICARAM na projeção**, e a assimetria é a decisão inteira. Eles casavam para achar "Cap" ou "CIGEx", e o preço era enumeração: o nome de uma organização devolvia o efetivo dela, vinte linhas por vez, para qualquer conta e para qualquer chave de API de escopo largo. Casar contra atributo COLETIVO é enumeração com outro nome. As duas colunas continuam vindo em cada linha (`SEARCH_USERS`, `backend/src/modules/users/users.queries.js`), porque quem compartilha reconhece a pessoa pelo par posto + OM e não por um login. Como o `LEFT JOIN` é permissivo, um usuário sem posto ou sem OM volta com esses campos em `null`: renderize com fallback, não assuma string. Ver [[organizacoes-om]].
- **O piso é de três caracteres, com 422 abaixo dele** (`USER_SEARCH_MIN_TERM`, `backend/src/modules/users/users.schemas.js`), o mesmo desenho da busca de atlas do administrador. O cliente carrega a própria cópia do número (`PEOPLE_SEARCH_MIN_CHARS`, `frontend/src/js/catalog/grant-tree.js`) só para não gastar uma requisição já sabidamente recusada, e o espelho tem guarda (`frontend/tests/unit/busca-de-pessoas-piso-espelha-backend.test.js`).
- **O teto de vinte agora FALA.** Continua sem offset e sem paginação, mas a resposta deixou de ser o array nu: ela é `{ term, results, truncated }`, e `truncated` separa "vinte resultados" de "vinte de muitos", que antes eram a mesma tela. Refinar o termo continua sendo a única saída, e agora a tela diz isso.
- **O número de CONSULTAS tem teto próprio** (`userSearchLimiter`, `backend/src/modules/users/users.rate-limit.js`), chaveado pela CONTA e não pelo endereço, porque a rota é `auth` estrito e chavear por endereço puniria a OM inteira atrás de um egress NAT pelo que uma conta fez. Ele é o que fecha a enumeração lenta que o teto de linhas sozinho não fecha, e é por isso que "só limitar o número de linhas" foi a alternativa recusada na decisão. Ver [[hardening-borda-api]].
