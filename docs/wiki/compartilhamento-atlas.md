...
Por isso **não reaproveite o objeto do `POST` para atualizar a lista em memória**: releia o `GET`, que é o que o modal faz (`frontend/src/js/modals/sharing.modal.js:176-183`). O `json_agg ... FILTER (WHERE s.id IS NOT NULL)` existe para devolver `[]` e não `[null]` quando não há shares; preserve o `FILTER` ao mexer na query.
...
> Até 2026-07-25 esta seção afirmava o oposto, que a permissão do WS "é resolvida uma vez, na conexão, não a cada frame, então a remoção só morde na próxima sessão". A citação que a sustentava (`:86-100`) resolvia, e era a resolução do handshake; a conclusão tirada dela é que era falsa, porque a mesma função é rechamada pelo sweep. Quatro páginas irmãs já descreviam o comportamento certo ([[permissoes-atlas]], [[sintese-eixos-de-permissao]], [[link-publico]], [[atlas-modelo-de-dados]]): a lição é que citação verdadeira não valida a frase que a acompanha.
...
Ao escolher alguém na busca, o modal concede `read` por decisão explícita: "a permissão padrão abaixa, nunca eleva" (`frontend/src/js/modals/sharing.modal.js:47-51`). Preserve isso.
