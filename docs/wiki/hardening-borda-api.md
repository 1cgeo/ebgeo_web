...
- **Em `/register` o `username` é escolhido pelo atacante, então a chave composta não estrangula nada.** A frase "força-bruta contra uma conta é estrangulada" vale para `/login`, onde o alvo **é** o campo que compõe a chave. Em `/register` o alvo é o **e-mail**, que não entra na chave: cada tentativa com um `username` novo estreia um balde novo. Somado ao fato de que a rota responde `409` para e-mail já cadastrado e `201` caso contrário (ver [[gestao-usuarios]]), `/auth/register` é um oráculo de existência de e-mail sem estrangulamento efetivo, sempre que `ALLOW_SELF_REGISTRATION` estiver ligado. Só não é exposição de produção porque o default lá é desligado.
