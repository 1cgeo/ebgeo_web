...
- **Apply que grava DOCUMENTO LATERAL toma a trava daquele documento, com a mesma chave do escritor local.** `serializeGuardedApply` serializa remoto contra remoto, e nunca remoto contra LOCAL: os dois lados fazem read-modify-write assíncrono do documento inteiro, então a op de um par que caia dentro da janela de um `addMarker360`/`addMarker` local devolve ao disco a versão que leu antes. A perda é silenciosa e vai nos dois sentidos, o marcador recém-criado some e o recém-apagado volta. Até 2026-09-16 só o comentário tomava a dele; 3D, 360, orientação, câmera, o carimbo de revisão e a gravação de side-store do snapshot gravavam sem trava nenhuma. Chaves em `sideDocumentKey` (`frontend/src/js/store/document-lock.js`), que normaliza nome e UUID para a mesma porta, e regressão em `frontend/tests/integration/streetview360-write-ahead.test.js` (a intercalação perdedora é forçada por um portão, nunca sorteada).
- **O MAPA BASE é a forma mais traiçoeira dessa mesma classe, porque o dono do gesto EMITE o evento em vez de ouvi-lo.** Quem chama `map.setStyle` é o `BaseLayerControl`, e era ele o emissor de `BASE_LAYER_CHANGED`; os assinantes daquele evento (o cartão do seletor, a barra lateral, o 3D, o 360, o terreno) mexem em rótulo e estado. O handler remoto emitia `BASE_LAYER_CHANGED` direto, então no par o cartão passava a mostrar a base nova enquanto o MapLibre seguia desenhando a antiga, e só um F5 trocava: a UI anunciava uma base que a tela não tinha, que é pior do que não propagar. Desde 2026-09-16 o caminho remoto emite `BASE_LAYER_REMOTE_CHANGED` com o mapa a que pertence; o controle ouve, aplica por `applySharedBasemap` (o mesmo gesto do link compartilhado, que troca o estilo, remonta as feições e NÃO persiste nem enfileira op, senão a operação voltaria ao servidor em laço) e só então anuncia, lendo de volta o que ficou na tela. O filtro por mapa não é detalhe: sem ele um colega editando outro mapa do atlas troca a base debaixo de quem está em outro lugar. Regressão em `frontend/tests/integration/basemap-remoto-na-tela.test.js`.
- **Persistir e emitir não é aparecer na tela: a CENA precisa escutar o evento.** O mapa 2D repinta porque `layers/remote-feature-render.js` assina os eventos de feição remota; o visualizador 3D não assinava `MARKERS_3D_CHANGED`, e por isso o marcador do colega só surgia depois de fechar e reabrir o 3D, que era a única repintura a partir do store. Quem ligar uma cena nova a este fluxo tem duas obrigações que o 2D não tem: coalescer a rajada (um lote de sync emite um evento por op) e **não desmontar o que o usuário local está fazendo** (`syncMarkersFromStore` reconcilia por id e preserva a seleção, enquanto `refreshMarkersForCurrentTileset` limpa tudo e desseleciona, o que é certo numa troca de mapa e seria uma regressão aqui).
