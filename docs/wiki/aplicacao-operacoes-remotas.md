...
- **O MAPA BASE e o INTERRUPTOR TEMPORAL são a exceção DELIBERADA a esta regra, desde 2026-09-20.** A op `baseLayer` e o `ativo` da op de config temporal são gravados no documento e NÃO mexem na tela de quem recebe: os dois viraram estado de vista de cada pessoa, e o que chega do par é a vista SALVA do mapa, que vale na próxima entrada ([[vista-da-pessoa-e-vista-salva]]). A história curta, porque ela se repete: o handler emitia `BASE_LAYER_CHANGED` direto e o cartão do seletor anunciava uma base que o MapLibre não desenhava (quem chama `map.setStyle` é o `BaseLayerControl`, que EMITE aquele evento e nunca o ouviu); em 2026-09-16 o conserto foi fazer o controle ouvir um evento próprio e aplicar o estilo, e foi esse conserto que expôs o defeito de produto, que era um colega trocar a base debaixo de quem está trabalhando. Emitir e não persistir continua sendo defeito para todo o resto desta lista; para estes dois, persistir e NÃO repintar é o contrato, preso em `frontend/tests/integration/remote-operation-handler.test.js` e em `frontend/tests/integration/mapa-base-e-vista-da-pessoa.repro.test.js`.
