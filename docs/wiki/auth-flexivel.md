...
- **Uma amarra que olhasse só `req.authVia` derrubaria o app inteiro, e isso foi MEDIDO.** O cliente logado manda `Bearer` **e** carrega o cookie (mesma origem, o navegador o envia sozinho), então até 2026-09-19, quando o cookie tinha precedência de resolução, `authVia` era `'cookie'` em toda requisição do app: a primeira versão da amarra recusava por isso e quebrou a criação de atlas. Quem pegou foi a captura de UI, **não** a suíte, e a razão vale depois da inversão de precedência: cada caso da suíte manda UMA credencial por vez, que é a combinação que o navegador real nunca faz. Com o Bearer decidindo, a troca silenciosa de conta (cabeçalho de uma, cookie de outra) só aparece num caso que mande as duas juntas.
...
- A renovação **não é exclusiva do cookie**. Uma chamada autenticada por Bearer também recebe `Set-Cookie` perto da expiração, ganhando um cookie que ela não pediu. Isso morde uma integração que guarde cookies (um cliente HTTP com jar ligado): a partir dali ela passa a carregar duas credenciais. Desde 2026-09-19 esse cookie não desloca mais o Bearer explícito; ele existe para as leituras que não aceitam cabeçalho nenhum (tile do MapLibre, `img.src`, `<video src>`).
...
- **2026-09-19.** Bearer explícito passou a preceder cookie: um cabeçalho de outra conta jamais autoriza como a conta do cookie, e Bearer inválido segue anônimo em vez de herdar a identidade ambiente. A reconciliação viva passou a alcançar as leituras só-flexíveis fora da janela de renovação; o memo de autorização privada dos assets 3D mantém o teto próprio de 30 segundos, para não consultar o banco por fragmento. Preso por `backend/tests/integration/flexible-auth-precedence.test.js` e `backend/tests/integration/sharing-privacy-edge-cases.test.js`.
