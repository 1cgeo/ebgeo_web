...
As finuras que se perdem numa reescrita desatenta: a medição de disco tem prazo PRÓPRIO e guarda de voo, porque `fs.statfs` roda no mesmo threadpool de quatro slots que grava o `.jsonl`, e uma medição por intervalo num volume pendurado satura os quatro em quatro amostras, levando junto o subsistema que escreve o log; a sonda ao banco distingue **prazo** de **erro** (o Postgres fora e o nosso pool entupido pedem providências opostas), e o prazo dela é MAIOR que o do `GET /api/v1/health` de propósito, porque aquele responde a um orquestrador (para quem resposta atrasada já não serve) e esta escreve história (onde distinguir "lento" de "morto" é o registro).
