...
O clone exige permissão mínima **`read`** (`backend/src/modules/atlas/atlas.routes.js:41`), enquanto duplicar um mapa *dentro* do atlas exige `write` (`backend/src/modules/atlas/atlas.routes.js:44`). Não é descuido: duplicar dentro do atlas é escrita no atlas alheio; clonar é escrita num atlas novo, seu. A consequência é forte e precisa ser entendida antes de compartilhar qualquer coisa: **quem consegue ler um atlas pode forkar o conteúdo inteiro e virar `owner` da cópia** (`backend/src/modules/atlas/atlas.controller.js:55` usa `req.user.id`, não o dono original). Inclui quem chegou por [[link-publico]] com permissão sintetizada `read`. Não trate clone como operação privilegiada; ver [[permissoes-atlas]].
