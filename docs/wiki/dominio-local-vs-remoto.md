...
**Múltiplos atlas locais nomeados deixaram de ser não-objetivo, e desde 2026-08-16 a interface expõe todos.** O registro, o teto (`MAX_LOCAL_ATLASES`) e as operações de criar, apontar, renomear, duplicar e excluir estão em `frontend/src/js/store/local-atlas.api.js`, e `atlas.html` as liga por inteiro (`frontend/src/js/projects/projects-page.js` sobre a seção local de `frontend/src/js/projects/atlas-drive.js`). Fora da tela ainda existem dois criadores: importar um `.ebgeo` com um atlas de servidor aberto ([[formato-ebgeo-roundtrip]]) e o resgate de logout.

**O estado da tela é mantido em [[namespace-por-atlas]], não aqui.** Esta frase já apodreceu três vezes descrevendo uma interface que não existia mais, e duas cópias do mesmo fato divergem sempre pela cópia que ninguém relê. O que fica desta página é a distinção entre marcador e namespace acima; o que a tela faz, e o que ela deliberadamente não faz, fica lá.

Consequência prática para o usuário: desde 2026-09-19 trocar de atlas de servidor **não apaga nada**, nem o dado do atlas que se abre nem a fila pendente do que se deixa (a fila é por atlas, e o wipe de entrada saiu do caminho normal de abertura). O que a troca faz é zerar apresentação e caches em memória (`resetAtlasView`, `frontend/src/js/store/store.js`) e deixar o retrato novo entrar numa geração própria ([[namespace-por-atlas]]). Para trabalhar offline em algo do servidor, **baixe o `.ebgeo` antes de desconectar** (ver [[formato-ebgeo-roundtrip]]).

## Ordem, que é onde está a corretude

Duas ordens não são estilo, são o que torna o crash seguro:

- **`markStoreRemote` ANTES do `connect`** (`frontend/src/js/account/open-atlas.service.js`). É intenção durável: se a aba morrer no meio do pull, o boot guard vê `remote` e descarta o parcial em vez de promovê-lo a atlas local permanente. E o `catch` **não** rebaixa mais a origem: desde 2026-09-19 ele desfaz só a conexão e a reivindicação da aba (`disconnect({ forgetAtlas: true })` e `retractAtlasClaim`, `frontend/src/js/account/open-atlas.service.js`), porque o `markStoreLocal()` que morava ali transformava dado de servidor em área de trabalho irrestrita a cada erro de rede.
...
**O marcador REMOTE decide sozinho cada vez menos, e desde 2026-09-07 ele não decide o segundo apagamento.** O boot deslogado tem duas guardas: a primeira destrói TODO namespace de servidor registrado, e só depois o marcador decide se o atlas montado também tem de ser esvaziado. Essa segunda pergunta era feita sobre o ATLAS que o marcador nomeia (`purgeReachedAtlas`), e um marcador órfão, com um `atlasId` que registro nenhum conhece, respondia "não alcancei" e mandava o apagamento para os bancos SEM SUFIXO, que são o acervo local do usuário: 639 de 647 registros, medidos em navegador. Hoje a pergunta é também sobre os BANCOS que o apagamento atingiria, e ele não roda quando uma entrada LOCAL do registro global reivindica o sufixo vazio, porque essa entrada só nasce de um boot de origem LOCAL ou do degrau 3.0 depois do descarte do resíduo. Duas coisas seguram o conserto no lugar: a pergunta é feita UMA vez, no guarda, e ANTES de `activateBootAtlasScope` criar a entrada que ela procura (feita uma linha depois, a instalação pré-namespace responderia "reivindicado" também, e o apagamento que existe para ela nunca mais rodaria); e a leitura do marcador em `reconcileWithRegistry` continua sem vetar nesse caso, de propósito, porque ela roda duas vezes num boot de fixture 2.2. Ver [[namespace-por-atlas]] e [[sessao-boot-e-ciclo-de-vida]].
...
1. **Anti-vazamento no flush.** `frontend/src/js/store/sync/operation-dispatcher.js` descarta a op de `mapId` não-UUID antes de enfileirar: no servidor `maps.id` é UUID, então ela não teria a que se ligar. Ops atlas-level passam `mapId = null` e não são afetadas. **A justificativa forte desta defesa caiu em 2026-08-30, e a página dizia o contrário:** o `mapId` não-UUID levantava 22P02 numa consulta feita FORA do savepoint por operação, e **uma op ruim derrubava o lote inteiro** (400 a cada 1,5 s, sync parado). As duas recusas que consultam o banco desceram para dentro do savepoint, então hoje ela volta recusada individualmente. O descarte segue certo pelo motivo original (o `Principal` local não pode vazar), e não mais porque o servidor não sobreviveria a ele.
...
A precedência real é link público, depois deep link `?atlas=<uuid>`, depois `openAtlasChooserOnBoot` (`frontend/src/js/index.js`), que **abre o seletor** e, desde 2026-09-23, não descarta nada antes (o descarte levava a fila do atlas cuja abertura acabara de falhar; ver [[sessao-boot-e-ciclo-de-vida]]). A URL é a fonte de verdade do que reabrir; o marcador só decide o que descartar. O F5 reabre o atlas apenas porque `syncAtlasUrl` (`frontend/src/js/deep-link/atlas-url-sync.js`) mantém `?atlas=<uuid>` na barra de endereços enquanto há conexão, e a limpa assim que a sessão deixa de estar autenticada.
...
- **2026-09-19.** Duas afirmações desta página descreviam a abertura de atlas de servidor como destrutiva e o `catch` dela como rebaixamento de origem, e as duas deixaram de valer no mesmo commit: a abertura normal chama `resetAtlasView` em vez de `clearAllDataStore`, e o erro de rede preserva a procedência REMOTA. Supersessão, não contradição; o porquê e a alternativa recusada estão em [`decisions-2026.md`](../decisions/decisions-2026.md).
- **2026-08-23.** A frase "o produto expõe só a criação", com o corolário de que trocar e excluir não tinham tela e de que o slot anterior ficava no disco sem caminho de volta, descrevia o mundo anterior a 2026-08-16: `frontend/src/js/projects/projects-page.js` liga `createLocalAtlas`, `setCurrentLocalAtlas`, `renameLocalAtlas`, `duplicateLocalAtlas` e `deleteLocalAtlas` nos handlers da seção local do Atlas Drive. Supersessão, não contradição. Em vez de reescrever o fato aqui, ele passou a ser um ponteiro para [[namespace-por-atlas]]: era a terceira vez que esta linha apodrecia, e o padrão de corrigir a cópia é o que produz a quarta.
