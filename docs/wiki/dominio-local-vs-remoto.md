...
> **Nota histórica.** guia *ui-ux-ebgeo* (absorvido) §2 diz "O boot não passa pelo Drive: F5 reconecta o último atlas automaticamente". Em `src/js/index.js:272-283`, um boot autenticado numa URL nua (`/`) **abre** o Atlas Drive e descarta o dado remoto anterior. O F5 só reabre o atlas porque `deep-link/atlas-url-sync.js:31-35` mantém `?atlas=<uuid>` escrito na barra de endereços enquanto há conexão.
