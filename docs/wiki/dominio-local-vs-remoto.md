...
Um único IndexedDB guarda dois domínios, o workspace local permanente e a cópia efêmera de um atlas do servidor, discriminados por um marcador de origem (`src/js/store/store-origin.js`) e não por namespacing.
...
- **`markStoreRemote` ANTES do `connect`** (`src/js/account/open-atlas.service.js:60`). É intenção durável: se a aba morrer no meio do pull, o boot guard vê `remote` e descarta o parcial em vez de promovê-lo a atlas local permanente. E o `catch` reverte para `markStoreLocal()` (`src/js/account/open-atlas.service.js:69`), senão um F5 retentaria eternamente um atlas morto (403/404).
- **`markStoreLocal()` DEPOIS dos clears** (`src/js/store/store.js:213`). `clearAllDataStore` apaga o appStore, onde o próprio marcador mora. Marcar antes seria apagar a marcação.

## Armadilhas

**`atlasId` é campo morto.** O marcador persiste `{kind, atlasId}` e há teste pinando a persistência (`tests/store/store-origin.test.js:42`), mas **nenhum código de produção lê `atlasId`**: só `kind` é consultado (`index.js:279`, `src/js/store/sync/permission-guard.js:71`, `src/js/locking/map-lock.controller.js:86`, `src/js/store/store.js:139`). Não escreva código novo assumindo que ele é a fonte de verdade do atlas conectado, use `syncEngine.atlasId`.

**Duas listas paralelas de clear.** `clearAllDataStore` e `enforceLocalStoreWhenLoggedOut` (`src/js/store/store.js:137`) apagam o mesmo conjunto de side-stores em código duplicado. Adicionou um side-store persistido? Ele tem que entrar nas **duas**, senão dado remoto sobrevive ao logout. O código não força isso de forma alguma.

**Replique a cláusula local em todo gate por papel.** `checkPermission` libera tudo quando `!isRemoteStoreSync()` (`src/js/store/sync/permission-guard.js:71`) e `isReadOnly()` retorna `false` de saída (`src/js/locking/map-lock.controller.js:86`). Um gate novo que consulte `sessionContext.role` cru quebra a edição do workspace local de um usuário logado cujo papel global seja restritivo. Ver [[permissoes-atlas]].
...
1. **Poison pill no flush.** O backend rejeita `mapId` não-UUID com Postgres 22P02, e **uma op ruim derruba o lote inteiro**, travando todo o sync. Por isso `src/js/store/sync/operation-dispatcher.js` descarta antes de enfileirar (`:120`, `:133`, `:192`, `:266`). Ops atlas-level passam `mapId = null` e não são afetadas.
2. **Sombreamento no connect.** Leitura por nome acerta direto a chave de armazenamento: um `Principal` local coexistindo com um mapa de atlas homônimo faz `repo.getMap('Principal')` acertar o **stray vazio**. `activateAtlasInitialMap` apaga strays não-UUID antes de resolver (`src/js/store/map.operations.js:363`).
3. **Criação com sync ativo.** `addMap` nasce UUID-keyed quando o logging está ligado (`src/js/store/map.operations.js:184`), senão uma reaplicação de snapshot duplicaria o mapa em vez de atualizá-lo.
4. **Exibição.** `getAllMapNamesStore` resolve UUID→nome (`src/js/store/map.operations.js:102`), senão o mapa de um peer aparece como UUID cru.
...
Remoto → local é "Salvar projeto" (exporta `.ebgeo`), a única forma suportada de levar um atlas do servidor para uso offline. Local → remoto é `src/js/import_export/save-local-atlas.service.js`, que **não** conecta: o serviço empacota e importa preservando IDs do cliente, e o chamador da UI faz `clearAllDataStore` + `markStoreRemote` + `connect` depois (`src/js/import_export/save-local-atlas.service.js:11`). Ver [[atlas-import-offline]] e [[clone-atlas]].
...
> [!CONTRADICAO 2026-07-18] O comentário de boot em `src/js/index.js:148` ainda descreve "otherwise reconnect the last remote atlas for a restored authenticated session", mas `index.js:160` chama `openAtlasChooserOnBoot()`. Comentário desatualizado, o comportamento é o seletor.
