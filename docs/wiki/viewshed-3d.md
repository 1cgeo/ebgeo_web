...
A translucidez protege o quadro DESENHADO e não a passada de seleção. No Firefox a profundidade que `pickPosition` lê pode conter a linha translúcida da prévia, e o segundo clique media a distância até o fio de arame, com câmera e ponteiro parados (medido em 2026-09-22); o comentário anterior dava o caso por impossível porque a leitura de profundidade translúcida do Cesium nasce desligada, premissa que só o Chromium confirmava. `pickScenePosition` (`frontend/src/js/3d_models_viewer_tool/services/viewshed-3d.js`) esconde as três coleções da prévia só durante aquela passada síncrona e as devolve antes do quadro seguinte.
