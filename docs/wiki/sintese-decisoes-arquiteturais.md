...
O produto nasceu local-first (IndexedDB + arquivos `.ebgeo`) e o backend foi acrescentado sem virar pré-requisito do caminho de edição. Tudo que funcionava sem conta continua idêntico; login é capacidade extra, nunca porta de entrada.
...
O servidor **é** pré-requisito para o app *subir*. `GET /api/config` é fonte única de config e catálogo, e o boot é fail-fast: 3 tentativas com 1s de intervalo e, sem resposta, a tela "EBGeo indisponível" (`src/js/index.js:73-86`, `CONFIG_BOOT_ATTEMPTS = 3` / `CONFIG_BOOT_RETRY_MS = 1000`). Não existe config estático de reserva; o `config.js` embarcado é só o *shape* que o servidor hidrata.
...
O estado de salas e presença é um `Map` **em memória por processo** (`backend/src/modules/collab/collab.rooms.js:6`, `const rooms = new Map()`). Não há Redis, pub/sub ou backplane em lugar nenhum do backend.

Isso significa, em termos duros:
...
Decisão correlata de armazenamento: **BLOB pesado não vai para o Postgres**. Binários WebP do 360 ficam em SQLite por projeto (`{orgId}__{slug}.db`, ~41 GB no dataset real) e assets 3D em SQLite mais fallback de FS; o Postgres guarda metadado e ponteiro. O nome do `.db` é derivado server-side de `(orgId, slug)` e o `db_filename` do manifest do cliente é **ignorado**, como guard anti-overwrite cross-OM. Ver [[assets3d-distribuicao]], [[catalogo-3d]] e [[ingestao-projetos-360]].

## Persistência: JSONB e PostGIS coexistindo isolados por schema

Um banco, três schemas: `public` (atlas, maps, features com geometria em **JSONB**, sem PostGIS, mais users/orgs/audit/operations), `ng` (gazetteer PostGIS, catálogo 3D, zonas) e `sv360` (metadados do 360). PostGIS foi **aditivo**: não converteu o schema do atlas.

Armadilha de deploy: a migração `004` faz `CREATE EXTENSION postgis`, que é untrusted e exige superusuário, e roda **incondicionalmente**. Logo **PostGIS é pré-requisito de qualquer deploy completo, mesmo um deploy só do atlas**. Ver [[deploy-backend]], [[gazetteer-nomes-geograficos]] e [[zonas-acesso-geografico]].

Migrações são **forward-only, sem rollback, rastreadas por NOME de arquivo**. Nunca renumere, renomeie ou reordene uma migração já aplicada; para corrigir defeito, adicione a próxima. A mesma convenção forward-only levou o schema de comentário a entrar **editando o baseline `002_atlas.sql` in-place**, não numa migração nova.

No cliente, o mesmo princípio: migrações de schema do IndexedDB são forward-only e aditivas (v1→v2→v2.1→v2.2), rodam no boot e não destroem dado. **Invariante de migração: atualizar o app nunca pode apagar ou tornar inacessível um atlas que já estava em operação.**
...
A permissão por atlas é um **terceiro** eixo, resolvido em waterfall: owner → share → público → 403, na hierarquia `owner > manage > write > comment > read`. Ver [[permissoes-atlas]], [[sintese-eixos-de-permissao]] e [[sintese-capacidades-por-papel]].
...
- **Visualizador não recebe comentários do servidor** — é **filtro de transmissão** (snapshot e broadcast não enviam), não esconde-UI. Ver [[comentario-espacial]] e [[presenca-colaborativa]].
- **"A permissão padrão abaixa, nunca eleva"** — convite concede Leitura por padrão; elevar é ação deliberada.
- **Config por atlas é interseção, nunca expansão** — um `atlas.settings` só desliga o que o deploy suporta, jamais liga o que não existe, e é revertido ao desconectar. Ver [[atlas-settings]] e [[modos-operacao]].
...
- **Observabilidade de sync é test/dev only** — o SyncLedger é env-gated (`EBGEO_TRACE=1` ou `NODE_ENV=test` no backend) e em produção é branch morto. Ver [[syncledger]].
...
Nem tudo que o app faz atravessa o motor de operações. Gazetteer, catálogo 3D, assets e panoramas 360 são **REST read-only** sobre PostGIS, com autorização embutida na própria query SQL (um dado privado só aparece para admin, permissão direta ou geometria contida numa zona do usuário). Ver [[sintese-modulos-fora-do-sync]], [[ranking-busca-toponimos]], [[sync-admin-operacoes]], [[atlas-import-offline]], [[hardening-borda-api]], [[api-keys]], [[auditoria]] e [[modulo-temporal]].
