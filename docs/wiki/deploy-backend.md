...
- **Nunca renomeie nem renumere** migração já aplicada: o tracking é por nome, renomear reaplica o DDL.
  A ÚNICA exceção é o esmagamento deliberado do histórico enquanto nenhum banco de produção o aplicou, e ele já ocorreu DUAS vezes (a última em 2026-08-19, quando 22 arquivos incrementais viraram 8 baselines por domínio). Depois de um esmagamento, banco pré-existente **tem de ser recriado**: não há caminho de upgrade, e o próprio schema recusa a tentativa com mensagem própria (a guarda no topo de `backend/src/database/migrations/001_identidade.sql`) em vez do enigmático "relation already exists" que o runner produziria sozinho.
