...
Ele **não autoriza nada**. Quem autoriza é o JWT ([[autenticacao-jwt]]) e a permissão por atlas resolvida no handshake ([[permissao-vs-papel]]). Forjar o `clientId` de outro cliente não dá acesso a nada, só bagunça presença e de-duplicação.
...
- ele garante que a **fila offline** reenviada após a reconexão continue sendo reconhecida como do mesmo cliente (ver [[fila-operacoes-pendentes]]);
...
- `docs/guias/04-websocket-collab.md`: parâmetro `?clientId=` no handshake, regex `^[a-zA-Z0-9_-]{8,64}$` e fallback do servidor, eco em `connected.sessionId`, contrato congelado, janela de graça `away`/`user_back` (§4), broadcast REST sem exclusão do emissor (§3.9), semântica de `idempotent` no ack (§3.5).
- `docs/guias/08-offline-import.md`: uso do mesmo `ebgeo_client_id` do localStorage ao carimbar operações de update geradas na importação offline (§4.5).
