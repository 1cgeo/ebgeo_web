...
- **O servidor mede o RTT de cada destinatário, mas só para a presença, e não alimenta este monitor.** Desde 2026-09-24 todo quadro de presença sai seguido de um ping de protocolo com sequência, e o pong dá o RTT daquele socket, que decide quando a próxima presença sai (`aoPong`, `backend/src/modules/collab/collab.fluxo.js`; ver [[canal-collab-websocket]] §Backpressure). Esse valor não vira banda, não gera `adaptive-settings` e não muda a cadência do flush: o laço deste monitor continua aberto nos dois extremos, como descrito acima.
...
- **`truncateCoords` parece dead code e não é.** Sem call site em `backend/src/`, coberta só por `backend/tests/unit/collab-quality.test.js` e marcada como preservação deliberada em `backend/src/modules/collab/collab.quality.js`. O que a segura são essas duas coisas e nada mais: **nenhuma varredura de dead-code alcança o backend**, porque o `knip` da raiz delega só ao frontend (`package.json`) e o pacote do backend não tem nem script nem dependência de `knip`. A propriedade a reter é a inversa da intuição: aqui a função sobrevive por leitura humana e por um teste, não por estar fora do alcance de uma ferramenta que alguém acha que rodou.
