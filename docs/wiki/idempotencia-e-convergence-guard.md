...
**A guarda se decide DENTRO da trava do documento, e decidir antes dela reabre a janela que ela existe para fechar** (2026-09-22). `applyRemoteOperation` lia o contador de edição pendente e a versão antes de `applyRemoteFeatureOp` tomar `withMapDocument`, de modo que a operação aprovada ficava na fila da trava atrás do gesto seguinte do autor e escrevia depois dele. A vítima não era só a op de par: o recibo do próprio autor passa pelo mesmo caminho, porque `resolveLocalEdit` reaplica a operação canônica, e um ACK atrasado apagava a edição feita enquanto ele esperava. `featureApplyPermission` refaz as duas perguntas com a trava já tomada e devolve três desfechos, aplicar, descartar como superada ou `FEATURE_DEFERRED`, que entrega a operação à fila de adiamento sem anunciar mudança nenhuma. A interleaving perdedora está forçada, sem estatística de navegador, em `frontend/tests/integration/convergencia-autor-vencedor.repro.test.js`: três esperas (recibo próprio antigo, op de par mais antiga, op de par vencedora) e um recibo novo que chega durante a espera, os quatro vermelhos contra o código anterior.
