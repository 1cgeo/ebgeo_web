...
**Uma chave PODADA pelo mesmo desenho: `map2d.globe_projection`.** Desde 2026-09-22 ela não existe mais: a projeção é do atlas, globo por padrão ([[atlas-settings]]), e a chave passou de 2026-08-16 àquela data gravada pela caixa "Projeção globo" do painel e servida sem nenhum leitor no cliente. O schema a recusa com 422 nomeado, pela mesma razão do zoom, e `podarProjecaoDoPainel` a apaga da linha gravada nas duas passadas. A diferença para o zoom: aqui não há valor fixo a repor depois da poda, a chave simplesmente some, e a poda de leitura mora em `getConfigOverrides`, então alcança também o eco `overrides` de `GET /config/admin`. Uma linha velha não reprova o salvamento seguinte, porque o schema valida só o corpo que chega.

**Uma chave com borda de EXISTÊNCIA, ao contrário das vizinhas: `map2d.defaultBasemap`** (2026-09-23). É a base com que o mapa nasce e com que nasce todo documento de mapa novo, escolhida na aba Sistema; o padrão em `MAP2D_BASE` é `carta-topografica`, o valor da antiga constante do cliente. Além do tipo no Joi, `assertDefaultBasemapKnown` recusa com 422 o id que não esteja entre os mapas base PÚBLICOS servidos, porque este id é GRAVADO em documento (o mapa em branco da primeira visita, e o primeiro mapa de um atlas criado no servidor, por `getDefaultBasemap` em `createAtlas`), enquanto `terrainPreferredBasemap` e `miniMapBasemap` só desenham e deixam o id inexistente para o recuo do cliente. O que a borda não alcança (a base apagada, desabilitada ou tornada privada depois de salva) o cliente resolve no nascimento, com `initialBaseLayer` (`frontend/src/js/baselayers/base-layer.control.js`). Um mapa que já existe NÃO segue a escolha: ele abre na base que o documento dele carrega.
