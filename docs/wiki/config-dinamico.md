...
Duas leituras erradas ainda circulam: que as fontes são **três** (esquecendo os overrides de admin, que vencem todas as outras), e que o catálogo vem de uma **única tabela `resources`**. É uma tabela e uma rota CRUD **por tipo** (`backend/src/modules/catalog/catalog.tables.js:5-11`, `backend/src/app.js:102-106`); rota `/api/v1/resources` não existe. Ver [[resources-catalogo]].
...
**Duas das oito consultas são a mesma, e isso é conhecido.** `listBasemaps` e `listBasemapStyles` chamam ambas `catalogService.listCatalog('basemaps')` e entram lado a lado no mesmo `Promise.all` (`backend/src/modules/config/config.service.js:116`, `:130`, montadas em `:188`): projetam campos diferentes da MESMA linha, uma tirando `config.style` e a outra usando só ele. Ficou assim de propósito ao fechar a auditoria de backend de 2026-07: deduplicar exige mudar a assinatura exportada das duas funções, e o que se ganha é uma consulta entre oito que, desde a memoização, custam por *mudança* e não por requisição. **O gatilho para revisitar é a memoização sair ou passar a variar por chamador**; enquanto ela valer, a duplicata é o item mais barato da lista.
