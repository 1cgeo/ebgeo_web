...
**`catalogLayer` teve DUAS casas e hoje tem uma.** Até 2026-08-18 a mesma camada podia morar na tabela dedicada `catalog_layers` e numa coluna homônima de `maps`, e o backend decidia pelo shape do payload. O modo dual custou caro em silêncio: o merge de mapas movia a tabela e não a coluna, então quem usasse a forma de array perdia as camadas ao mesclar; e a coluna saía crua por rotas que não passam pela reidratação do snapshot, servindo `config.source.url` de recurso privado a quem só tem `read`. A coluna foi apagada, e o ramo de array do handler passou a materializar cada item como linha da tabela (upsert, nunca remoção).

**E a forma de LISTA saiu inteira em 2026-09-13**, recusada por nome antes do log (`catalogLayerArrayDenialReason`, `backend/src/modules/sync/sync.service.js`). O motivo não é arrumação: ela **não endereçava uma linha**. O `entityId` dela era o id do mapa ou um UUID descartável e o payload era a lista, de modo que não havia entidade sobre a qual observar uma base, e a verificação por base e revisão ([[modelo-conflito-lww]]) não tinha o que comparar: o replay literal era barrado pelo recibo, e o FORA DE ORDEM escrevia a linha viva com o que o remetente ainda carregava. Nenhum cliente vivo a emitia (o cliente escreve entrada por entrada em `frontend/src/js/store/catalog.operations.js`), e o que a substitui é uma op por camada, no mesmo push. Duas coisas continuam valendo: a chave `catalog_layers` do payload de IMPORT de atlas não é op de sync e segue aceita; e o shape de lista continua sendo ENTENDIDO pela poda de definição e pelos extratores de referência, porque o log é append-only e ainda guarda ops de lista escritas antes daquela data, que um pull incremental replica de saída. Ver [[api-rest-atlas]].
...
**A instância viva da colisão é o `setting`, e ela nunca esteve escrita.** `logAtlasSetting` usa o id do atlas (ou o sentinela) como `entityId` para todas as preferências, e cada op carrega um patch de uma chave só. Seis chaves compartilham esse grupo hoje: `mapOrder`, `mapBadgeColors`, `colorUsage` e `customIcons` por `logAtlasSetting`, mais o par de aparência `terrainExaggeration` e `globeProjection` (`APPEARANCE_KEYS`, `frontend/src/js/store/atlas-appearance.service.js`), que chega por `logSettingOperation` direto. O par não conta como duas ops concorrentes porque é escrito num patch só, e é exatamente esta colisão que obriga a isso (é a razão dita na nota sobre exagero de terreno, acima). A compactação **substitui o payload, nunca o funde**: enfileirados juntos, um sobrevive e os demais somem sem erro. O dano é latente porque `_compact` só roda acima de `MAX_QUEUE_SIZE`, alcançável numa rajada offline e não no uso normal.
