...
Uma inconsistência interna do schema, em migração já aplicada e portanto congelada (mexer nela violaria a regra de migrações, então fica registrada aqui em vez de corrigida): o trigger faz `SET current_version = NEW.server_version` sem `GREATEST` (`backend/src/database/migrations/004_sync.sql`), apesar de a semântica pretendida ser um máximo. Coincide na prática (sequência crescente, push transacional), mas não raciocine sobre inserções concorrentes assumindo máximo.

**O CHECK do banco e a lista do cliente divergem em dois tipos, e a divergência é desenho.** A constraint `valid_feature_type` (`backend/src/database/migrations/003_atlas.sql`) aceita 21 tipos; `SOURCE_TYPES` (`frontend/src/js/store/store.constants.js`) tem 19, porque exclui `processed_los` e `processed_visibility`, que são saídas de análise, desenhadas e nunca selecionadas. Quem decide a exclusão é a capacidade `selectable` no registro de tipos (`frontend/src/js/store/feature-type.registry.js`), não uma lista à parte. O que morde ali: o mapeamento de armazenamento dessas duas é o nome VERBATIM, e não `source + 's'`, porque o fallback produziria `processed_loss` e a feição sincronizada cairia num bucket fantasma no peer receptor, sem nunca renderizar. O registro é onde um tipo nasce; as constantes derivadas dele não se editam à mão.
...
1. Op cujo `mapId` de contexto não é UUID é descartada **antes da fila** (`frontend/src/js/store/sync/operation-dispatcher.js`), porque ela nunca poderia se ligar a nada do outro lado: no servidor `maps.id` é UUID. O motivo mudou de peso em 2026-08-30, e vale registrar porque a frase antiga vive em várias páginas: até ali o Postgres rejeitava com 22P02 numa consulta feita FORA do savepoint por operação, e **uma** op inválida derrubava o lote inteiro do flush (400 reenviado a cada 1,5 s, sync parado). Hoje as duas recusas que consultam o banco correm dentro do savepoint, então essa op volta recusada individualmente e o lote sobrevive; o descarte pré-fila continua sendo o certo (é anti-vazamento do `Principal` local), mas ele deixou de ser a única coisa entre uma op e a fila travada. Op de `SETTING` escapa apenas com UUID ou o sentinela literal `'atlas'`.
...
Metadados de atlas são REST; feições, mapas, camadas, grupos, briefings, slides, 3D, 360 e comentários são **sync-only**, sem rota REST de escrita ([[sintese-rest-vs-sync]], [[envelope-operacao]]). Rotas em `backend/src/modules/atlas/atlas.routes.js`, autoexplicativas.
...
- **`features.layer_id` é UUID puro, SEM FK, e a ausência é load-bearing.** `backend/src/database/migrations/003_atlas.sql` declara só `layer_id UUID,` enquanto as irmãs do mesmo arquivo declaram `REFERENCES` (`groups.parent_id`, `group_features.group_id`). A ausência parece esquecimento e não é: pela mesma razão do bullet acima, uma feição pode citar camada cujo create ainda não chegou ou foi eliminado pela compactação da fila, e uma FK ali transformaria isso em `23503`, que hoje descarta a feição em silêncio (e, antes de 2026-07-25, travava a fila daquele cliente para sempre). **Integridade referencial não é imponível num log de aplicação por ordem de chegada.** Registre isto antes de "consertar": acrescentar a FK que falta parece higiene de schema e é regressão de sync. O porquê também vive no ponto de uso, no comentário junto da coluna em `backend/src/database/migrations/003_atlas.sql`.
...
- 2026-08-23: a seção dos contadores apontava um comentário `-- 18 valid feature types` acima de `features.feature_type` como uma das "duas inconsistências internas do schema". O rebaselinamento das migrações levou o comentário; sobrou o fato durável, CHECK com 20 e cliente com 18. Na mesma linha, a página mandava dar entrada explícita em `FEATURE_TYPE_MAPPINGS`, o que contradiz o registro de tipo de feição adotado em 2026-08-16: aquela constante é derivada, e um tipo nasce em `frontend/src/js/store/feature-type.registry.js`.
- 2026-08-23: duas citações a um `002_atlas.sql` (que não existe) foram trocadas pelo caminho completo de `backend/src/database/migrations/003_atlas.sql`, que a própria página já citava certo em outros pontos.
