...
**E no ramo INCREMENTAL a ordem certa é a OPOSTA, o que é a armadilha inteira desta função.** Ali a fronteira (`min_version`, por `getAtlasSyncInfo`) é lida DUAS vezes, e a que decide é a SEGUNDA, depois de `GET_OPERATIONS_SINCE_VERSION` ter trazido a cauda: se ela subiu no meio, a resposta vira snapshot (`historyWasPruned`). O motivo é que a limpeza administrativa commita o apagamento das linhas e o novo `min_version` na mesma transação, então uma poda que caia entre as duas leituras devolveria cauda vazia ou incompleta **com o cursor avançado**, e a edição que faltou ficaria permanentemente fora do replay incremental, sem erro em lugar nenhum. Ler a fronteira só no fim é o que torna a verificação suficiente: uma poda posterior já não pode invalidar linhas que esta consulta capturou. Ou seja, no snapshot o cursor vem ANTES dos dados e no incremental a fronteira vem DEPOIS deles, e "uniformizar" as duas quebra uma das duas. O preço declarado é uma ida a mais ao banco por pull incremental. Guarda: `backend/tests/integration/sync-version-cursor.test.js`, que suspende a leitura do log com trava de tabela e executa a poda numa segunda transação.
...
- **O zero dizia DUAS coisas, e quem as separa é `haveSnapshot`.** "Não tenho nada" e "estou em dia com um atlas que nunca teve op" eram o mesmo `lastVersion: 0`, e todo atlas fica em `current_version = 0` até a primeira escrita: a abertura de um atlas novo era servida com dois retratos completos idênticos, o do pull HTTP e o do handshake. Desde 2026-09-13 o `sync_request` carrega `haveSnapshot: true` quando o estado local está COMPLETO na versão pedida, e com o campo presente `pullOperations` lê o zero como uma versão qualquer e responde cauda vazia. O campo só viaja quando é verdadeiro: a ausência continua significando "manda tudo", que é o que um cliente antigo diz e o que um cliente sem prova de completude precisa dizer. Abaixo de `min_version` continua sendo retrato, com campo ou sem.
- **`initialPull: false` deixa `_lastVersion` em 0**, porque a atribuição está dentro do `if` (`frontend/src/js/store/sync/sync-engine.js`). O socket abre com `lastVersion: 0` e sem `haveSnapshot`, e o `sync_request` seguinte pede o mundo inteiro, que aqui é o certo: sem pull inicial não há prova de que o disco tenha coisa alguma.
...
`applyRemoteSnapshot` grava cada mapa e briefing do snapshot e **nunca apaga entidade local ausente dele** (`frontend/src/js/store/sync/remote-operation-handler.js`). Quem garante a substituição não é mais um wipe: o retrato é encenado numa GERAÇÃO própria e o ponteiro só é publicado ao fim, então a entidade que o servidor não mandou simplesmente não existe na geração nova ([[namespace-por-atlas]]). O `clearAllDataStore()` antes do `connect` continua nos caminhos que TROCAM o endereço sem retrato garantido (`frontend/src/js/account/account.control.js`, que sobe o atlas local para o servidor, e o caminho de `?atlasPublico=` em `frontend/src/js/index.js`); `openRemoteAtlas` deixou de chamá-lo em 2026-09-19 e chama `resetAtlasView`, que é só memória. Pelo mesmo motivo de antes, um `resync()` no meio da sessão não remove localmente o que um peer deletou por fora do log.
...
- **O prazo de `pullSync` conta SILÊNCIO, não duração** (desde 2026-09-23; a frase anterior desta linha, "`pullSync` não tem timeout", era falsa desde que os prazos fixos de rede entraram). O corpo é lido em stream e cada pedaço rearma o prazo, então um retrato de vários MB abre num link de 40 kbps e um corpo que PARA de chegar continua sendo cortado. Um prazo total fixo cortava o retrato em toda tentativa e o atlas nunca abria naquele link (P6: transferência grande em rede ruim não pode ser abortada pelo tamanho).
