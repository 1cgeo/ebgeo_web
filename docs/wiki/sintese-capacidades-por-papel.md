...
Admin global curto-circuita tudo: vira `admin` na tradução (`backend/src/utils/roles.js:13`) e recebe `req.atlasPermission = 'owner'` em qualquer atlas (`backend/src/middleware/permissions.js:82-87`). Ver [[sync-admin-operacoes]] e [[gestao-usuarios]].
