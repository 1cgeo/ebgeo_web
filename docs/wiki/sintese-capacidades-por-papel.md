...
A tabela `ROLE_PERMISSIONS` (`src/js/store/sync/session-context.js:60-84`) não espelha o backend. Duas folgas conhecidas:

1. **`canLockMaps` do `manager` é folgado demais.** `manager` compartilha `FULL_PERMISSIONS`, logo `canLockMaps: true`, mas o backend só aceita lock do `owner` (`backend/src/modules/sync/sync.service.js:616`). Quem realmente decide na UI é `LOCK_CAPABLE_ROLES = [OWNER, ADMIN]` (`src/js/locking/map-lock.controller.js:39`). **Não use `canLockMaps` como fonte de verdade.**
2. **`editor` tem `canDelete: true`**, e `DELETE_MAP → canDelete` (`src/js/store/sync/permission-guard.js:30`). Um Editor enfileira o delete de mapa localmente e só toma 403 no flush (`backend/src/modules/sync/sync.service.js:611`). Ver [[fila-operacoes-outbound]] e [[erros-api]].
...
`checkPermission` retorna `{ allowed: true }` quando offline **ou** quando o store não é remoto (`src/js/store/sync/permission-guard.js:71`). Estar logado como `viewer` não impede desenhar no store local, senão o usuário não conseguiria trabalhar no próprio workspace. Ver [[dominio-local-vs-remoto]] e [[modos-operacao]].
...
O papel do login é o **global** (`org_role`), não o do atlas (`src/js/store/sync/sync-engine.js:126`). O papel por atlas só se resolve no `connect`, e ali há uma antecipação deliberada: se o `ownerId` do snapshot bate com o `userId`, o papel sobe para `owner` **antes** do handshake WS (`src/js/store/sync/sync-engine.js:177`), para que os botões de Gestor apareçam já no F5 sem esperar o socket. O `connected` confirma depois via `payload.role`.

Transferência de posse ao vivo não exige reconexão: `atlas_owner_changed` chama `updateRole('manager')` no ex-dono (`src/js/store/sync/sync-engine.js:445-447`). Use `updateRole`, nunca `setSession`, para trocar papel de sessão viva: `updateRole` preserva `userId`/`username` (`src/js/store/sync/session-context.js:296-302`), `setSession` os substitui.
...
- `connectPublic` chama `disableOperationLogging()` (`src/js/store/sync/sync-engine.js:227`), e um `connect` autenticado posterior **precisa reabilitar explicitamente** (`enableOperationLogging()`, `src/js/store/sync/sync-engine.js:168-169`). Sem isso o usuário logado depois de visitar um link público editaria sem enfileirar nada.

No cliente, `setVisitorSession()` (`src/js/store/sync/session-context.js:264-273`) deixa `isAuthenticated()` em `false` (sem menu de conta) mas ONLINE com papel `viewer`, então o guard bloqueia a edição. Ver [[link-publico]] e [[auth-flexivel]].
