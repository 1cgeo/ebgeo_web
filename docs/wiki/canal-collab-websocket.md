...
**Armadilha de sessão longa:** a permissão é cacheada no objeto `ws` no handshake, mas um socket vive horas. Ela é reconciliada por `reconcileAuthorization` (`backend/src/modules/collab/collab.gateway.js`) no heartbeat e, desde 2026-09-19, antes da resposta HTTP a mudanças de compartilhamento, de publicação e de composição de grupo (`reconcileAtlasConnections`, `reconcileGroupConnections`). Revogação fecha com `4003`; rebaixamento apenas abaixa `ws.permission`. **A confirmação imediata alcança só os sockets DESTE processo:** para outro processo e para alteração administrativa, a janela de staleness continua sendo um intervalo de heartbeat (~30 s), e não há como encurtá-la sem mudar o intervalo.

**Expiração natural continua distinta de revogação explícita.** A varredura não chama `jwt.verify` nem olha `exp`: o JWT pode expirar sem derrubar o socket, conforme a decisão de 14/09/2026 em [`../decisions/decisions-2026.md`](../decisions/decisions-2026.md). Além da conta, OM, papel, compartilhamento e publicação, desde 2026-09-19 ela também confere `sessions_valid_from`. Um corte explícito de todas as sessões encerra conexões existentes na reconciliação; o teste é `backend/tests/ws/collab-session-cut.test.js`.

O motivo é curto: **a expiração do token não muda o que a pessoa pode fazer**, e a queda por autorização já é imposta, a cada trinta segundos, pela mesma varredura. Derrubar por `exp` cobraria um preço real (perder a sala no meio de uma edição, a cada quinze minutos, sem que nada tenha mudado sobre quem é a pessoa ou sobre o que ela alcança) para comprar uma garantia que a reconciliação já dá por um caminho melhor: ela lê o estado VIVO, enquanto o `exp` só diria que o papel de pé tem quinze minutos de idade. É a mesma família do `auth.logout`, que revoga o refresh token e não fecha socket nenhum.

A assimetria com o handshake é o ponto, e ela não é inconsistência: **no upgrade o token expirado é 401**, porque ali ele É a credencial e não há socket a preservar. O mesmo token, depois de vencido, abre zero conexões novas e mantém de pé a que já existia. Guarda dos dois lados (mais o contraste do `4003` na revogação do share): `backend/tests/ws/collab-expiracao-de-token-nao-derruba.test.js`, irmão de `backend/tests/ws/collab-logout-keeps-socket.test.js`. Os dois existem porque a propriedade é uma **ausência** dentro de `reconcileAuthorization`, e ausência não se lê: quem "consertasse" isso acrescentando uma verificação de `exp` na varredura não deixaria nada vermelho.
...
3. **O `1006` de um socket ZUMBI não é ausência, desde 2026-09-22.** Quando a conexão fica meio aberta (rede que troca, notebook que suspende), o cliente percebe primeiro, fecha o socket morto sem que o servidor saiba e reconecta com o mesmo `clientId`; o servidor só termina o velho na varredura seguinte. Esse `1006` anunciava `user_away` de quem estava online no socket novo, e nada o limpava: o socket novo não passou pelo caminho de reconexão que emite `user_back`, e a remoção ao fim da graça era muda por causa do sobrevivente. Hoje `onClose` tira o zumbi da sala em silêncio quando existe um gêmeo vivo do mesmo par `(userId, clientId)` (`temGemeoVivo`). Regressão em `backend/tests/ws/presenca-fantasma-apos-saida.repro.test.js`.
...
- **O lote sai DEPOIS do `user_left`, e isso era o fantasma do relato de 2026-09-22.** A saída é anunciada na hora e o lote no tique seguinte, então o último quadro de quem fechou a aba chegava ao par depois do anúncio e recriava a pessoa na lista, sem nome e para sempre. `removeConnection` descarta o quadro pendente de quem sai (`descartarCursorPendente`, `backend/src/modules/collab/collab.rooms.js`) no mesmo ramo que anuncia, e o cliente recusa quadro atrasado de chave que acabou de sair. Detalhe em [[presenca-colaborativa]].
- **O lote com escopo PRIVADO sai uma vez por CLASSE de acesso, desde 2026-09-22.** Um item de cursor do 3D, da cena ou do 360 cujo modelo ou projeto é privado vai inteiro só aos sockets que `fn_can_see_resource` libera no escopo do atlas, e aos outros sem escopo e sem posição; os destinatários são agrupados pelo que podem ver e cada grupo recebe uma serialização (`emitirLotesDeCursor`, `backend/src/modules/collab/collab.rooms.js`). Sem escopo privado no tique, a sala continua recebendo uma serialização só. Regra e motivo em [[presenca-colaborativa]] §"O escopo do cursor e da seleção é recortado por destinatário".
