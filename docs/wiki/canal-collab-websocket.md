...
**A varredura reconcilia AUTORIZAÇÃO, e nunca SESSÃO, e a diferença é a resposta à pergunta que mais se faz sobre este canal.** O JWT que abriu o socket vive quinze minutos; o socket vive horas. Quando aquele token expira, **nada acontece com o socket**: `reconcileAuthorization` não chama `jwt.verify` e não olha `exp`. O que ela relê do banco são os cinco eixos que decidem o que a pessoa pode fazer: a conta ativa, a OM ativa, o papel global, o compartilhamento (por pessoa ou por grupo, via `fn_user_atlas_shares`) e a publicação do atlas. Decisão de 14/09/2026 registrada em [`../decisions/decisions-2026.md`](../decisions/decisions-2026.md), e ela é **declarar, não mudar**: o comportamento é o que sempre foi.
