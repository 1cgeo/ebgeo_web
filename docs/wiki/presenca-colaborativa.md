...
**Não replique este gate de presença como gate de escrita.** Esta linha mandava conferir `permission !== 'read'` para autorizar escrita, o que era exato até `1d23ac9` (2026-07-19) e deixou de ser quando o nível `comment` chegou ao servidor: escrita é decidida por `assertOperationAllowed` (`backend/src/modules/sync/sync.service.js:973-982`), onde o Comentarista só pode escrever `target: 'comment'`. Presença e escrita têm gates com formatos diferentes de propósito, e o de escrita é hierárquico. Detalhe e consequência em [[canal-collab-websocket]].
