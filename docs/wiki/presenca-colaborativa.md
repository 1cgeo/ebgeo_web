...
**Descartável por construção, e isso é deliberado.** `sendCursorFrame` faz `if (!wsClient.isConnected()) return;` (`frontend/src/js/presence/presence-bridge.js`) e nunca enfileira: misturar presença com a fila offline faria o usuário reviver cursores de dez minutos atrás no reconnect. No servidor, `cursor`/`cursors`/`selection` passam pelo controle de fluxo por destinatário (`entregarPresenca`, `backend/src/modules/collab/collab.fluxo.js`): enquanto o destinatário não confirmou, pelo pong do ping de protocolo, que recebeu o quadro anterior, a presença para ele é RETIDA e coalescida, só a última por remetente, e sai numa descarga quando o pong chega. Num link de 40 kbps com três colegas movendo o mouse isso tirou a presença de 62% para 36% do link e a edição do par lento de até 4,9 s (crescendo) para menos de 0,7 s (medido em 2026-09-24; [[canal-collab-websocket]] §Backpressure). O mesmo laço continua terminando (`terminate()`) o socket afogado por operações duráveis, para forçar reconnect e replay (`broadcastToRoom`, `backend/src/modules/collab/collab.rooms.js`). Perder um cursor intermediário é invisível; perder uma operação é divergência. O quadro retido de quem sai da sala é esquecido junto com o pendente do lote (`descartarCursorPendente`), senão a descarga depois do `user_left` recriaria o fantasma.
