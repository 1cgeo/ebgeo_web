...
O caminho em si se lê seguindo as chamadas a partir de `runTransaction` (`src/js/store/store-transaction.js:109`) até `syncEngine.flush` (`src/js/store/sync/sync-engine.js:262`), e os módulos do sync são densamente comentados: os porquês de baixo nível (poison pill `22P02`, semente de versão no ack, filtro do próprio eco, buracos de `serverVersion`) já estão nos comentários. Esta página cobre só o que atravessa arquivos. Envelope em [[envelope-operacao]], lado servidor em [[tabela-operations]] e [[modelo-conflito-lww]].

## O flag de logging mente no comentário

`src/js/store/sync/operation-dispatcher.js:23` nasce com `enabled = false` e o comentário em `:107` diz "Offline/anonymous: logging is off, so the op is intentionally not queued". **Isso está errado na prática:** `initServices()` chama `enableOperationLogging()` no boot (`store/services.js:81`), inclusive no boot anônimo. Um usuário offline enfileira ops normalmente para qualquer mapa com id UUID.

Quem realmente contém a fila offline é a guarda de `mapId` não-UUID (que dropa tudo do mapa local `Principal`, ver [[dominio-local-vs-remoto]]) e o auto-purge de 7 dias, não o flag. Só `connectPublic` e `logoutAndDisconnect` desligam o logging (`src/js/store/sync/sync-engine.js:227,375`).
...
- **Formato da chave da fila.** `op_{timestamp}_{id}` (`src/js/store/sync/operation-queue.js:83-85`) é ordenado por sort lexicográfico (`:224-229`). Funciona porque epoch em ms tem 13 dígitos e terá até 2286. Mudar o formato quebra a ordem cronológica **em silêncio**, sem erro e sem teste vermelho óbvio.
...
**O `await` do store resolve antes de a op estar na fila.** `commit()` roda os efeitos assíncronos em fire-and-forget (`src/js/store/store-transaction.js:73-81`). Teste de integração que inspeciona a fila logo após uma mutação precisa aguardar a microtask; confiar no `await addFeature(...)` produz flake.

**Não presuma compaction.** Ela só roda acima de `MAX_QUEUE_SIZE = 10000` (`src/js/store/sync/operation-queue.js:29,102-104`). É válvula de sobrecarga, não otimização do caminho normal: 40 arrastes de uma feição viram 40 `UPDATE`s empurrados um a um.
...
**Ack com `success: false` ainda é dequeueado.** O dequeue usa os ids enviados, não os acks (`src/js/store/sync/sync-engine.js:285`). A resposta só é lida para tracing e para semear versões. Uma op recusada individualmente pelo servidor num HTTP 200 desaparece da fila e nunca é reenviada.

**Um lote reprovado bloqueia tudo atrás dele.** O `peek` é sempre da cabeça em ordem cronológica, e o lote falho não é dequeueado (`src/js/store/sync/sync-engine.js:270-283`). Head-of-line blocking indefinido. Antes de culpar a rede, confira o papel do usuário no atlas: sem permissão de escrita o push é recusado e a fila trava igual ([[permissoes-atlas]]). O gate fino é do servidor por operação; o dispatcher não checa papel.

**Perda silenciosa.** `purgeOldOperations` (7 dias, a cada 6h, `src/js/store/sync/operation-queue.js:360,378-392`) remove ops antigas sem log por op e sem evento de erro. Troca de atlas e logout limpam a fila inteira (`store/store.js:154,202` via `src/js/account/open-atlas.service.js:59`) por desenho: ops pendentes pertencem ao atlas abandonado e vazariam para o atlas errado. Nos dois casos a recuperação é snapshot ([[snapshot-e-pull-incremental]]), não replay da fila. Ver [[atlas-modelo-de-dados]].

**O gate de flush é estrito.** `connectionState.isOnline()` só aceita `ONLINE` (`src/js/store/sync/connection-state.js:62-64`); `RECONNECTING` **não** libera push, mesmo com socket em recuperação.

**Falta gatilho de evento para metade dos tipos.** `FLUSH_TRIGGER_EVENTS` (`src/js/store/sync/sync-flush.js:28-45`) cobre FEATURE/LAYER/GROUP/MAP/BRIEFING e `REMOTE_OPERATION_APPLIED`. Ficam de fora comentários espaciais, 3D/360, catálogo, temporal, notas e grid: dependem do tick de 1,5s. Se a latência de um tipo novo importa, adicione o evento aqui.

## Por que HTTP e não WebSocket

`wsClient.sendOperation` / `sendOperations` existem (`src/js/store/sync/ws-client.js:161,170`) mas **não têm nenhum call site em `src/`**, só em `tests/integration/ws-client.test.js:148`. Toda op de entidade sai por `POST /api/v1/atlas/:id/sync`. O canal WS serve inbound de ops, presença, cursor, seleção e `sync_request` ([[canal-collab-websocket]]).

A consequência de projeto: como o broadcast do servidor não consegue excluir o autor num push HTTP, o autor recebe o próprio eco e o filtra por `clientId` (`src/js/store/sync/ws-client.js:392`, [[client-id-estavel]]). Isso, por sua vez, obriga `recordPushAcks` a semear a versão aplicada do próprio autor (`src/js/store/sync/sync-engine.js:76-78`): sem essa semente ele nunca saberia a ordem de chegada da própria op e uma op concorrente mais antiga de um peer poderia sobrescrevê-la ([[idempotencia-e-convergence-guard]]). Trade-off completo em [[sintese-rest-vs-websocket]] e [[sintese-rest-vs-sync]].
...
O guia *08-offline-import* (absorvido) §2.1 desenha pull REST → merge → push → reconectar WS. **O código faz o inverso:** reconecta o socket primeiro com backoff exponencial (`src/js/store/sync/ws-client.js:462-476`), só então pede o pull via `sync_request(lastVersion)` dentro do frame `connected` (`src/js/store/sync/ws-client.js:416-427`), e só libera o push depois da transição para `ONLINE` (`src/js/store/sync/sync-flush.js:65`). O pull de recuperação acontece **pelo WebSocket**, não por um GET REST.
