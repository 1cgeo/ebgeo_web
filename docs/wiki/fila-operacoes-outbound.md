...
**O gate de flush pergunta pelo SERVIDOR, não pelo socket, desde 2026-09-25.** `connectionState.canReachServer()` (`frontend/src/js/store/sync/connection-state.js`) aceita `ONLINE` e `HTTP_ONLY`, o atlas aberto sem tempo real ([[canal-collab-websocket]] §"Sem o canal"). Até aquela data o gate era `isOnline()`, e um proxy que recusasse o Upgrade deixava a fila parada para sempre com o HTTP funcionando, embora o push nunca tenha passado pelo socket. `RECONNECTING` continua **não** liberando push: nele nem o socket nem o pull respondem.
