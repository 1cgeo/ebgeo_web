...
- **Formato da chave da fila.** `op_{timestamp}_{id}` (`_buildKey`, `frontend/src/js/store/sync/operation-queue.js`) é ordenado por sort lexicográfico (`_getOrderedKeys`). Funciona porque epoch em ms tem 13 dígitos e terá até 2286. Mudar o formato quebra a ordem cronológica **em silêncio**, sem erro e sem teste vermelho óbvio. A chave é também a única fonte do id ao remover: o dequeue resolve `id` a partir dela, do DISCO, e nunca de um índice em memória (que ficava preso ao que ESTA aba tinha visto, e por isso não removia o que a outra havia enfileirado).
...
**Ack com `rejected: true` ainda é dequeueado, e isso é o desenho.** A recusa por operação é permanente (mapa bloqueado, tipo desconhecido, violação de dado, op de outro atlas), então reenviar é garantir o mesmo não para sempre; o usuário é avisado pelo `reason` do ack. O que MUDOU em 2026-08-15: o dequeue deixou de ser "os ids que eu enviei" e passou a ser "os ids sobre os quais o servidor se pronunciou" (`acknowledgedOperationIds`, `frontend/src/js/store/sync/sync-engine.js`). Op que o servidor não mencionou fica na fila, porque ela não foi aplicada, não foi recusada e não tem versão. Resposta que não identifica op nenhuma é o contrato antigo (2xx pelo lote) e ainda desenfileira tudo.
...
**Perda silenciosa.** `purgeOldOperations` (7 dias, `frontend/src/js/store/sync/operation-queue.js`) remove ops antigas sem log por op e sem evento de erro. Ela alcança só a fila do atlas MONTADO, e isso é consequência do banco por atlas, não escolha: o processo não abre as outras, e enumerá-las exigiria perguntar ao navegador quais bancos existem. Op velha de atlas desmontado espera a próxima montagem daquele atlas ou morre junto com o banco dele. Já `operationQueue.clear()` deixou de ser passo automático do wipe: esvaziar a fila é decisão do chamador (`clearQueue` em `clearAllDataStore`, `frontend/src/js/store/store.js`), e quem monta um atlas remoto em seguida NÃO a esvazia. Quando a fila se perde, a recuperação é snapshot ([[snapshot-e-pull-incremental]]), não replay. Ver [[atlas-modelo-de-dados]].
...
Três consequências: a compaction **não** salva lote de create (agrupa por escopo mais `entityType` mais `entityId`, e N feições novas são N entidades distintas); fan-out alto multiplica a chance de o gesto encontrar uma recusa por operação; e a atomicidade é só local, pois o `runTransaction` garante um persist no IndexedDB, mas os pares recebem ops independentes, possivelmente entremeadas com edições de terceiros.
