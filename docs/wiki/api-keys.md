...
- **A chave na query string vaza fora do alcance do backend.** O backend redige `api_key` da URL nos próprios logs (`SENSITIVE_QUERY_KEYS`, `backend/src/utils/redact-url.js`) e elide os valores do texto de SQL por outro mecanismo (`elidirSql`, desde 2026-08-31, porque até ali a chave saía inteira pelo log de query do banco, que a redação de URL nunca alcançou), mas nada disso alcança nginx, CDN, histórico de shell ou `Referer`. Prefira sempre o header; a query existe só para clientes que não conseguem setar headers. Ver [[hardening-borda-api]].
- **A redação delimita menos do que o "nunca em texto puro" sugere.** Ela cobre credencial que viaja na **URL da requisição**, reescrevendo `req.url` antes do log, e nada mais; o texto de SQL tem mecanismo próprio (`elidirSql`) e os dois não se cobrem. Campo de log estruturado fora desses dois passa inteiro, e o sistema usa isso de propósito: fora de produção o mailer grava o link de verificação, com o token dentro, num campo do próprio log ([[autenticacao-jwt]]).
