...
- `logAtlasSetting` (`src/js/store/sync/operation-dispatcher.js:346-363`) usa o UUID do atlas quando resolve, e o sentinela literal `'atlas'` quando não. Funciona porque o backend escopa pelo atlas da **rota** e ignora o `entityId`. Um `entityId` que não seja UUID nem `'atlas'` é descartado antes do flush (`src/js/store/sync/operation-dispatcher.js:120`), porque **uma única op inválida faz o Postgres estourar `22P02` e derruba o lote inteiro**, travando todo o sync.
...
A compensação é um **frame de notificação**, não uma op. O cliente colapsa `atlas_updated`, `map_duplicated` e `maps_merged` num único sinal `serverResync` (`src/js/store/sync/ws-client.js:352-358`), que dispara `syncEngine.resync()`, ou seja, um `pullSync(atlasId, 0)` forçado, **snapshot completo** ([[snapshot-e-pull-incremental]]). O comentário no `src/js/store/sync/ws-client.js` registra que antes esses frames caíam no `default` e a mudança sumia sem erro.
...
`uploadImageBlob` é best-effort e retorna `null` em qualquer falha (`src/js/store/sync/image-sync.js:44-51`), e o chamador cai para um id local. O resultado não é um erro visível: a feição fica com **uma imagem que nenhum peer consegue ver**. O clone de atlas herda o modelo, copia referências e não duplica arquivos.
...
- **`EntityType.ATLAS` existe e é código morto.** Declarado em `src/js/store/sync/operation-types.js:9` e não referenciado em lugar nenhum do frontend. Atlas não é entidade de sync. Não "reative" a constante achando que existe um caminho pronto.
...
2. Se virou op, ela existe em `EntityType` (frontend) **e** no dispatch de `applyOperation` (backend)? Um tipo desconhecido cai no `default` do `src/js/store/sync/remote-operation-handler.js:341` com apenas um `console.warn`, e a op se perde sem erro.
3. Se foi para REST e altera dados que o peer renderiza, você emitiu broadcast e o `src/js/store/sync/ws-client.js` mapeia esse `type`?
