...
- **`owner` nunca vem de `atlas_shares`.** É sintetizado de `atlas.owner_id`, e o CHECK da tabela não aceita `owner` como nível concedível (`backend/src/database/migrations/002_atlas.sql`; o enum do Joi espelha isso em `backend/src/modules/sharing/sharing.schemas.js`). A posse só muda pela rota de transferência. **Nunca ancore contrato de schema em comentário de código**: a versão anterior desta linha citava um comentário de rota que a própria wiki registra como mentiroso.
