...
O filtro por `targetId` nasceu junto com o alargamento, e é o que paga a migração: sem ele "tudo que já foi feito com este recurso" continuaria não sendo respondível, apesar do índice `(target_type, target_id)`. Desde 2026-09-01 existe o índice que de fato serve essa pergunta, com o alvo na frente e a data em ordem decrescente. O que havia antes não era varredura sequencial, e o diagnóstico correto é pior que esse: o planejador percorria o índice de ponta a ponta aplicando a condição na SEGUNDA coluna, ou seja, um índice que se chamava cobertura e era lido inteiro, que é a forma de desligar a suspeita de quem olha o plano. O filtro por **período** passou a existir em 2026-08-21 (`from` e `to`), e ele é **meio-aberto**: `created_at >= from` e `created_at < to`, para que a linha nascida na virada do dia não caia nos dois lados. `total` e as linhas vêm de duas queries em `Promise.all` sem transação (`backend/src/modules/audit/audit.service.js`): sob escrita concorrente podem discordar por uma linha, irrelevante para tela de admin, relevante se alguém usar isso para reconciliação exata.

## A impressão digital vale para QUALQUER emissor, não só para o de-para

O regime de impressão é fácil de ler como propriedade do motor de `audit-diff.js`, e não é: ele é a doutrina da casa para qualquer coisa que escreva `details`. Desde 2026-09-01 há um segundo consumidor de `impressaoDeValor` FORA daquele motor, num `details` escrito à mão: a ação de compartilhamento, que gravava o link público do atlas por extenso. Aquele link são 128 bits e a rota que o consome não tem autenticação, então num registro append-only ele era uma credencial permanente, exercível sem identidade, legível em qualquer dump ou réplica, sobrevivendo à revogação no produto e à perda da conta de quem a leu. A doutrina não tinha sido violada: ela não ALCANÇAVA aquele caminho, porque a ação nunca passa pelo motor de regimes. Quem escrever `details` à mão daqui em diante herda a mesma regra, e a pergunta que a dispara é uma só: este valor, sozinho, dá acesso a alguma coisa?
