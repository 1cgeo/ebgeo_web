...
- **Zona NÃO se aplica ao catálogo 3D**, e desde 2026-08-19 nem existe um catálogo 3D neste schema a que ela pudesse se aplicar: o segundo catálogo saiu inteiro ([[catalogo-3d]]). O que decide quem vê um modelo 3D é o eixo de [[acesso-a-recurso-privado]], que não conhece geometria. Conceder uma zona que cobre o `lon/lat` de um modelo **não** libera o modelo, e nunca liberou. Ver também [[assets3d-distribuicao]].
- **O catálogo 3D deste schema saiu em 2026-08-19**, e com ele a duplicação verbatim do predicado de acesso entre a consulta de listagem e a de contagem, que era a armadilha mais cara daqui. Ver [[catalogo-3d]].
...
- **`user_id` NÃO tem FK** (tabela `ng.zone_permissions`, `backend/src/database/migrations/006_ng.sql`): UUID de usuário inexistente entra em silêncio com 200 e fica pendurado. Valide o usuário na UI, o banco não valida. Fonte de UUIDs válidos: [[gestao-usuarios]].
...
- **Grupos e membresia** (`ng.groups` mais `ng.user_groups`, `backend/src/database/migrations/006_ng.sql`): o ramo por grupo funciona quando as linhas existem, mas não há CRUD. Grupo aqui é entidade do schema `ng`, independente de [[organizacoes-om]] e dos papéis de atlas de [[permissoes-atlas]].
