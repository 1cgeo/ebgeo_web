...
- *alvo desconhecido* — `entityType` que o `applyOperation` não sabe aplicar (`unknownTargetDenialReason`). É a única das três que é recusada **antes** do INSERT: uma op que ninguém consegue aplicar não pode consumir `server_version` nem ser replicada. Até 2026-07-25 ela era gravada aqui e acked como sucesso, o que fazia esta tabela guardar a única cópia de um dado que o cliente já tinha descartado.
