...
- **Só `false` tira o globo.** `globeProjection` tem dois estados e o padrão é globo: ausência, `null` ou lixo de um `settings` antigo resolvem para globo (`resolveGlobeProjection`, `frontend/src/js/store/atlas-appearance.service.js`, a mesma função que acende a barra "Globo / Plano" do modal). Houve um terceiro estado, "padrão do sistema", herdando a config de deploy do painel do administrador; foi cortado em 2026-08-16 por decisão do dono, e a config de deploy deixou de decidir a projeção. **A chave de deploy sobreviveu ao corte por um mês, e isso é a armadilha**: a caixa "Projeção globo" da aba Sistema continuou gravando `map2d.globe_projection`, o servidor continuou validando, fundindo e servindo a chave em `GET /api/config`, e nada no cliente a lia. O dono desmarcou a caixa, o mapa continuou globo, e ele a leu como invertida. Em 2026-09-22 a caixa e a chave foram podadas de ponta a ponta, e não reanimadas: o servidor recusa a chave no corpo e a apaga da linha já gravada, na leitura e na escrita (ver [[config-dinamico]]).
