...
**Não mande deltas.** GET, mute o objeto local, mande o bloco inteiro, que é o que o modal faz (`frontend/src/js/modals/atlas-settings.modal.js:347-350`). Uma correção "óbvia" no backend (trocar `||` por merge profundo) muda o contrato de quem já manda bloco completo apenas na aparência, mas passa a impedir a remoção de chave: avalie antes de mexer.
...
- **Lista vazia significa "sem restrição"**, nunca "nada permitido". Inverter isso trancaria todo atlas existente, cujo default é `[]`. Por isso o modal colapsa "seleção total" de volta para `[]` antes de salvar (`frontend/src/js/modals/atlas-settings.modal.js:333-345`): gravar a lista cheia congelaria um snapshot do catálogo daquele dia, e itens novos do deploy nunca apareceriam.
