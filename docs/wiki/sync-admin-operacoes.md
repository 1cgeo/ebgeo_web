...
**2. A ordem das rotas é load-bearing.** `/admin/stats` e `/admin/cleanup` precisam vir antes de `GET /:version` (comentário explícito em `backend/src/modules/sync/sync.routes.js:12`). Se `/:version` capturasse `admin`, o controller faria `parseInt('admin', 10) || 0` (`backend/src/modules/sync/sync.controller.js:44`), ou seja `sinceVersion = 0`, e um GET malformado devolveria o snapshot inteiro do atlas em vez de 404. Mover essas linhas para baixo abre um vazamento silencioso.

**3. `keepFromVersion: 0` é silenciosamente ignorado.** O controller faz `keepFromVersion ? parseInt(...) : undefined` (`backend/src/modules/sync/sync.controller.js:65`). Zero é falsy, então o caminho por dias assume com o `keepDays` que o Joi sempre preencheu por `.default(7)`. Inofensivo na prática (o service trataria 0 como no-op em `backend/src/modules/sync/sync.service.js:843`), mas quem depurar "por que meu corte 0 virou 7 dias" perde tempo aqui.

**4. `keepFromVersion` tem precedência e não tem clamp.** Enviar os dois campos faz `keepDays` ser ignorado (`backend/src/modules/sync/sync.service.js:820-822`), e nada impede um valor acima da `currentVersion`: o log é esvaziado e `min_version` fica **acima** da versão corrente, condenando todo cliente a snapshot em cada pull até que novas operações elevem `current_version`. Consulte `/admin/stats` antes e escolha entre `oldestOperationVersion` e `currentVersion`.

## Contratos de fronteira

- O delete é `server_version < $2` e o `min_version` recebe exatamente esse mesmo valor (`backend/src/modules/sync/sync.queries.js:111`, `backend/src/modules/sync/sync.queries.js:117`). Logo `min_version` é a versão da operação mais antiga **sobrevivente**, não a última apagada.
- Um cliente exatamente em `sinceVersion === minVersion` **ainda recebe incremental**, porque o teste é `sinceVersion < minVersion` (`backend/src/modules/sync/sync.service.js:781`). Trocar por `<=` transformaria o caso de borda mais comum (cliente em dia com o corte) em snapshot.
- Caminho por dias é auto-limitado: se nenhuma operação for mais nova que o cutoff, retorna zerado **sem tocar em `min_version`** (`backend/src/modules/sync/sync.service.js:835-838`). É isso que impede um atlas parado há meses de ser zerado por engano; o caminho por versão não tem essa rede.
- `GET_ATLAS_SYNC_INFO` filtra `deleted_at IS NULL` (`backend/src/modules/sync/sync.queries.js:28`), então atlas soft-deletado não tem stats e o controller converte o `null` em 404 (`backend/src/modules/sync/sync.controller.js:55-57`).
