...
- **`config.minzoom`/`config.maxzoom` de basemap são a faixa da CÂMERA, e são o único nível de zoom configurável do produto** (decisão do dono, 2026-08-31). A aplicação é fixa em `[2, 21]` (`MAP2D_BASE`) e o atlas não tem zoom nenhum. O schema de escrita do mapa base recusa valor fora de `[2, 21]` e `minzoom > maxzoom`; as duas são opcionais, e a omissão vale a faixa inteira. Quem edita é o administrador ou o produtor da OM dona da linha, pelo gate que já existia (`fn_can_produce_resource`), o que na prática deixa as cinco linhas semeadas (institucionais, `owner_org_id` NULL) só para o administrador. O cliente aplica em `frontend/src/js/baselayers/basemap-zoom.js`, e a ORDEM de escrita importa: `setMinZoom` levanta se o valor passa do `maxZoom` corrente e `setMaxZoom` levanta se fica abaixo do `minZoom` corrente, então a faixa se escreve em três chamadas. Não confunda com o `minzoom`/`maxzoom` de `data_layers`, que é da FONTE vetorial (a partir de que zoom o tile existe) e continua sem régua.
...
**Para publicar de verdade**, o destino é `/api/v1/assets3d` ([[assets3d-distribuicao]]), e o alimentador depende do armazém: `scripts/assets3d-import.js` para o SQLite plano, e a importação de modelo para o armazém de um `.3dtiles` por modelo, que é por onde sai o acervo fotogramétrico convertido ([[acervo-3d-convertido]]). É esse o mecanismo desenhado para binário pesado, e ele mantém os arquivos fora do git **e** fora do `dist/`, ao contrário de `frontend/public/3d/`, que o Vite copia para o build a cada publicação (a origem do custo que motivou a retirada). Atenção ao ligar os dois: `config.tilesets` é servido verbatim e o cliente não aplica `assets3dBaseUrl` ([[resources-catalogo]]), então a `url` gravada no catálogo precisa ser o caminho final já resolvido, e não um relativo à espera de prefixo.
...
**A miniatura tem DUAS formas vivas, e só uma é embutida.** O acervo 3D adotado grava a miniatura como ARQUIVO do mesmo servidor (`dev/import-catalogo-3d-legado.mjs` escreve `/api/v1/assets3d/<id>.webp`, fora da pasta do modelo), e o índice de regime daquela rota indexa `previewThumbnail` e `thumbnail` como campo de ARQUIVO da linha (`CAMPOS_DE_ARQUIVO`, `backend/src/modules/nomes/assets3d-regime.js`): linha privada, miniatura privada, com o mesmo gate do modelo. A consequência que custou um relato do dono em 2026-09-22 é de TRANSPORTE: a miniatura é um `<img src>`, que o navegador busca sem cabeçalho, então para quem só alcança o item pelo EMPRÉSTIMO do atlas em foco a única autorização que atravessa é o `?atlasId=`. Enquanto o cartão, o popup do marcador 3D e o do marcador 360 punham o endereço cru no `src`, o modelo emprestado abria (o `tileset.json` sai carimbado por `descritorDeAsset`) e a miniatura caía no desenho padrão, sem erro. Hoje as três passam por uma receita só: `enderecoDaMiniatura` (`frontend/src/js/catalog/endereco-da-miniatura.js`) no cartão e no popup 3D, e `sv360ReadUrl` no popup 360. A forma embutida é a armadilha do conserto: uma query escrita num data URL entra nos bytes, e é por isso que `escoparUrlDeAsset` devolve intacto todo endereço com esquema próprio (`data:`, `blob:`). Repro dos dois lados: `frontend/tests/unit/miniatura-emprestada-carimba-escopo.repro.test.js` e `backend/tests/integration/miniatura-3d-emprestada.repro.test.js`.
...
- [[assets3d-distribuicao]], [[acervo-3d-convertido]], [[streetview-360]]: consumidores de `tileset` e 360, e o registro de produção que fica ao lado do catálogo sem repetir os eixos de acesso.
