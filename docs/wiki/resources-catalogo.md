...
O DDL (`backend/src/database/migrations/003_sync.sql`) rejeita explicitamente a tabela única com coluna `category`, em favor de cinco tabelas de forma idêntica (`LIKE basemaps INCLUDING ALL`) para permitir evolução independente por tipo. O router é fábrica por tabela (`makeCatalogRouter`).

**Armadilha de segurança:** o nome da tabela é **interpolado em SQL** (o driver `pg` não faz bind de identificador), e `assertTable()` (`backend/src/modules/catalog/catalog.tables.js`) é a única barreira. Hoje o `table` vem fixo dos cinco mounts em `backend/src/app.js`. Passar um `table` derivado de request é injeção direta. Ver [[hardening-borda-api]].
