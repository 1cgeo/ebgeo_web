...
Estrutura dos grupos de chunk e a armadilha dos nomes gerados ficam em `.claude/rules/paginas-e-chunks.md`; publicação em [[deploy-web]]. O custo de EXECUÇÃO do mapa (o quadro, o worker, o terreno), que é outra conta, está em [[desempenho-do-mapa-2d]]. Aqui fica só o que a leitura do `vite.config.js` não conta.
