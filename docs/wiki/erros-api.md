...
2. **Vaza mensagem crua em produção.** `message: err.message` sem gate de `isDev` (`backend/src/modules/collab/collab.handlers.js:157`, `:204`, `:281`), enquanto o REST substitui por texto genérico fora de dev (`backend/src/middleware/error-handler.js:98-100,114`). A política de não vazar texto de driver e nome de constraint vale só metade do sistema. Ver [[hardening-borda-api]] e [[canal-collab-websocket]].
