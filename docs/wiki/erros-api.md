...
- `refresh()` compartilha uma promessa em voo (`frontend/src/js/store/sync/api-client.js:289-311`), senão uma rajada de requisições vira uma rajada de POSTs `/auth/refresh`, que estão sob rate limit compartilhado (ver [[refresh-token-rotacao]]).
