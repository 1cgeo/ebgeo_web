...
- **`layers.locked`, `groups.locked` e `properties.bloqueado` da feição são apenas colunas/propriedades sincronizadas.** O servidor as persiste (`backend/src/modules/sync/sync.service.js:1463-1472`, `:1446-1455`) e nunca as consulta como gate. O respeito é 100% do cliente: os controles de desenho testam `!feature.properties?.bloqueado` para decidir se aceitam edição (por exemplo `frontend/src/js/draw_tools/point_tool/add_point_control.js:416`, `polygon_tool/add_polygon_control.js:190`), e `frontend/src/js/store/layer.operations.js:98,155,201,245` bloqueia localmente por mapa travado. Um cliente modificado, ou um cliente com bug, escreve por cima de camada/feição "bloqueada" e o servidor aceita. Trate lock fino como **convenção de UI**, não como garantia. Ver [[modelo-conflito-lww]] e [[sintese-nao-e-crdt]].
