...
- Cursor, seleção e presença temporal de um usuário que está em outro mapa chegam a todo mundo. O **frontend** é quem filtra: `src/js/presence/remote-cursors.layer.js:119-129` só renderiza cursores do `mapId` ativo (`presenceStore.getCursors(mapId)`), e um `mapId` ausente resulta em zero cursores.
...
Do lado do cliente, `src/js/store/sync/sync-engine.js:262-289` faz `peek` de um lote, e só chama `operationQueue.dequeue(opIds)` **depois** de um push bem-sucedido; um lote rejeitado não é removido e o próximo flush re-peeka os mesmos ops. Isso é correto para erro transitório e é um **poison batch** para erro permanente: se um op da fila mira um mapa que foi travado por outro usuário, a fila outbound para de avançar até o mapa ser destravado ou a fila ser limpa. Ver [[fila-operacoes-outbound]] e [[fila-operacoes-outbound]].
...
- Código verificado: `backend/src/modules/collab/collab.rooms.js`, `backend/src/modules/collab/collab.gateway.js`, `backend/src/modules/collab/collab.handlers.js`, `backend/src/modules/sync/sync.service.js`, `backend/src/utils/errors.js`, `backend/src/config.js`, `src/js/store/sync/ws-client.js`, `src/js/store/sync/sync-engine.js`, `src/js/store/sync/api-client.js`, `src/js/index.js`, `src/js/presence/remote-cursors.layer.js`, `src/js/store/layer.operations.js`, `src/js/draw_tools/*/add_*_control.js`.
