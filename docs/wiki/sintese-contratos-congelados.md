...
**Dois contratos 3D vivos ao mesmo tempo.** A descoberta documentada é `GET /nomes/catalogo3d` mais `assets3dBaseUrl + m.url`; nenhum arquivo de `frontend/src/` chama essa rota ou lê `assets3dBaseUrl`, embora o backend publique o campo (`backend/src/modules/config/config.service.js`). O ebgeo_web descobre modelos por `config.tilesets`, populado pelo `/api/config` e lido em `frontend/src/js/3d_models_viewer_tool/map_3d.js`, com shape próprio (`locate: { lon, lat, height }`, não o `lon`/`lat`/`heightoffset` planos do catálogo). São dois contratos 3D vivos, não um: quem for unificá-los precisa escolher qual sobrevive antes de escrever a primeira linha.
...
**Armadilha, `zoom` nunca é enviado.** O guia 13 recomenda mandar `zoom` para calibrar o decaimento por distância. Nenhum call site manda: os dois montam a query só com `q`, `lat`, `lon` (`frontend/src/js/search/search-bar.search-providers.js`, `frontend/src/js/search/feature-search.control.js`). O backend cai no raio padrão de 50 km e desliga o ajuste por tipo, então em zoom alto a busca não prioriza o que está perto. Não é bug de contrato, é qualidade deixada na mesa.
