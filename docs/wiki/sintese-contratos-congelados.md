...
> [!CONTRADICAO 2026-07-18 — RESOLVIDO 2026-07-24] Três rotas admin do 360 passam pelo cliente genérico (`frontend/src/js/store/sync/api-client.js:612`, `:622`, `:631`), e o parser de erro lia `parsed.error?.message`: com o envelope **plano** do sv360, `parsed.error` é string e `.message` é `undefined`, então `frontend/src/js/admin/catalog-tab.js:466`, `:534` e `:549` exibiam "HTTP 404" no lugar de "Project not found". O parser passou a aceitar os **dois** envelopes (`api-client.js:309-317`), com dois casos em `frontend/tests/integration/api-client.test.js` — um para o envelope plano e um afirmando que o fallback `HTTP <status>` sobrevive para corpo sem erro utilizável. O `code` segue `undefined` no envelope plano, porque o sv360 não emite código.
