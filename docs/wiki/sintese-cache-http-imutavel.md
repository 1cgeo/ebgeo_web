...
O `parseRange` foi **copiado verbatim** entre os módulos (o cabeçalho de `backend/src/modules/streetview360/sv360.controller.js` admite isso). Consequência operacional: um bug de protocolo em uma rota provavelmente existe nas três, e a correção precisa ser aplicada três vezes.
...
- **Multi-range (`bytes=0-100,200-300`) vira 416, não 200.** O regex de `parseRange` aceita um único intervalo, então qualquer cliente ou proxy que tente multipart byteranges quebra. `end` além do tamanho é clampado, não é erro; só `start` fora dos limites gera 416.
...
- **Na imagem 360 o tamanho autoritativo é `buf.length`, não o `size_bytes` do Postgres** (`backend/src/modules/streetview360/sv360.controller.js`). O motivo é que o BLOB vive no `{slug}.db` e o tamanho no Postgres, e na janela swap/commit da ingestão os dois divergem. Isso garante `Content-Length == corpo` sempre, mas **o custo aceito não está no código**: nessa janela o ETag (Postgres) pode não corresponder aos bytes (SQLite), e o cliente cacheia um ETag antigo para conteúdo novo.

## O eixo que a doc esquece: public vs private

Nenhuma das duas rotas de BLOB emite um escopo fixo. O sv360 decide por projeto, emitindo `private` mais `Vary: Authorization, Cookie` quando o projeto está `disabled` (`setImmutableHeaders`, `backend/src/modules/streetview360/sv360.controller.js`), e o `assets3d` decide por requisição, a partir da linha de catálogo dona do caminho (`backend/src/modules/nomes/assets3d.controller.js`): esta página dizia "sempre `public`" e isso valeu só até o regime privado existir. A razão é a mesma nos dois: um cache compartilhado só pode guardar resposta que qualquer chamador poderia ver, e o `Vary` é cinto e suspensório para proxies que ignoram `private`. Quem enxerga o quê: [[acesso-a-recurso-privado]], [[auth-flexivel]], [[organizacoes-om]]; postura de borda em [[hardening-borda-api]].
...
- **`assets3d` nunca responde 401 nem 403 por falta de token, e isso deixou de significar "rota aberta".** Ela é montada antes das autenticadas (`assets3dRoutes`, `backend/src/app.js`) e o caminho de modelo público continua servido sem credencial nenhuma; o de modelo privado passa por `gateDeAsset3d` e responde **404** a quem não o alcança, porque modelo escondido tem de ser indistinguível de inexistente ([[assets3d-distribuicao]]). Esta linha dizia que a proteção real era a descoberta ("só o catálogo autenticado revela os caminhos"), e o argumento caiu por dois lados: a URL viaja no payload de recursos visíveis e quem a recebe pode repassá-la, e o catálogo que a revelava saiu em 2026-08-19. Modelo público continua "não secreto"; não estenda essa conclusão ao privado.
- **Traversal responde 404, não 403.** `path.posix.normalize` colapsa os `..` contra a raiz antes da checagem de prefixo (`backend/src/modules/nomes/assets3d.service.js`), então `/assets3d/../../etc/passwd` vira caminho inexistente dentro da raiz. O `ForbiddenError` é quase inalcançável pela via HTTP: **não use 403 como sinal de ataque**.
...
- **Tiles MVT** usam `max-age=60` com ETag FRACO do corpo e sem Range, e o **escopo segue o chamador**: `private` + `Vary` quando há sessão, `public` só no anônimo (`mvtTile`, `backend/src/modules/streetview360/sv360.controller.js`). Os 60 s são deliberados (o tile muda a cada ingestão, tombstone ou toggle, e `immutable` congelaria pontos apagados por um ano); o escopo variável também, porque o corpo do tile inclui projeto `disabled` para quem pode vê-lo. Tile vazio é 200 com Buffer vazio, nunca 404 ([[streetview-360]]). **Os 60 s têm um efeito no CLIENTE que o servidor não vê:** o MapLibre atualiza o tile pela rede quando a validade vence, e um tile VETORIAL respondido 404 nessa atualização (o atlas do `?atlasId=` deixou de ser legível) é carregado vazio sem renovar a validade, e era pedido de novo a cada ida e volta, para sempre (medido em 2026-09-21 no stack de teste: ~530 pedidos por segundo sobre 26 tiles). Quem para o laço é `installTileExpiryGuard` (`frontend/src/js/map/tile-expiry-guard.js`), com o mecanismo no `fileoverview`; encurtar ou alongar o `max-age` não o resolve, só muda quando ele começa.
- **Imagens de atlas** ([[imagens-atlas]]) delegam ETag, 304, Range e `Last-Modified` ao `res.sendFile` (`backend/src/modules/images/images.controller.js`). Divergência que atravessa arquivos: elas emitem `Last-Modified` e ETag fraco do Express, que as rotas artesanais **não** emitem, então validador condicional que funciona lá falha aqui.

## Resolução de URL

O `previewThumbnail` do 360 é relativo e resolve contra o `streetView360.serviceUrl` do `/api/config`, para manter o dado portável entre ambientes ([[config-runtime-urls-relativas]], [[assets3d-distribuicao]]). O catálogo 3D que sobreviveu não segue essa forma: ele guarda a URL pronta na própria linha, e quem decide o prefixo é o catálogo, não uma junção em tempo de execução.

> **Nota histórica.** O guia *14-catalogo3d-assets* (absorvido) afirma que o cliente concatena `assets3dBaseUrl` com `m.url`. No cliente as-built não existe nenhuma referência a `assets3dBaseUrl`: `frontend/src/js/3d_models_viewer_tool/map_3d.js` passa `tilesetConfig.url` direto ao `Cesium3DTileset.fromUrl`, já completo. A concatenação é contrato oferecido pelo backend, não caminho exercido hoje. O lado 360 segue o contrato (`getServiceUrl`, `frontend/src/js/street_view_tool/streetview-api.service.js`).
