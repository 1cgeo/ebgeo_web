...
- **Abrir atlas remoto**: `claimRemoteAtlas → disconnect → activateRemoteAtlas → resetAtlasView → markStoreRemote → connect → activateAtlasInitialMap → startAutoFlush` (`frontend/src/js/account/open-atlas.service.js`, o único pipeline de abertura). `markStoreRemote` **antes** do connect é intenção durável: se a aba morrer durante o pull, a guarda de boot vê `remote` e descarta o parcial em vez de promovê-lo a atlas local permanente. Desde 2026-09-19 a abertura normal **não apaga nada**: `resetAtlasView` (`frontend/src/js/store/store.js`) zera apresentação e caches em memória, e o IndexedDB, a fila e os bytes de imagem continuam ali como rede de segurança para o caso de o pull falhar. O wipe sobreviveu num ramo só, o descarte confirmado do slot RESGATADO, e é por causa dele que `activateRemoteAtlas` continua vindo antes: aquele wipe esvazia o escopo ATIVO, e com um namespace por atlas o escopo anterior pode ser o dado vivo de outra aba ([[namespace-por-atlas]]). O atlas local COMUM não é perguntado nada e não perde nada.
