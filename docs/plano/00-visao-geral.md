...
| D3 | **360: separado ou absorvido?** | ~~Separado (recomendado)~~ **↺ REVERTIDA na Fase 9: ABSORVIDO** (decisão de produto). Serviço + metadados viraram o módulo `sv360`; BLOBs ficaram em SQLite (a "paridade de performance" não foi bloqueio — reusa o padrão worker-pool/ETag O(1)/semáforo dos assets 3D). | Implementado nas stages 1–3b (708 testes). |
...
| 7 ⛔ | **Gateway + 360 — SUPERSEDIDA pela Fase 9** (o 360 foi absorvido, não gatewayed). Permanece válido: o JWT emissor único + aliases org/login + padrões 360 carregados. NGINX agora é de um upstream só | fase-5 | Médio | [fase-7-gateway-360.md](fase-7-gateway-360.md) |
...
| 9 ✅ | **Absorver o 360 (IMPLEMENTADA)** — REVERTE a D3: serviço + metadados do `ebgeo_360` no backend (`sv360`, PostGIS); BLOBs em SQLite (`{slug}.db`, worker pool). Stages 1–3b: leitura/imagem, escrita/calibração, ingestão/ETL, tiles/thumbnails — remediadas + verificadas adversarialmente (708 testes). | fase-3, fase-5, fase-0 | Alto | [fase-9-absorver-360.md](fase-9-absorver-360.md) |
