...
| F22 | S3 | O inventário de dependências cobre seis vendors e deixa fora `frontend/src/vendor/three/` inteiro (snapshot de desenvolvimento) e um segundo WASM do GDAL; milsymbol declara 3.0.3 no banner e 3.0.2 no objeto; a imagem Docker não tem digest. | [inventário](../seguranca/dependencias-lancamento-inventario.json). |
