...
| B10 | segurança e dependências | inventário reproduzível por script com escopo ampliado; autorização revisada (A1..A15); V1..V8 respondidos e executados em 14/09/2026 (D9); **resta** o refazer do inventário sobre o SHA candidato e a reescrita do viewshed, com prazo do dono |
...
3. **Feito em 14/09/2026 (D9):** o three vem do npm em `three@0.164.0` por `frontend/src/js/vendor/three.js`; `cesium-measure.js` virou código da casa em `frontend/src/js/3d_models_viewer_tool/services/cesium-measure.js`; `cesium-viewshed.js` está declarado (154 kB ofuscados, onze campos privados de `ShadowMap`) com o aceite da reescrita em `docs/seguranca/cesium-viewshed-reescrita-aceite.md`, **prazo a fixar pelo dono**; as sete bibliotecas nativas do GDAL viram lista fechada declarada; Draco e Basis dentro do aholo-viewer ficam declarados como ponto cego, com o pedido de manifesto pronto em `docs/seguranca/aholo-viewer-pedido-de-manifesto.md`.
