...
3. **Resolver os cinco itens sem versão determinável, ou registrar decisão e responsável para cada um:** o three de `frontend/src/vendor/three/` (revisão 164dev, sem par publicado); `frontend/public/vendors/cesium/cesium-viewshed.js` (sem upstream conhecido, carregado em runtime pelo visualizador 3D; o irmão dele saiu de vendor em 14/09/2026 e hoje é `frontend/src/js/3d_models_viewer_tool/services/cesium-measure.js`); as bibliotecas nativas dentro do WebAssembly do GDAL além das três legíveis; a imagem Docker efetivamente em execução no servidor interno; e o sistema operacional dessa imagem.
