...
| B8 | uploads duráveis | fundido; a prova em duas browsers da retomada entrou em 15/09/2026 e achou dois defeitos mudos no caminho do F5, corrigidos |
| B9 | indicadores e administração | fundido; a validação com telemetria real e a instalação da sonda são do dono, na rede interna, depois da implantação |
| B10 | segurança e dependências | inventário reproduzível por script; autorização revisada (A1..A15); V1..V8 executados (D9); viewshed reescrito (D15) e tutorial na quinta entrada (D16) em 15/09/2026, costura do viewshed provada e fechada em 16/09/2026, `frontend/public/vendors/` vazia; **resta** só o refazer do inventário e do `npm audit` sobre o SHA candidato |
| B11 | homologação final e migração da main | **aberto**; os dois cenários de config dedicada verdes; Firefox entrou na matriz em 15/09/2026 com a matriz mínima verde nos dois navegadores; sete passadas do Playwright, as três últimas sem vermelho |
...
1. **Fechada em 13/09/2026: comparação de geometria no painel**, com a captura lida em 14/09/2026. Dois folhas de zero imports, `frontend/src/js/account/pendencias/comparacao-de-conflito.js` e `comparacao-phrases.js`: tipo, vértices, centro, deslocamento em metros e os campos que diferem, em texto, nomeando os dois lados e dizendo quando `serverData` não veio (as duas imagens mostraram exatamente isso: "linha; 3 vértices aqui, 2 vértices no servidor; o centro está 612 m distante. Uma propriedade difere: lineColor", e "O servidor não devolveu o conteúdo atual deste item"). **Os dois achados visuais foram fechados em 15/09/2026, e a causa de cada um não era a suposta:** os avisos nasciam ANTES de o painel abrir e a posição só era decidida na criação, então passou a ser revista enquanto o aviso vive (observador em `frontend/src/js/utilities/toast_service.js`); e o nome do mapa vinha da tabela em memória, que fica vazia logo depois de um retrato, então passou a vir do disco, com a remoção só afirmada com testemunha (`frontend/src/js/account/pendencias/pendencias-rows.js`). De passagem: o painel dizia "Nenhuma pendência" com operações na fila, porque só lia o que exige decisão; agora diz quantas estão a caminho, e o crachá e o painel leem a mesma conta (`aCaminhoDoCenso`).
...
6. **Encerrada por decisão do dono em 15/09/2026:** o espaço em disco das duas cópias da transição não é preocupação de lançamento. O caminho de poda (D8) está implementado, com a tela capturada e lida em 14/09/2026.
7. **Encerrada por decisão do dono em 15/09/2026:** o tamanho do acervo na transição não é preocupação de lançamento; a única medição (41 registros com um blob de 8 MiB em 5,68 s) fica como referência em `frontend/src/js/store/migration/legacy-transition.js`.
...
2. **Fechada em 15/09/2026: a prova em duas browsers da retomada** (`frontend/tests/e2e-ui/browser-collab-imagem-retomada.spec.js`, três casos, 5 de 5 em série), lendo o BLOB, o bitmap desenhado e a linha no Postgres. Dois defeitos de produto achados e corrigidos no caminho do F5: o wipe de entrada apagava a pendência de blob (`preserveBlobUploads`), e o retrato liberava a op antes dos bytes (`idsComBlobPendente`). Guardas em `frontend/tests/integration/wipe-poupa-blob-pendente.repro.test.js` e `retrato-nao-libera-op-sem-blob.repro.test.js`.
...
1. **Com o dono, depois da implantação (decisão de 15/09/2026):** a validação com telemetria real na rede interna. Onde mora: `frontend/src/js/account/sync-status.control.js`, `frontend/src/js/session/pendencias-monitoramento.js` e o painel de presença do administrador. Aceite: duas abas do mesmo usuário, visitante anônimo, logout, expiração e perda da telemetria produzindo contagens coerentes.
2. **Com o dono, depois da implantação (decisão de 15/09/2026):** instalar a sonda de disponibilidade em host separado da rede interna (`backend/scripts/sonda-disponibilidade.js`, script npm `diag:sonda`), com o diretório de saída alcançável pelo backend.
...
1. **Fixar o SHA candidato e refazer o inventário sobre ele** por `node scripts/inventario-de-vendors.mjs --check` (preso por `frontend/tests/unit/inventario-de-vendors.test.js`, com escopo ampliado a todo código de terceiro em `frontend/public/` desde 14/09/2026). É o único passo do fecho que continua aberto, porque depende do SHA de lançamento.
2. **Feito em 13/09/2026:** `npm audit` zero em todas as severidades nos três lockfiles; repetir sobre o SHA candidato.
3. **Feito em 14/09/2026 (D9):** o three vem do npm em `three@0.164.0` por `frontend/src/js/vendor/three.js`; `cesium-measure.js` virou código da casa em `frontend/src/js/3d_models_viewer_tool/services/cesium-measure.js`; `cesium-viewshed.js` foi REESCRITO como código da casa em 15/09/2026 (D15: `frontend/src/js/3d_models_viewer_tool/services/viewshed-3d.js`, três campos privados no lugar de onze, presos por teste que lê o fonte do Cesium; desenho comparado em pixel com 2,651% de classe diferente; a costura acima de 150 graus ganhou prova visual em 16/09/2026: lida de perto, ela tinha fresta de 3 a 4 px em todas as oito aberturas, com DUAS causas fechadas, a folga `SEAM_NARROWING_DEGREES` zerada e os pedaços medindo azimute em torno da mesma vertical local, e dois casos de emenda com referência versionada e quatro réguas em `frontend/tests/e2e-ui/viewshed-3d-pixel.spec.js`, registrado em `docs/wiki/viewshed-3d.md`); as sete bibliotecas nativas do GDAL viram lista fechada declarada; Draco e Basis dentro do aholo-viewer FECHARAM por hash em 15/09/2026 (Draco 1.5.7 byte a byte; Basis a linha de snapshot entre 1.60 e 2.0, tags v1_60_snapshot do repositório upstream), sem pedido ao fornecedor, por decisão do dono; nenhum aviso publicado afeta os dois.
4. **Feito em 14/09/2026 (D9):** digest do índice de `node:22-bookworm-slim` fixado nas duas linhas `FROM` do `backend/Dockerfile`, lido por três caminhos independentes, com construção de prova (código 0, Node 22.23.2, Debian 12). Procedimento em `docs/wiki/deploy-backend.md`.
...
6. **Feito em 14/09/2026 (D9), e a migração para o npm FECHOU na mesma noite:** os seis vendors sem consumidor e a segunda cópia do three foram podados, e Cesium (1.145.0, `frontend/src/js/vendor/cesium.js`, ativos copiados do pacote pelo plugin do Vite; só `cesium-viewshed.js` sobrevive em `frontend/public/vendors/cesium/`), GDAL (`gdal3.js` 2.8.1, `frontend/src/js/vendor/gdal.js`, wasm e data por URL com hash, sem worker e sem COOP/COEP), Turf 7.4.0 e milsymbol 3.0.4 (pontos únicos com carga sob demanda) passaram a vir do npm, e o MapLibre subiu para 6.9.1 sem mudança de código. O que sobrevive em `frontend/public/vendors/`: `docsify.min.js` e `vue.css`, porque `public/docs/doc.html` (o botão Tutorial) os consome, e o docsify 5.0.0 é major; migrar o tutorial é decisão de arquitetura do dono (quinta entrada do Vite, ou gerar em `public/`). Cada biblioteca teve captura lida (modelo 3D e viewshed; PDF georreferenciado de folha única; seta e elipse do Turf; símbolo militar; ponto e linha atravessando a troca de basemap). Achado que a migração produziu e fechou: o namespace ES do Cesium não carrega `__esModule`, e sem a cópia rasa que o ponto único publica o viewshed sumia em silêncio. O inventário acusa código de terceiro fora das pastas cobertas e fechou com 3 artefatos.
...
### A quinta passada (14/09/2026, noite): a rodada inteira depois da migração ao npm

Sobre `52931b03`, com D9 a D14, a migração ao npm e os consertos da quarta passada fundidos: **368 verdes, zero vermelhos, 1 flaky em 60 min**, e o flaky (`frontend/tests/e2e-ui/browser-briefing-write-ahead.spec.js`) remedido em série logo depois, **12 de 12 verdes** com `--retries=0` e `--repeat-each 4`. É a primeira rodada inteira sem vermelho desta fase.

Os quatro vermelhos da quarta passada, fechados antes desta: o F5 num atlas conectado reabria um "Mapa 1" porque `activeGenerationHolds` pulava o retrato sobre disco em branco (produto, 4 de 4 depois, era 2 de 8); o menu da camada morria num redesenho da aba entre o clique e a abertura (produto, 12 de 12); `logoutUI` não respondia ao diálogo de pendências (instrumento, 6 de 6); a sonda de listeners contava alvo não ancorado (instrumento, 4 de 4). Dos quatro flaky, dois eram spec (`catalog-modal` escolhia o filtro escondido; `painel-de-feicao-na-troca-viva` repetia ajudantes que não lançam), um caiu com o conserto do F5, e `three-client-flow` não reproduziu em 4 de 4, com o mecanismo (três recolors concorrentes serializando em duas aplicações) declarado e ainda aberto.
...
### A sétima passada (15/09/2026, noite): o candidato com R7, Firefox, retomada de imagem e o painel

Sobre `2b809a2f`: **376 verdes, 1 vermelho, 1 flaky em 49 min**. O vermelho (`browser-collab-shared-atlas`, o fluxo de nove passos em duas browsers) e o flaky (`context-menu-local` §14.8) são os DOIS a mesma queda do renderizador do Chromium ao bootar o mapa (`Target crashed`, código `0x80000003`, sem erro de página), o mecanismo declarado inevitável na sexta passada; isolados, o primeiro deu 2 de 2 e o segundo 3 de 4, com a única reprovação sendo a mesma queda. A taxa da queda subiu ao longo de dois dias de máquina ligada, e um agente está medindo argumentos de lançamento do Chromium no harness (renderizador por software, entre outros) para reduzi-la sem tocar no produto.

Fundidos antes desta passada, todos verdes nela: D17 (empréstimo por atlas no tile), o Firefox na matriz, a prova em duas browsers da retomada de imagem, a fase 5 do fluxo de três clientes, o painel de pendências (avisos, nome do mapa, e a contradição com o crachá), e Draco e Basis fechados por hash.

### Depois da sétima passada (16/09/2026): a costura do viewshed, lida de perto

O único item de produto que a sétima passada deixou aberto era a costura entre sub-viewsheds acima de 150 graus, medida em 15/09/2026 como "0 px de fresta" só num enquadramento largo (0,42 m por pixel). Lida de perto (0,046 m por pixel), com um muro cruzando a emenda, ela tinha fresta contínua de 3 a 4 px nas oito aberturas (151, 155, 180, 240, 300, 301, 320, 360), cortando a sombra do muro em duas; o vendor substituído, nas mesmas cenas, tinha 60 px. Duas causas, as duas fechadas em `21eff868`: a folga `SEAM_NARROWING_DEGREES` (0,1 grau, sub-pixel no enquadramento largo) foi a zero, porque o erro residual entre os pedaços já tem o sinal da sobreposição abaixo do horizonte; e a fresta que sobrou acima do horizonte (2 px por 16 a 25 px no topo do muro, achada pela leitura da referência versionada) vinha de cada pedaço medir azimute em torno do eixo "para cima" da PRÓPRIA câmera, que o Cesium inclina contra a direção de cada pedaço (0,5 a 0,6 grau entre eles), com erro proporcional à tangente da elevação; o azimute passou a ser medido em torno da vertical local, comum a todos, e o resíduo caiu a 0,0015 grau constante, sempre sobreposição. Prova: dois casos de emenda (180 e 320 graus) com referência própria na vista aproximada e quatro réguas (fresta zero na coluna inteira da emenda, do arco à borda, muro incluído; nenhuma faixa saturada; sombra do muro contínua; pixels diferentes da referência), com controles negativos executados (eixo por pedaço reabre 2 px; folga 0,1 reabre 4 px; folga 1,5 reabre 64 px; folga negativa produz faixa saturada). Lição registrada no livro-razão: a régua de pixel herda a escala do enquadramento, e "0 px" numa vista larga não prova nada sobre uma fresta de um decímetro.
...
- **Fechado em 15/09/2026: o Firefox entrou na matriz** (`test:e2e:firefox`, projeto fora da rodada normal). Matriz mínima verde nos dois navegadores (migração 2.2, atlas-safety, mutações de feição, mapas e camadas, duas abas, logout com pendências, 3D). Um defeito de produto que só o Firefox mostrava: `navigator.storage.persist()` é diálogo lá e a promessa nunca resolvia, então o mapa não montava; corrigido com prazo. Três armadilhas de instrumento (o `/@vite/client` dentro do worker derrubava o Playwright no Firefox; relógio três vezes mais lento; `clearTraces` perseguindo a op errada) consertadas. Tabela em `frontend/tests/e2e-ui/README.md`.
...
Duas revisões pedidas pelo dono (achados P1..P25 no eixo de permissão e R1..R7 no acesso a recurso). Os dois relatórios foram absorvidos, em 2026-09-15, pelas guardas que criaram, pelas decisões D13 e D14 e pela tabela de cláusulas com prova parcial em [`docs/wiki/permissoes-atlas.md`](../wiki/permissoes-atlas.md), e apagados. **R7 fechado em 15/09/2026 (D17):** o empréstimo por atlas alcança o tile pelas três pontas (o cliente carimba o atlas no pedido de tile; o gate do servidor lê o carimbo e avalia o predicado de empréstimo, com 401 em vez de 404 porque o `auth_request` do NGINX só entende 2xx, 401 e 403; a resposta emprestada sai com regime de cache privado e o censo dos dois pacotes ganhou a classificação). A cláusula 6.7 passou a VIGENTE no mesmo commit. Mais a revisão de autorização do sync, das quatro exceções REST, dos uploads, recibos e sockets (achados A1..A15, registrados na seção B10). O que ficou ABERTO, todos para decisão do dono:

- **P8, fechado em 14/09/2026 (D13):** `GET /users/search` exige três caracteres, devolve no máximo vinte linhas com `truncated`, casa só nome e login, escapa curinga e passa pelo limitador. Guardas em `frontend/tests/unit/busca-de-pessoas-piso-espelha-backend.test.js` e no backend.
- **P9 e R6, fechados em 14/09/2026 (D14):** o vídeo de prévia sai por rota gateada pelo mesmo predicado das outras mídias do recurso (404 para quem não pode), e marcar privado re-cunha o nome do arquivo. Guarda: `backend/tests/integration/video-de-previa-gateado.test.js`.
- **A13, fechado em 14/09/2026 (D10), com efeito colateral que o dono pode reverter em uma linha:** `sync_receipts` é expurgado abaixo de `min_version` no mesmo ponto do expurgo do administrador. O que a implementação descobriu: a permanência era CONTRATO (três testes a prendiam pelo nome) e o recibo tem um segundo emprego, resolver a base da edição encadeada. Consequência: um cliente de volta de ausência longa recebe conflito nomeado no lugar de "já aplicado" para as ops abaixo da fronteira, e a edição encadeada a uma delas é recusada. Nada reaplica. Guarda: `backend/tests/integration/expurgo-de-recibos-por-min-version.test.js`.
- **A14, declarado em 14/09/2026 (D11):** socket aberto sobrevive à expiração do JWT e cai na revogação de share; escrito em `docs/wiki/canal-collab-websocket.md` e preso por `backend/tests/ws/collab-expiracao-de-token-nao-derruba.test.js`.
- **A15, fechado em 14/09/2026 (D12):** cem atlas vivos por conta (`ATLAS_MAX_PER_ACCOUNT`, 429 `QUOTA_EXCEEDED`, administrador não isento, lixeira não conta) e teto de duzentos mapas por importação (`IMPORT_MAX_MAPS`, 400). Guarda: `backend/tests/integration/cota-de-atlas-por-conta.test.js`; o caminho de criar, que era mudo no cliente, passou a mostrar a frase do servidor.
