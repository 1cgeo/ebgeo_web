...
2. Não há transição de banco a aplicar, e a instrução aditiva que este item trazia foi retirada em 2026-09-13. Por D6 ([plano de correção total](../plano-correcao-total-lancamento-2026-09-13.md), bloco B2), a linha de integração nunca foi implantada: a primeira implantação do backend é instalação nova a partir das doze baselines, a baseline consolidada permanece editável até o SHA candidato, e banco de desenvolvimento anterior a uma edição de baseline se recria. O congelamento passa a valer no SHA implantado, e daí em diante toda mudança de schema entra por arquivo numerado novo, recusada em disco pelo guarda de checksum se alguém editar uma baseline já aplicada.
