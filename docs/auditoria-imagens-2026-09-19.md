...
Esses registros são preservados. O teste os identifica explicitamente como defeitos da entrada e exige uma imagem de erro disponível; não os contabiliza como desenhos recuperados. Não há bytes para reconstruir a foto, nem informação suficiente para escolher com segurança os símbolos e a medida. Os três PNG de fotos existentes no arquivo continuam sendo conferidos. Uma migração bem-sucedida não torna recuperável conteúdo que já estava ausente ou indefinido na origem.

## Verificação e limites

Medições dos 13 cenários distintos de migração, incluindo as nove entradas defeituosas do primeiro acervo: [auditoria-imagens-2026-09-19.json](auditoria-imagens-2026-09-19.json). Os cenários do primeiro acervo foram repetidos após o inventário completo dos defeitos da entrada; os testes não usam retries automáticos.
...
- Suíte frontend: **717 arquivos, 13.247 testes aprovados**.
- Migração/arquivos: **13 cenários Chromium**, consolidados por cenário no JSON, sem retries automáticos.
