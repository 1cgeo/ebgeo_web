...
class MapManager {
    ...
    async copyMap(mapName, newMapName) {
        try {
            ...
            if (isRemoteStoreSync()) {
                idMapping = await this._copyMapThroughOperations(mapName, trimmed, originalMapData, originalNotes, layerManager);
                if (!idMapping) return { success: false, message: 'Não foi possível duplicar o mapa: você não tem permissão para criar mapas neste atlas.' };
            } else {
                const layerIdMapping = await layerManager.duplicateMapLayers(mapName, trimmed);
                const regenerated = await IDUtils.regenerateMapIds(originalMapData, trimmed, layerIdMapping);
                idMapping = regenerated.idMapping;
                await addMap(trimmed, regenerated.newMapData, originalColorUsage, originalNotes);
            }
    ...
    /**
     * The copy of a map in a SERVER atlas: the map first, then every child through its own
     * operation, the order `combineSelectedMapsIntoTarget` already follows.
     *
     * The local composition does not travel. It writes the layers with a raw repository write
     * (`duplicateMapLayers`) BEFORE the map exists and without any operation, and hands the
     * features to `addMap` inside the map document, where the server's map CREATE does not look:
     * the copy reached the server with no feature and no layer of its own, the author kept features
     * pointing at layer ids that never existed, and an F5 showed an empty map
     * (`frontend/tests/e2e-ui/duplicar-mapa-no-servidor.repro.spec.js`). Here the map is created
     * empty, the layers by `createLayerForImport` (a layer operation each) and the features by
     * `addFeatures` (one batch of feature creates). The colour usage is left to `addFeatures`,
     * which counts every feature it adds; handing the original counts to `addMap` too would count
     * them twice.
     *
     ...
     * @param {Object|null} originalNotes - Its notes.
     * @param {Object} layerManager - Layer manager instance.
     * @returns {Promise<Map|null>} Old feature id -> new feature id, or null when the map creation
     *   was refused (the refusal already spoke through STORE_OPERATION_BLOCKED).
     * @private
     */
    async _copyMapThroughOperations(mapName, trimmed, originalMapData, originalNotes, layerManager) {
        const emptyFeatures = Object.fromEntries(Object.keys(originalMapData.features ?? {}).map((type) => [type, []]));
        const { newMapData: shell } = await IDUtils.regenerateMapIds({ ...originalMapData, features: emptyFeatures }, trimmed);
        const created = await addMap(trimmed, shell, null, originalNotes);
        if (!created) return null;

        const layerIdMapping = await this._mergeLayersFromMap(layerManager, mapName, trimmed);
        const { newMapData, idMapping } = await IDUtils.regenerateMapIds(originalMapData, trimmed, layerIdMapping);
        await addFeatures(newMapData.features, trimmed);
        return idMapping;
    }
