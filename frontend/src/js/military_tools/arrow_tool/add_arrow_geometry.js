...
class AddArrowGeometry extends BaseGeometry {
     ...
     * nominal length untouched and never reads `axisLength`, so a single-headed
     * arrow keeps exactly the geometry it had before the flag existed.
     *
     * It takes the axis length as a NUMBER, not the turf LineString: Turf is
     * loaded on demand in this branch (`utilities/turf-loader.js`), so keeping
     * `turf.length` at the call site, inside the ternary of the flag, is what
     * makes this function pure and node-testable without a stub, and what keeps
     * a single-headed arrow from measuring an axis nobody asked it to measure.
     *
     * @param {number} axisLength - Length of the arrow axis in meters (ignored without the flag)
     ...
     */
    resolveHeadLengths(axisLength, nominalHeadLength, doubleHeaded) {
        ...
        // reaching the division below and producing NaN coordinates.
    ...
    generateNormalArrowGeometry(mainLine, width, headLengthRatio, absHalfBodyWidth, showArrowHead, doubleHeaded = false) {
        ...
        // `turf.length` only inside the ternary of the flag: without it the axis
        // is never measured, which is what keeps the one-headed arrow byte for
        // byte the arrow it was and costs no extra turf call on the hot path.
        const { headLength, tailLength } = this.resolveHeadLengths(
            doubleHeaded ? turf.length(mainLine, { units: 'meters' }) : 0,
            nominalHeadLength,
            doubleHeaded
        );
        ...
        if (doubleHeaded) {
            ...
            // ring arrives here on the `bearing - 90` side of the first vertex,
            // which relative to `tailBearing` (the reversed first segment) is the
            // `tailBearing + 90` side, i.e. the tail's own cornerRight. So the
            // corners are emitted cornerRight, tip, cornerLeft, mirroring the
            // leading head and leaving the ring free of self-intersections.
    ...
    generateAirmobileArrowGeometry(mainLine, width, headLengthRatio, airmobilePosition, showArrowHead, doubleHeaded = false) {
        ...
        try {
            ...
            const absHeadBaseWidth = Math.abs(width * 2.5);
            const nominalHeadLength = absHeadBaseWidth * headLengthRatio;
            // `mainLineLength` is already measured above for the crossover point,
            // so unlike the normal path there is no extra `turf.length` to gate
            // behind the flag here; `resolveHeadLengths` ignores it when off.
            const { headLength, tailLength } = this.resolveHeadLengths(
                mainLineLength, nominalHeadLength, doubleHeaded
            );
    ...
    createSingleHandles(feature, branchIndex = null) {
        ...
        if (showArrowHead) {
            ...
            // the nominal length may be clamped, and a handle left on the nominal
            // tip reads as a stuck handle. Without the flag this returns the
            // nominal length untouched and never measures the axis.
            const { headLength } = this.resolveHeadLengths(
                doubleHeaded ? turf.length(turf.lineString(coords), { units: 'meters' }) : 0,
