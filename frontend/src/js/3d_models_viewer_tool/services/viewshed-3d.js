 ...
 * 2. **The angle tests keep `>` strict, deliberately, and since 2026-09-16 that is the whole seam
 *    policy.** A sector wider than 150 degrees is split, and neighbouring pieces abut exactly
 *    (`SEAM_NARROWING_DEGREES` is zero): with `>` strict the shared boundary is painted TWICE,
 *    which on a set of measure zero is invisible, whereas `>=` would leave it painted NOT AT ALL,
 *    which is a hairline of raw ground. Switching the operator therefore has to come with a
 *    narrowing change in the same commit, and with a new measurement, because the two are a pair.
 *    The measured table lives on that constant.
