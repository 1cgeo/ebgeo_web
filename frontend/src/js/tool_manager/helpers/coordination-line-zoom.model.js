 ...
 * `military_tools/coordination_line_tool/add_coordination_line_geometry.js`, which
 * imports this module; nothing imports back, so the constants below have exactly one
 * home and need no parity test.
 *
 * IT LIVES UNDER `tool_manager/helpers/` AND NOT BESIDE THE TOOL, which is the same
 * place `boundary-zoom.model.js` sits, and for the same reason: `layers/styles/`
 * (in `core`) reads the width expression to paint the layer, so a model parked inside
 * the tool folder would draw a static edge from `core` into the `military-tools`
 * chunk. Keeping the two zoom models together in the helpers folder is what keeps
 * that edge out without a single line of chunk configuration.
 */
