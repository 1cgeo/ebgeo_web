...
/**
 * @fileoverview Pure zoom model for boundary features (node-testable; its one import
 * is the integer zoom stops, from a module that imports nothing itself).
 *
 ...
 *
 * They used to be plain `coalesce` lookups of the `calculated*` properties, which made
 * the drawing depend on a JavaScript pass rewriting THREE whole collections on every
 * frame of a zoom gesture. On this branch each of those rewrites also costs a `getData()`
 * round trip to the worker, because the three sources are owned by the diff dispatcher.
 * The composite interpolate below computes the same number on the GPU, so the pass is
 * left with what no expression can do (the echelon geometry, in kilometres, of the
 * screen-pinned boundaries) and runs once per gesture for the rest.
 *
 * Why the maths is exact between stops, and why the `min` inside each stop value deviates
 * from a hard clamp inside one zoom level only: see the header of
 * `layers/styles/zoom-expression.js`.
 *
 * Written here rather than through that module's `zoomScaledExpression` because this
 * model's rules are stricter than the builder can express. The base falls back to the
 * default unless it is a POSITIVE number (`positiveOr`), and the anchor counts only when
 * it is positive too (`hasZoomReference`: zero is the "never anchored" sentinel, which
 * the builder's `anchorDefault` would happily scale from).
 *
 * Domain: an anchor inside the map's zoom range, which is the only anchor the tool ever
 * stamps. There `2 ** (zoom - createdAtZoom)` is finite and positive, so the "factor
 * underflowed or overflowed, fall back to 1" guard of `getPixelZoomFactor` and the two
 * non-finite guards of `clampSize` are unreachable, and the expression does not carry
 * them.
 *
...
function buildBoundarySizeExpression(baseProperty, defaultValue, maxValue) {
    // `['get', ...]` yields `value`, which no arithmetic operator accepts, so every number
    // goes through `['number', ..., 0]`: it returns the property when it IS a number and 0
    // otherwise, and never throws.
    const authored = ['number', ['get', baseProperty], 0];
    const anchor = ['number', ['get', 'createdAtZoom'], 0];

    // The `Number.isFinite(n) && n > 0` of `positiveOr` and `hasZoomReference`. `n < 2n`
    // holds for every positive finite number and fails for Infinity; NaN is already out
    // on `n > 0`.
    ...
    // construction and so is the factor, so the clamp is just a `min`, and it wraps the
    // unscaled branches too (an authored size above the ceiling is clamped whether or not
 ...
 *
 * `text-size` is a LAYOUT property, but its spec entry in the vendored MapLibre 5.18
 * declares `parameters: ['zoom', 'feature']` with `interpolated: true`, so a
 * zoom-and-property function is legal as long as the `interpolate` on `['zoom']` is the
 * OUTERMOST node, which is what the builder produces.
 *
 * The missing-size fallback is the model's 35, not the 14 this expression carried before.
 ...
 *
 * That layer is a `line` layer tracing the circle polygons, so the expression feeds
 * `line-width`; the circle features carry their own `strokeWidth` base, which is what the
 * model reads.
 * @returns {Array} MapLibre expression
 */
