// Path: js/tool_manager/helpers/boundary-split.model.js
 ...
 *
 * WHY IT DOES NOT LIVE IN `military_tools/boundary_tool/`, next to the
 * orchestration it serves, and this is the same reason `boundary-zoom.model.js`
 * sits in this folder: `tests/unit/teto-de-peso-da-pagina-do-mapa.test.js` pins
 * `military_tools` at EXACTLY zero eager modules, and the context menu
 * (`ui-components`, eager on the map page) imports `canSplitBoundary`
 * STATICALLY, because the predicate decides whether the menu entry is drawn and
 * an `await` there would make the item sprout under the cursor. A model under
 * `military_tools/` would therefore drag the whole folder back into the boot
 * payload. `tool_manager/helpers/` is core by `vite.config.js`, so the static
 * edge runs ui-components to core, which is the direction that already exists.
 *
 ...
 * class would drag `military_tools` into a model the context menu loads eagerly.
