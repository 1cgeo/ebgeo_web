 ...
 * ---------------------------------------------------------------------------
 * A server atlas NEVER gets a namespace of its own. There is exactly ONE remote scope,
 * a fixed scratch suffix (`__remote`) reused by every server atlas the user ever opens,
 * and it is never entered in the local registry, never counted against the cap of 10,
 * and never listed by `listLocalAtlases()`.
 *
 * Why: the invariant that remote data does not survive logout is enforced by wiping a
 * KNOWN, SINGLE target (`enforceLocalStoreWhenLoggedOut` in `store.js`). Give the remote
 * side `ebgeo_maps__<atlasId>` and that target stops being knowable: the wipe would have
 * to enumerate databases, one missed name leaves a logged-out user with a persistent,
 * editable copy of a server atlas, which is precisely what the store-origin marker
 * exists to prevent. A single fixed scratch keeps the wipe a fixed list.
 *
 * What happens on a switch of REMOTE atlas: nothing new. The scratch is wiped and
 * refilled by the snapshot pull, exactly as `openRemoteAtlas` does today
 * (`clearAllDataStore` then `connect`). Two server atlases are never resident at once,
 * and closing a session leaves no per-atlas database behind to be found later.
 *
...
/** The single scratch suffix shared by every remote atlas (Decision 1). Module-private:
 * nothing outside needs to name the scratch, and `remoteScope()` is the way to reach it. */
const REMOTE_DB_SUFFIX = 'remote';
...
/**
 * Builds the remote scratch scope. Note that `atlasId` is carried for diagnostics only
 * and never reaches a database name (Decision 1).
 * @param {string|null} atlasId - Connected server atlas id.
 * @returns {{ kind: string, atlasId: string|null, dbSuffix: string }}
 */
export function remoteScope(atlasId = null) {
    return { kind: StoreScopeKind.REMOTE, atlasId, dbSuffix: REMOTE_DB_SUFFIX };
}
 ...
 * databases behind.
 *
 * Note the browser mechanics, measured: `deleteDatabase` fires `versionchange` on other
 * connections and stays PENDING while any holder refuses to close. In this phase the
 * tab-lock guarantees a single tab holds an atlas; when multi-tab is enabled the caller
 * will need a protocol (refuse to delete an atlas another tab holds open, or surface the
 * block as a visible error instead of a silent wait).
 *
 * @param {{ kind: string, dbSuffix: string }} scope - Scope to destroy.
 * @returns {Promise<string[]>} Database names dropped, in descriptor order.
 */
export async function dropAtlasDatabases(scope) {
