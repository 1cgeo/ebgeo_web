 ...
 *
 * Every tool that keeps a feature at a constant SIZE ON SCREEN while the map
 * zooms computes `base * 2^(zoom - createdAtZoom)` in JavaScript and writes the
 * result into a `calculated*` property, which the layer reads. That forced the
 * zoom handlers to rewrite their collections on every frame of a zoom gesture
 * (see `tool_manager/helpers/zoom-correction.helpers.js`), and on this branch
 * every one of those rewrites goes through the worker: the sixteen migrated
 * sources are written by `layers/geojson-dispatcher.js`, and reading them back
 * costs a `getData()` round trip per frame.
 *
 ...
 * which for unit-spaced stops is `2^(zoom - zk) - 1`, and
 * `vk + (vk+1 - vk) * (2^(zoom - zk) - 1)` collapses to `vk * 2^(zoom - zk)`.
 * The clamp (`min`) lives inside each stop value: where both neighbouring stops
 * are clamped the value is the clamp, and where the clamp bites between two
 * stops the interpolation deviates from a hard `min` inside that one zoom level
