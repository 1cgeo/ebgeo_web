...
## As quatro formas e as colunas que não dependem do relógio (2026-09-05)

`ferramentas.mjs` conhece as treze ferramentas de desenho: as nove de linha e, desde 2026-09-05, `circle`, `ellipse`, `rectangle` e `sector` (as quatro fecham no segundo clique, `conclusao: { gesto: 'clique-final', cliquesAntes: 1 }`). O campo `snapa` do descritor diz se a ferramenta resolve snap; com `--snapping true` a bancada conta três grandezas que não passam pelo relógio e por isso mentem menos: `resolve` (chamadas a `snapping.resolve` no gesto), `res/quadro` (resolves por quadro desenhado; o utilitário de preview leva de 10 a 13 para 1) e `timers` (temporizadores armados durante o gesto). A coluna `perdidos` conta só a escrita que outra substituiu dentro do mesmo intervalo de quadro; o critério antigo dividia a mesma variável com a latência e, com a escrita colada ao quadro, acusava 179 perdidas em 180 desenhadas (o autoteste guarda essa sequência como pior caso). A semeadura do cenário de zoom dá passo nos dois eixos, porque o retângulo recusa em silêncio a feição com menos de 10 m de altura, e `ok` sai do que o STORE guarda, nunca da contagem de chamadas a `createFeature`. Medido nesta branch com 8 `mousemove` por quadro: resolves por quadro de 11 a 12 para 1, intervalo p95 de 68 a 84 ms para 19, quadros acima de 33 ms de 17 a 19 para 0 a 2, e `updateData` na fonte principal igual antes e depois (1 por conclusão).
