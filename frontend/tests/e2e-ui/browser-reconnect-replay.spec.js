...
async function connectClient(page, cfg) {
    return page.evaluate(async ({ api, atlasId, clientId, reconnectBaseMs }) => {
...
describeOrSkip('Reconnect replay (two real browser clients + real backend)', () => {
    test('B drops, A pushes meanwhile, B auto-reconnects and recovers the missed op via sync_request', async ({
        ...
        // 1. Seed ONE shared OWNER user + atlas + map via the backend API. A conta nasce no
        //    NODE, com o e-mail já confirmado pela rota pública: o token de verificação só
        //    existe como linha no Postgres, fora do alcance do `page.evaluate`.
        const owner = await createVerifiedUser({ prefix: 'reconnect', nome: 'Reconnect Owner' });
