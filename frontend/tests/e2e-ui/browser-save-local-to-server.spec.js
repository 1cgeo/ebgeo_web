...
describeOrSkip('Salvar atlas local no servidor (UI, item 2)', () => {
    test('logged-in local user packages the local store into a new server atlas and goes live', async ({ browser }) => {
        ...
        const result = await page.evaluate(async ({ api, fid, aid }) => {
        ...
        }, { api: await clienteNaPagina(page, creds), fid: featureId, aid: atlasId });
        ...
        const synced = await page.evaluate(async ({ api, fid, aid }) => {
            for (let i = 0; i < 25; i++) {
                const pulled = await api.pullSync(aid, 0);
            ...
            return false;
        }, { api: await clienteNaPagina(page, creds), fid: liveId, aid: atlasId });
