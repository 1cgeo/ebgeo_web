...
for (const [tool, storage, name] of [['measureDistance','lines','MeasurementDistanceControl'], ['measureArea','polygons','MeasurementAreaControl']]) {
    test(`${tool}: saving a measurement preserves the next drawing tool`, async ({ page }) => {
        await page.goto('/');
        ...
        await page.evaluate(async name => {
            ...
            control._saveAsFeature = async (...args) => {
                try { return await save(...args); } finally { globalThis.measurementSaveFinished = true; }
            };
        ...
        await page.waitForFunction(() => globalThis.measurementSaveFinished);
        await expect(page.locator('[data-tool-id="point"]')).toHaveAttribute('data-active', 'true');
