 ...
 * (`trazerParaAreaLivre`): selecionar abre o painel, e ele cobre a faixa esquerda.
 */
...
import {
    esperarAlcas, alcasNaTela, arrastarAlca, botaoDireitoNaAlca, verticesDaFeicao, pontaFinal,
...
for (const ferramenta of FERRAMENTAS.filter((f) => f.alca)) {
    collabTest(`forma de ${ferramenta.id}: cada alca edita, chega ao par e ao servidor, e sobrevive ao F5`, async ({ collab }, testInfo) => {
        ...
        for (const tipo of tipos) {
            const alca = (await alcasSelecionadas(A, ferramenta, id)).find((a) => a.tipo === tipo);
