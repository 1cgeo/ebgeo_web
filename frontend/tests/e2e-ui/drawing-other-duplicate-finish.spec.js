...
for (const [tool, storage] of [['arrow', 'arrows'], ['boundary', 'boundarys'], ['coordinationLine', 'coordination_lines']]) {
    ...
    test(`${tool}: ${newGesture ? 'a new drawing can finish while the previous one saves' : 'finishing twice creates only one feature'}`, async ({ page }) => {
        ...
        await page.evaluate(async tool => {
            ...
            const control = store.getControl(({ arrow: 'AddArrowControl', boundary: 'AddBoundaryControl', coordinationLine: 'AddCoordinationLineControl' })[tool]);
        ...
        const firstCoordinates = await page.evaluate(async tool => {
            const store = await import('/src/js/store/index.js');
            const control = store.getControl(({ arrow: 'AddArrowControl', boundary: 'AddBoundaryControl', coordinationLine: 'AddCoordinationLineControl' })[tool]);
            return structuredClone(control.drawPoints);
        }, tool);
        if (newGesture) {
        ...
        const result = await page.evaluate(async storage => {
            const store = await import('/src/js/store/index.js');
            const features = (await store.getCurrentMapFeatures())[storage];
            return {
                coordinates: features.map(feature => feature.properties.baseCoordinates),
        ...
        expect(result.coordinates).toContainEqual(firstCoordinates);
        expect(result.coordinates.every(points => points.length >= 2)).toBe(true);
        const { coordinates, ...counts } = result;
        expect(coordinates).toHaveLength(newGesture ? 2 : 1);
        expect(counts).toEqual({ persisted: newGesture ? 2 : 1, drawn: newGesture ? 2 : 1 });
    });
    }
}
