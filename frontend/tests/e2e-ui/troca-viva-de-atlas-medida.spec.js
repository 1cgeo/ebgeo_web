 ...
 *
 * 3. A TROCA AO VIVO E CRONOMETRADA ATE DEPOIS DO FIM DELA, de proposito. O `await` da promessa de
 *    `__ebgeoSwitchAtlas` so retorna depois do `switchMap` e da releitura de aparencia, e so
 *    ENTAO o criterio comum e conferido. A recarga para no criterio comum e nada mais. Ou seja, a
 *    medida da troca ao vivo e um TETO e a da recarga e um piso: o ganho relatado e o menor que os
 *    dados sustentam, nunca o maior.
 *
 * ============================ O QUE ELE NAO MEDE ==============================================
 *
...
describeOrSkip('a troca de atlas ao vivo contra a troca por recarga', () => {
    test('mede as duas na mesma bancada e no mesmo relogio', async ({ browser }, testInfo) => {
        ...
        for (let i = 0; i < REPETICOES; i += 1) {
            ...
            // A troca ao vivo TROCOU mesmo. Sem isto, um `switchAtlas` que devolvesse cedo
            // deixaria o cronometro medir uma pagina que ja estava pronta.
            expect(resultado).toMatchObject({ ok: true, changed: true });
        }
        ...
        const linha = [
            ...
            `troca AO VIVO:     ${aoVivo.map(Math.round).join(' / ')} ms (mediana ${medianaAoVivo} ms)`,
