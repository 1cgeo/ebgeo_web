...
const ISENTOS = new Set([
    ...
    // A ISENÇÃO DE `docs/seguranca/aholo-viewer-pedido-de-manifesto.md` SAIU EM 2026-09-19, e a
    // razão é a mesma que as isenções acima invocam ao contrário: isenção sem beneficiário é
    // como um guarda volta a abrir sozinho. Ela nasceu porque documentos de trabalho datados
    // citavam aquele pedido, apagado em 2026-09-15 (D9, item V5, superado pelo dono na mesma
    // data). Hoje nenhum markdown do corpus vigiado o cita: a linha que o comentário anterior
    // apontava, no documento de pendências do lançamento (apagado em 2026-09-19), já não trazia o caminho, e quem o citava
    // de fato era o manifesto de dependências, que não é markdown e portanto nunca foi varrido
    // por este teste. Ou seja, a isenção já estava sem sujeito antes desta poda, e o comentário
    // que a justificava apontava para uma citação que não existe.
    //
    // O manifesto, o script de inventário e o teste que prendia os dois foram apagados em
    // 2026-09-19, porque a pasta de cópias de terceiro que eles cobriam fechou em zero em
    // 2026-09-15. O que sobrevive daquele fecho é prosa: os dois veredictos (Draco 1.5.7 byte a
    // byte; o transcodificador Basis num intervalo de commits entre as tags de snapshot da
    // linha 1.60) moram no item 4 de `docs/wiki/inventario-de-vendors.md`, e nada cobra que
    // continuem lá.
]);
...
const SIMBOLO_INEXISTENTE_DE_PROPOSITO = new Map([
    ['extrairChavesDeSetting', 'Extrator da compactação removido em 12/09/2026; o livro-razão preserva o registro histórico.'],
    ['COMPACTION_STEP', 'A compactação da fila foi desativada em 12/09/2026; as decisões anteriores são históricas.'],
    ['DELETE_IMAGE', 'consulta da rota DELETE de imagem de atlas, removida em 2026-09-26 por decisão do dono; as entradas anteriores do diário a citam como história'],
    ...
    ['accessRowsFromSharing', 'a função que alimentava o painel somente-leitura "Quem tem acesso" do Atlas Drive, podado em 2026-08-23 quando o modal COMPLETO de compartilhamento passou a abrir daquela página. O livro-razão a nomeia porque ELA é a lição: a poda deixou a função exportada e a suíte que a testava de pé, e nem o lint, nem os 5205 testes, nem o `npm run knip` acusaram, porque o knip conta o arquivo de teste como consumidor. Apagar o nome do registro tiraria dele a única coisa que ensina'],
    ...
    ['removeComments', 'funcao interna de ShaderSource do Cesium (@cesium/engine), externa a este repositorio. O livro-razao a nomeia porque ELA e a licao: ela conta as quebras de linha de cada comentario de bloco do shader e recebe null quando o bloco nao tem nenhuma, o que para a renderizacao inteira e deixa o canvas preto com uma mensagem que fala de `length`'],
