// Path: tests/unit/layer-transfer-model.test.js
//
// A ARITMETICA DE MOVER OU COPIAR UMA CAMADA INTEIRA, em node puro.
//
// O modelo (`src/js/store/layer-transfer.model.js`) e' onde as quatro decisoes de forma
// moram: qual NOME a camada recebe no destino, QUAIS feicoes podem viajar, como uma feicao
// e' remodelada para o novo dono e como fica o REGISTRO da camada. Nenhuma delas toca
// IndexedDB nem o mapa corrente, entao todas se medem aqui, e o teste de store ao lado
// mede so' a orquestracao.
//
// TRES PROPRIEDADES QUE ESTE ARQUIVO PRENDE E QUE NAO SAO OBVIAS:
//
//   1. O NOME ESCAPA METACARACTERE DE REGEX. O nome e' dado do usuario: sem escapar,
//      "Setor (A)" compila como grupo de captura e casa "Setor A", devolvendo um nome que
//      ja' esta' em uso. O caso existe porque a primeira versao da regra nao escapava.
//   2. NADA MUTA A ENTRADA. Se a gravacao no destino falhar, a origem tem de estar byte a
//      byte como estava; um clone raso deixaria `properties.attributes` compartilhado.
//   3. O ID DA CAMADA E' SEMPRE NOVO, e por isso ele e' obrigatorio. Ids de camada NAO sao
//      unicos entre mapas (todo mapa nasce com uma camada literalmente chamada `default`),
//      entao reaproveitar o id da origem escreveria por cima da camada padrao do destino.
//
// O PREDICADO DE "NAO VIAJA" CHEGA POR PARAMETRO, e e' de proposito: escrever os nomes dos
// tipos de analise aqui duplicaria `store/feature-type.registry.js`, que e' onde um tipo de
// feicao nasce. O duplo local (`isUncopyable`) e' o que torna o caso legivel sem acoplar o
// modelo ao registro.
...
describe('partitionTransferableFeatures', () => {
    it('returns empty results for an empty collection', () => {
        expect(partitionTransferableFeatures({}, isUncopyable))
            .toEqual({ transferable: {}, skipped: [], skippedParents: 0 });
    });
...
// ============================================================================
// partitionTransferableFeatures — skippedParents
// ============================================================================

describe('partitionTransferableFeatures: skippedParents conta o que a pessoa desenhou', () => {
    /**
     * Uma metade DESENHADA, do jeito que `generateProcessedFeatures` a cunha: ela espalha as
     * propriedades do pai, entao carrega o MESMO `source`. E' por isso que o `source` nao
     * distingue os dois e o balde e a unica pista.
     * @param {string} id - Id de sync.
     * @returns {object} O filho.
     */
    const filho = (id) => makeFeature(id, 'los');

    it('uma visada inteira (pai + duas metades) conta UM', () => {
        const { skipped, skippedParents } = partitionTransferableFeatures({
            points: [makeFeature('p1')],
            los: [makeFeature('los1', 'los')],
            processed_los: [filho('los1-visible'), filho('los1-blocked')]
        }, isUncopyable);

        // A particao segura os TRES: nenhum deles pode viajar.
        expect(skipped).toHaveLength(3);
        // A frase cita UM, que e o numero de coisas que a pessoa desenhou.
        expect(skippedParents).toBe(1);
    });

    it('duas familias contam dois', () => {
        const { skippedParents } = partitionTransferableFeatures({
            los: [makeFeature('a', 'los')],
            visibility: [makeFeature('b', 'visibility')],
            processed_visibility: [makeFeature('b-x', 'visibility')]
        }, isUncopyable);

        expect(skippedParents).toBe(2);
    });

    it('CONTROLE: so metades desenhadas conta ZERO, que e o caso do `||` no chamador', () => {
        const { skipped, skippedParents } = partitionTransferableFeatures({
            processed_los: [filho('orfa-visible')]
        }, isUncopyable);

        expect(skipped).toHaveLength(1);
        expect(skippedParents).toBe(0);
    });

    it('nada pulado conta zero', () => {
        const { skipped, skippedParents } = partitionTransferableFeatures({
            points: [makeFeature('p1')]
        }, isUncopyable);
    ...
    it('entrada vazia e entrada nula devolvem o campo, e nao `undefined`', () => {
        expect(partitionTransferableFeatures({}, isUncopyable).skippedParents).toBe(0);
        expect(partitionTransferableFeatures(null, isUncopyable).skippedParents).toBe(0);
    });
});
