...
describe('a frase da transferência de camada', () => {
    ...
    it('origem NÃO esvaziada: é AVISO, nomeia o estado, e não fala em camada vazia nem em "movida"', () => {
        ...
        expect(aviso.text).toContain('o mapa de origem está bloqueado');
        // A saída prometida foi MEDIDA em navegador (`browser-collab-transferencia-origem-cheia.spec.js`),
        // e a frase é específica de propósito: recarregar tira ESTAS feições da origem, não traz "o
        // estado do servidor" (um registro que só existe neste disco sobrevive ao recarregamento).
        expect(aviso.text).toContain('Recarregue a página para que elas saiam do mapa de origem');
