...
// flush waits for a map settle signal a fake map never sends (2 s per flush, and the 5 s
// test timeout kills it). The preview path here writes only the NON migrated feedback and
// handle sources, so nothing under test goes through the dispatcher; the stand-in is here
// because importing a control pulls the real module in, and the same one is used by
// `tests/unit/preview-frame-gate-driven.test.js`, which drives the nine line tools.
 ...
 * that has nothing to do with the code. The reason and the measure are in
 * `tests/unit/preview-frame-gate-driven.test.js`, which pays the same cost for the nine
 * line tools; here the four shapes pay it. A hook has its own budget, so the cost is paid
 * once and out of the measurement.
 */
const HEAVY_MODULES = [
    '../../src/js/draw_tools/circle_tool/add_circle_control.js',
    '../../src/js/draw_tools/ellipse_tool/add_ellipse_control.js',
...
beforeAll(async () => {
    for (const modulo of HEAVY_MODULES) await import(/* @vite-ignore */ modulo);
}, 120000);
