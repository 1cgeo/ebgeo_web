...
describe('rotuloDeAndar', () => {
  ...
  test('andar de dois algarismos entrega os DOIS, nao o primeiro', () => {
    // Sem o `match(/^\d+/)`, um `limpo[0]` devolveria '1' para o 10o e para o
    // 11o andar, e os tres marcadores ficariam indistinguiveis entre si.
    assert.equal(rotuloDeAndar(10, '10º andar'), '10');
    assert.equal(rotuloDeAndar(11, '11º andar'), '11');
    assert.notEqual(rotuloDeAndar(10, '10º andar'), rotuloDeAndar(1, '1º andar'));
  });
