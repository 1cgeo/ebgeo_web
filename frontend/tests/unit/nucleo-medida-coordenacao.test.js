 ...
 *
 * O QUE ESTE RAMO ACRESCENTA AO LOTE DO `main`. Aqui o `applyCustomColor` ja pintava a
 * TINTA (traco e preenchimento preto, nas tres grafias que o catalogo usa) em vez de trocar
 * o branco, e o hex invalido e no-op BYTE A BYTE. O Nucleo e o pior caso das duas coisas ao
 * mesmo tempo, e por isso ele ganhou bloco proprio no fim deste arquivo: ele e simbolo de
 * TRACO PURO, sem uma unica area branca, ou seja exatamente a metade do catalogo em que a
 * regra velha da cor era inerte, e a situacao roda DEPOIS da cor sobre o mesmo elemento.
 */
...
describe('nitidez do bitmap', () => {
    ...
    it('a nitidez de TELA rasteriza no tamanho lógico, para a miniatura do combobox', async () => {
        // A previa e as treze miniaturas nao vao para o mapa. Este e o caminho que o
        // `point-selector.modal.js` usa, e ele nao pode mudar o tamanho LOGICO devolvido:
        // so o bitmap encolhe.
        //
        // DESDE O RECORTE a igualdade e ate a grade do bitmap, e nao byte a byte: o canvas
        // e `round(viewBox * escala * nitidez)` pixels, entao o tamanho logico devolvido
        // (canvas / nitidez) cai numa grade de 1/nitidez. Entre nitidez 4 e 1 as duas
        // medidas podem separar-se por menos de um pixel logico, e essa e a folga toda; a
        // regressao que este caso pega e a de ordem de grandeza (quatro vezes maior).
        const local = new CoordinationMeasureGenerator();
        let pedido = null;
        local.convertToPngBlob = async (svg, largura, altura) => {
            pedido = { largura, altura };
            return { blob: {}, width: largura, height: altura };
        };

        const mapa = await local.generateSymbolBlob({ pointCode: 'ECHELON_16' });
        const tela = await local.generateSymbolBlob({ pointCode: 'ECHELON_16' }, { nitidez: 1 });

        expect(tela.pixelRatio).toBe(1);
        expect(Math.abs(tela.width - mapa.width)).toBeLessThan(1);
        expect(Math.abs(tela.height - mapa.height)).toBeLessThan(1);
        expect(pedido.largura).toBe(tela.width);
    });
...
// ============================================================================
// A METADE QUE SÓ EXISTE NESTE RAMO
// ============================================================================

/**
 * A cor personalizada AQUI pinta a TINTA (traco e preenchimento preto nas tres grafias que
 * o catalogo usa), e nao o branco. O Nucleo e o pior caso dessa regra ENCONTRANDO a
 * situacao, e por dois motivos que so se juntam nele:
 *
 * - ele e TRACO PURO. Nao tem uma area branca em lugar nenhum, ou seja e a metade do
 *   catalogo em que a regra ANTIGA da cor (trocar `rgb(255,255,255)`) era inerte. Um teste
 *   de cor num simbolo com interior branco passaria com as duas regras e nao acusaria nada;
 * - a cor e a situacao escrevem no MESMO elemento, o contorno, e a cor roda ANTES. Nao ha
 *   um segundo alvo em que uma delas possa cair por acidente.
 */
describe('cor personalizada e situação juntas, sobre um símbolo de traço puro', () => {
    const NUCLEO = COORDINATION_POINTS_CATALOG.ECHELON_FT_16.svg;

    it('CONTROLE: o Núcleo é traço puro, sem uma única área branca', () => {
        // Se esta afirmacao cair, os casos abaixo deixam de medir o que dizem medir: com
        // area branca no desenho, a regra velha da cor tambem mexeria em alguma coisa.
        expect(NUCLEO).not.toMatch(/rgb\(255,\s*255,\s*255\)/);
        expect(NUCLEO).toContain('fill="none"');
        expect(NUCLEO.match(/stroke="black"/g).length).toBeGreaterThanOrEqual(1);
    });

    it('a ordem cor -> situação pinta a tinta E traceja, no mesmo elemento', () => {
        const pintado = gerador.applyCustomColor(NUCLEO, '#11FF00');
        const final = gerador.aplicarSituacaoDoNucleo(pintado, 'preparado');

        expect(final).toMatch(/data-nucleo="contorno"[^>]*stroke="rgb\(17, 255, 0\)"[^>]*stroke-dasharray/);
        expect(final).not.toContain('stroke="black"');
        expect(final.match(/stroke-dasharray/g)).toHaveLength(1);
    });

    it('as duas COMUTAM, e é isso que faz a situação "sobreviver" à cor', () => {
        // A cor mira `stroke="black"` e a situacao mira `data-nucleo="contorno"`: alvos
        // diferentes no mesmo elemento, entao a ordem nao muda o resultado. Isto e mais
        // forte que "a ordem que usamos funciona", porque nao depende de quem chama.
        const corDepois = gerador.applyCustomColor(
            gerador.aplicarSituacaoDoNucleo(NUCLEO, 'preparado'),
            '#11FF00'
        );
        const corAntes = gerador.aplicarSituacaoDoNucleo(
            gerador.applyCustomColor(NUCLEO, '#11FF00'),
            'preparado'
        );

        expect(corDepois).toBe(corAntes);
    });

    it('a REGRA ANTIGA da cor seria inerte aqui, que é por que ela precisou mudar', () => {
        // A regra de antes trocava so o branco. Sobre um simbolo de traco puro ela devolve
        // o desenho intacto, e o controle de cor nao fazia nada na tela.
        const regraAntiga = (svg, rgb) => svg.replace(/rgb\(255,\s*255,\s*255\)/g, rgb);

        expect(regraAntiga(NUCLEO, 'rgb(17, 255, 0)')).toBe(NUCLEO);
        expect(gerador.applyCustomColor(NUCLEO, '#11FF00')).not.toBe(NUCLEO);
    });

    it('hex inválido continua no-op byte a byte, e a situação ainda traceja', () => {
        // Contrato deste ramo, que o `main` nao tem: entrada que nao se entende nao
        // autoriza mexer no desenho. A situacao nao depende da cor para funcionar.
        //
        // LIMITE CONHECIDO, medido aqui: `null` nao entra na lista porque `hexToRgb` faz
        // `hex.replace` sem guarda e LANCA. O chamador real nunca o passa
        // (`properties.fillColor || 'none'` em `generateSymbolBlob`), entao fica registrado
        // em vez de consertado num lote de porte.
        for (const lixo of ['#GGG', 'verde', '#11FF0', '']) {
            expect(gerador.applyCustomColor(NUCLEO, lixo), String(lixo)).toBe(NUCLEO);
        }

        expect(() => gerador.applyCustomColor(NUCLEO, null)).toThrow(TypeError);

        const final = gerador.aplicarSituacaoDoNucleo(gerador.applyCustomColor(NUCLEO, '#GGG'), 'preparado');
        expect(final).toMatch(/data-nucleo="contorno"[^>]*stroke="black"[^>]*stroke-dasharray/);
    });

    it('o caminho INTEIRO do gerador aplica as duas, sem passar pelo chamador', () => {
        // As duas chamadas moram dentro de `generateSymbolBlob`, e e la que a ordem vale.
        // Sem este caso, os anteriores mediriam so os dois metodos soltos.
        const local = new CoordinationMeasureGenerator();
        let desenhado = null;
        local.convertToPngBlob = async (svg) => {
            desenhado = svg;
            return {};
        };

        return local.generateSymbolBlob({
            pointCode: 'ECHELON_FT_16',
            fillColor: '#11FF00',
            status: 'preparado'
        }).then(() => {
            expect(desenhado).toMatch(/data-nucleo="contorno"[^>]*stroke="rgb\(17, 255, 0\)"[^>]*stroke-dasharray/);
            expect(desenhado).not.toContain('stroke="black"');
        });
    });
});
