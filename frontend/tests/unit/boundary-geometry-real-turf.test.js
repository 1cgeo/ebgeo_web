// Path: tests/unit/boundary-geometry-real-turf.test.js

/**
 * @fileoverview Boundary geometry against the REAL turf bundle the app ships.
 *
...
beforeAll(async () => {
    ...
    expect(turf.distance(turf.point([0, 0]), turf.point([1, 0]), { units: 'kilometers' }))
        .toBeCloseTo(111.19, 1);

    // The geometry reads the bare global, like the app does under its <script> tag.
 ...
 * this suite uses.
 * @param {Object} geometry - Generated MultiLineString
 * @param {Array} baseCoordinates - The boundary spine
 * @returns {{segments: Array, strokes: Array}} Lines split by where they lie
 */
 ...
 * @param {Array} baseCoordinates - The spine
 * @param {Object} [extra] - Property overrides
...
describe('stale calculatedSymbolSize', () => {
    it('changes nothing about the drawing when the zoom is known', () => {
        ...
        // 50 km is the MAX_SYMBOL_SIZE_KM clamp: the value a boundary picks up
...
// ============================================================================
// symbolSizeBounds — what the panel slider shows
// ============================================================================
...
describe('symbolSizeBounds (what the "Tamanho do símbolo" slider shows)', () => {
    ...
    it('tops out at the line-length cap and shows the authored size when anchored to the ground', () => {
        ...
        // lengthKm * 0.5 / (3 instances * 1 symbol * 1.8)
