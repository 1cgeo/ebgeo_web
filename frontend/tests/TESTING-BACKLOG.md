...
1. **`x ?? 0` / `x || default` NÃO protegem contra NaN.** Padrão recorrente confirmado em: `recalcLabelSize`, `add_circle/ellipse/visibility.validate` (NaN/Infinity radius/aperture), `wmm_calculator` (NaN coord), `terrain.exaggeration||1.5` (engole 0), `_niceNumber(0)→NaN`. **Sempre fixar comportamento atual com teste e marcar com flag** para a correção ser deliberada.

   **`calculateZoomCorrectedValue` SAIU desta lista em 2026-09-02, e a saída dele muda o conselho acima.** Ele foi guardado (sem `createdAtZoom` finito o fator é 1; valor-base não finito devolve `config.fallbackValue`), e o que se aprendeu ao guardá-lo é que "fixar o comportamento atual com teste" não é neutro: o bloco que afirmava os oito desfechos NaN desta função ficou verde por meses, e o verde dele era a prova de que o defeito continuava lá. O NaN chegava ao `icon-size` (que não tem `coalesce`) e à caixa de seleção, e o consumidor final é código NATIVO, que não levanta exceção: uma imagem sem `createdAtZoom` travava o thread principal da página, medido 6 de 6. Ou seja, **ao adiar um destes casos, escreva junto QUEM CONSOME o valor**, porque é o consumidor que decide se o adiamento é barato; e quando o repositório já resolveu a mesma pergunta noutro arquivo (aqui `boundary-zoom.model.js`, que devolve fator 1 e diz no `fileoverview` que não reusa esta função porque ela propaga NaN), a divergência entre os dois é o sinal de que o lado adiado é o errado. Decisão registrada em `docs/decisions/decisions-2026.md`; guarda em `frontend/tests/unit/zoom-correction-sem-referencia.repro.test.js`.
