...
class AddVisibilityGeometry extends BaseGeometry {
    ...
    async calculateViewshed(center, radius, bearing, aperture,
        ...
        // One sampler for the whole viewshed: ~10.000 samples share one lookup
        ...
        for (let rayIdx = 0; rayIdx <= numRays; rayIdx++) {
            ...
            // Yield on ELAPSED TIME, never on a ray count. The sweep itself costs
            // 14-23 ms for the whole 10.000-sample grid, while one nextPaint costs
            // a frame: yielding every 5 rays spent 957 ms of a 979 ms run parked
            // (aperture 359, measured 2026-09-11). Yielding every PAINT_BUDGET_MS
            // keeps the progress bar and the cancel path alive at a cost the
            // measurement can see, and the paste path (no callback) still yields.
