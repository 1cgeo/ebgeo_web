 ...
 *
 * WHY THE MECHANISM EXISTS. Measured on 2026-09-04 (docs/desempenho-terreno-2026-09-04.md):
 * with terrain on, a RASTER base costs between half and a third of a VECTOR base's
 * frame (1 RTT stack against 2 or 11) and holds 60 fps at rest on the four times
 * slower CPU (19 ms against 34 ms). The raster base pays for that with labels baked
 * into the image and coverage limited to the clip that was generated: outside it the
 * map goes WHITE. So the switch is opt-in, off by default, and gated by an optional
 * coverage box.
 *
 ...
 * which is what every base in this repository does today. Everything else fails
 * CLOSED and answers false, because the cost of a wrong "true" is a white screen
 * while the cost of a wrong "false" is the frame budget the app already lives with.
 *
 * THE ANTIMERIDIAN IS NOT HANDLED, deliberately. Nothing in `src/` treats it (zero
 * occurrences), so a treatment invented here would be the only one in the house and
 * would have no second reader. A box with `west > east` is rejected whole, which
 ...
 * @param {Array<string>|null} [params.available] - Base ids that are enabled AND have a style
