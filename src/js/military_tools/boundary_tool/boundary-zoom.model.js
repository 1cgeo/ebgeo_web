 ...
 * RELATIVE TO THE TERRAIN instead of a constant size on screen. The anchor is a
 * single pair of properties, named exactly like the military symbol's:
 *
 *  - `createdAtZoom`          the reference zoom (1 decimal place)
 *  - `zoomCorrectionEnabled`  `false` opts the feature out (fixed on screen)
 *
 * Legacy features carry neither, so `hasZoomReference` is false, every factor is
 * 1 and the render is byte-for-byte what it was before this model existed. That
 * is why `calculateZoomCorrectedValue` (`@tools/helpers/zoom-correction.helpers.js`)
 * is deliberately NOT reused here: it propagates `NaN` when `createdAtZoom` is
