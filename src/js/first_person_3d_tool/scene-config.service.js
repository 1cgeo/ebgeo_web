 ...
 * @description Reads the `firstPerson3d` section of config.js and turns each
 * scene into concrete asset URLs, then fetches the two data files a scene needs
 * (curated markers and the collision octree).
 *
...
import { hasFirstPersonScenes as configHasFirstPersonScenes } from '@js/config.helpers.js';
 ...
 *
 * @param {string} basePath - Raw basePath from the config entry
...
/**
 * Check that a config entry can actually be resolved into a scene.
 * `id` addresses the scene (deep link, catalog); `basePath` addresses its files.
 * @param {Object} scene - Raw config entry
 ...
 * @returns {boolean} True if the module is enabled and has at least one scene
 */
export function hasFirstPersonScenes() {
    return configHasFirstPersonScenes();
}
 ...
 * @returns {Array<Object>} Scene entries, or [] when the module is disabled
 */
export function getFirstPersonScenes() {
    if (!config.firstPerson3d?.enabled) return [];
    const scenes = config.firstPerson3d.scenes;
    if (!Array.isArray(scenes)) return [];
    return scenes.filter(isUsableScene);
}
