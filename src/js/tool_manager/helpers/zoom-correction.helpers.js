 ...
 *
 * WHY ZERO IS STILL A REFERENCE, and the guard tests finiteness only. Zero is
 * tempting to reject as well, since several tools stamp `createdAtZoom: 0` in
 * their DEFAULT_PROPERTIES and overwrite it at the point of creation, so it reads
 * like a "never anchored" sentinel. But a feature legitimately anchored at zoom 0
 * is indistinguishable from those, and rejecting zero would silently change the
 * size of every tool that uses this helper. The defect being fixed is the
 * non-finite one; zero keeps today's behaviour.
 */
