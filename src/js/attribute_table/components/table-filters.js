 ...
 * @param {HTMLElement} filtersContainer - Filters container element
 * @param {string} value - New search value
 */
export function setSearchValue(filtersContainer, value) {
    const input = filtersContainer.querySelector('.attribute-table-search-input');
    const clearBtn = filtersContainer.querySelector('.attribute-table-search-clear');
...
/**
 * Updates the selected only toggle.
 * @param {HTMLElement} filtersContainer - Filters container element
 * @param {boolean} value - New toggle value
 */
export function setSelectedOnly(filtersContainer, value) {
    const checkbox = filtersContainer.querySelector('.attribute-table-selected-toggle input');
    if (checkbox) {
...
/**
 * Updates the type chips active state.
 * @param {HTMLElement} filtersContainer - Filters container element
 * @param {Set<string>} activeTypes - Active types (empty = all)
 */
export function setActiveTypes(filtersContainer, activeTypes) {
    const chips = filtersContainer.querySelectorAll('.attribute-table-type-chip');
    const allActive = activeTypes.size === 0;

    chips.forEach((chip) => {
        const type = chip.dataset.type;
        if (allActive || activeTypes.has(type)) {
            chip.classList.add('active');
        } else {
            chip.classList.remove('active');
        }
