 ...
 *
 * WHY THIS EXISTS. A MapLibre GeoJSON source is read with an ASYNCHRONOUS
 * `getData()` (a round trip to the worker that answers with a clone) and written
 * with a synchronous `setData()`. Every `getData -> mutate -> setData` therefore
 * has a window in which a second caller can read the pre-mutation clone and,
 * when it writes, silently drop the first caller's changes: last write wins, no
 * error anywhere. With several such call sites on the same source (the boundary
 * tool has nine) the losing interleaving is not rare, it is the normal case on
 * boot, where a restore pass and a zoom pass share a frame.
 *
 * Running those read-modify-write cycles through one queue makes the pair
