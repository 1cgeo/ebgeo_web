 ...
 * Ensures coordination_measures exists (added in v1.4) and that the v2.3
...
function normalizeMapDataForCurrentVersion(mapData) {
    ...
    // Give the map the v2.3 shape. The IndexedDB half of this is `migrateToV2_3`,
    // which a FILE never passes through: MIN_SCHEMA_VERSION is 1.3, so an accepted
    // .ebgeo may well predate the Coordination Line tool and arrive without its
    // collection. Left alone, the tool would have no source to draw into on that
    // map. Same pure function as the storage migration, so the two paths cannot
    // drift apart.
