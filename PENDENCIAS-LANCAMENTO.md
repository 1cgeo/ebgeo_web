...
- cobertura-desenho-estilo.spec.js passa a comparar o valor escolhido pela pessoa, nunca as propriedades derivadas do zoom (calculatedLineWidth, calculatedSize, selectionBox), que o autor recalcula depois do envio e o servidor guarda da derivação anterior, na quarta casa decimal. O produto não muda. Taxa medida antes: 7 de 12 no integracao_backend (eaa4dd14), 14 de 39 com os gestos em massa, 0 de 39 na nuvem de 4 núcleos sozinho; há ainda localizadores que não aparecem e páginas fechadas no meio, a olhar junto.
- Coleta de imagem órfã: medir, nesta nuvem, o gatilho zerar_marca_de_imagem_citada com import e envio grandes, com e sem imagem marcada, e só então escolher o conserto. Enquanto nenhuma imagem está marcada ele custa uma consulta a um índice parcial vazio; depois da primeira marcação, toda escrita nas doze tabelas-fonte (operations e features inclusive) lê o texto da linha (cerca de 10 ms por MB) e abre uma subtransação por linha, por causa do bloco EXCEPTION, e um import, clone ou envio grande numa transação só passa de 64 subtransações, onde o Postgres degrada. Os consertos na mesa: tirar o EXCEPTION (a escrita do usuário passaria a falhar junto com o gatilho) ou mover a zerada para fora da transação da escrita.
- Catálogo semeado: uma migração nova tira as quatro camadas de exemplo com endereço http://localhost/tiles/... (declividade, hipsometria, rodovias-federais e limites-municipais, da base backend/src/database/migrations/005_catalogo.sql, que está congelada). Nenhum código cita esses ids; o administrador cadastra as reais depois do deploy.
