...
- A figura de slide por referência (feita em 2026-09-26, desenho em docs/wiki/imagens-atlas.md). Medida na nuvem com briefing-figura-em-link-lento.spec.js numa cópia que estrangula a página inteira, 3 de 3: figura de 156 kB no colega em 45 a 46 s, o texto digitado ao lado dela junto, e um único envio do /sync de 10 kB (antes, 8 envios somando 2,4 MB e o texto sem chegar em 5 minutos). Tem três limites declarados, nenhum com perda do que já estava no slide:
  ...
  - uma figura colada de OUTRO atlas (a área de transferência atravessa a troca de atlas) chega citando um id que este atlas não tem e aparece vazia; antes ela era um data URL e viajava junto. Conserto proposto, não feito: o formato do Quill escrever os bytes na cópia (o método html() do blot) quando a figura já estiver resolvida;
...
- Proposta que ficou do mapeamento da figura, não decidida: tirar o content dos slides de dentro da op de BRIEFING e do previousData dela, porque o servidor só usa a ordem dos slides dessa op. Com a figura por referência o content é só texto, então o ganho caiu muito; medir antes de decidir.
...
Nenhum item em aberto: todos foram feitos em 2026-09-26, cada um com repro, conserto, controle negativo e gate (o último foi a figura de slide por referência). Os limites que ficaram estão na seção 3.1.
