...
- Figura de slide de briefing por referência, como as fotos (o custo medido está no item 3). Mapeado em 2026-09-26, sem código ainda:
  - hoje toda porta de figura do editor (botão, colar, soltar, HTML colado) termina em insertQuillImages (utilities/quill-helpers.js), que comprime para no máximo 800x600 e grava um data URL JPEG dentro do content do slide; cada autosave leva o content umas quatro vezes (a op de BRIEFING carrega o briefing inteiro com todos os slides, mais o previous, e a op de SLIDE leva o slide inteiro mais o anterior), o que fecha com os 2,4 MB medidos;
  - desenho mínimo: a figura vira <img data-figura="<uuid>"> sem src guardado (o sanitizador aceita o atributo como aceita data-list); os bytes vão por storeImage + registrarEnvioDeImagem, como a foto anexa; a espera da op pelo blob (idsDeFotosDaEntidade, user_data/photo-refs.js) passa a ler data-figura do content, o que já cobre o despachante, o retrato e a marcação de recusa; o editor, a apresentação (inclusive o visitante do link público) e o PDF resolvem a figura pelo getImage antes de desenhar; o .ebgeo, a importação aditiva, o "Enviar ao servidor" e o clone do servidor passam a reescrever os ids de data-figura junto com os das fotos; slide antigo com data URL continua sendo lido;
  - ganho independente e grande: tirar o content dos slides de dentro da op de BRIEFING e do previousData, porque o servidor só usa a ordem dos slides dela;
  - PERGUNTA AO DONO (o risco que decide a forma): um colega com build antigo (a aba velha com fila viva na virada do main) tem sanitizador que apaga data-figura e vê a figura vazia; se ele editar o texto daquele slide, grava o HTML sem a referência e a figura some para todos. A alternativa é um src sentinela em https (https://figura.ebgeo/<uuid>), que sobrevive ao sanitizador e ao Quill antigos, mas aparece quebrada naquela aba.
- Depois de um resgate involuntário, a mesma conta ganha a saída "enviar as pendências a este atlas", além de "Apagar e abrir" e de "Enviar ao servidor". A operação não carrega autor, então o desenho começa por aí. Lido no código em 2026-09-26, sem código ainda:
  - o resgate (adoptRemoteAtlasAsLocal, store/local-atlas.api.js) grava só id, nome, dbSuffix e datas, então o primeiro passo é guardar na entrada a conta e o atlas de origem no instante do resgate;
  - a saída nova devolveria o namespace ao atlas do servidor sem esvaziá-lo, e a fila (que é a do atlas de origem, com as ops carimbadas) sai no connect seguinte;
  - PERGUNTA AO DONO: no atlas local resgatado a fila NÃO registra edição (o log de operações fica desligado fora do atlas de servidor), então o que a pessoa editou na cópia local DEPOIS do resgate não está em fila nenhuma, e o retrato do servidor o sobrescreve ao abrir. Ou a saída nova só aparece enquanto a cópia não foi editada depois do resgate (exige marcar a primeira edição), ou ela aparece sempre e o texto avisa que o que foi feito depois do resgate se perde, ou as duas edições são reconciliadas (bem maior).
