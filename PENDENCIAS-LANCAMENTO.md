...
- A figura de slide por referência (feita em 2026-09-26, desenho em docs/wiki/imagens-atlas.md). Medida na nuvem com briefing-figura-em-link-lento.spec.js numa cópia que estrangula a página inteira, 3 de 3: figura de 156 kB no colega em 45 a 46 s, o texto digitado ao lado dela junto, e um único envio do /sync de 10 kB (antes, 8 envios somando 2,4 MB e o texto sem chegar em 5 minutos). Tem dois limites declarados, nenhum com perda do que já estava no slide (o terceiro, a figura colada de outro atlas, foi consertado em 2026-09-26: a cópia leva os pixels):
...
A primeira leva foi toda feita em 2026-09-26, cada item com repro, conserto, controle negativo e gate (o último foi a figura de slide por referência); os limites que ficaram estão na seção 3.1. A segunda leva são as sete decisões da tarde de 2026-09-26 (registro no diário de decisões) e dois specs desatualizados:

- Navegador mínimo: o piso fica (Chrome e Edge 109, Firefox ESR 115), e o CSS se ajusta. O seletor has exige Firefox 121 e aparece em 16 regras de 10 arquivos (em sidebar.css as duas regras que escondem .unified-attributes-panel caem inteiras no ESR 115, e ui_manager.js ainda cria painéis com essa classe; não medido): trocar por uma classe posta pelo JS. O color-mix exige Chrome 111 (33 usos em admin.css, mais catalog, account, sidebar e attribute-table): trocar por cores calculadas nos tokens. Falta também o censo do dist moderno contra APIs acima do piso, a consequência 3 da política.
- Coleta de imagem órfã: MEDIDO em 2026-09-26, falta o dono confirmar a recomendação. A bancada nova (backend/tests/bench/imagens-orfas-gatilho.bench.mjs, import de 5000 feições e push de 500 ops, cinco repetições, amostrando o contador de subtransações) desmentiu a premissa da pendência: a subtransação do gatilho só ganha XID quando zera uma marca, então com imagem marcada e não citada o import fica em zero subtransações contadas; o push transborda as 64 com o gatilho DESLIGADO também (um savepoint por op do sync); e um leitor de outro atlas fica em 156 ms de mediana nas quatro condições. O custo que existe é a varredura do texto enquanto houver marca: import de 653 para 985 ms (+50%), push de 9,1 para 10,7 s (+17%). Os dois consertos que estavam na mesa não tiram esse custo, e tirar o EXCEPTION passaria a derrubar a escrita do usuário junto com o gatilho. Recomendação: não mexer no gatilho. Detalhe em docs/wiki/imagens-atlas.md.
...
4. npm run lint e npm test na raiz, em comandos separados. Nesta nuvem, cada perna pede um Node (seção 0.1, passo 5): o vitest do frontend com o Node 24, o backend e o e2e de contrato com o Node 22 do sistema. A raiz para na primeira perna vermelha; se o frontend reprovar por ambiente, rode as outras duas separadas (npm run test:backend e npm run test:e2e).
...
- Máquina carregada (o integracao_backend com outras sessões rodando): cobertura-desenho-estilo.spec.js e cobertura-desenho-abas.spec.js comparam desde 2026-09-26 o valor escolhido (tests/e2e-ui/helpers/valor-escolhido.js), sem as propriedades que o autor deriva do zoom, e o comparador tem teste em node com o par medido na quarta casa decimal. Nesta nuvem nunca reprovaram (0 de 39 antes; linha e texto verdes depois), então a taxa sob carga está por medir: rodar em série, com --retries=0, e comparar com os 7 de 12 de antes (eaa4dd14). Os localizadores que não aparecem e as páginas fechadas no meio, vistos na mesma máquina (14 de 39 com os gestos em massa), são outro defeito e continuam abertos.
- Firefox:
