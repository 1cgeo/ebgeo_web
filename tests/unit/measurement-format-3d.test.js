...
// value, not from the `formatted` string persisted alongside it.
...
describe('formatArea3D', () => {
    ...
    it('never emits hectares for any finite input', () => {
        // The plain unbounded double generator almost never lands inside the
        // old hectare window [1e4, 1e6), so on its own this property stays
        // green with the hectare branch put back (measured). The bounded
        // generator is what makes it discriminate.
        const anyFinite = fc.double({ noNaN: true, noDefaultInfinity: true });
        const aroundTheOldHectareWindow = fc.double({ min: 0, max: 1e7, noNaN: true });

        fc.assert(fc.property(
            fc.oneof(anyFinite, aroundTheOldHectareWindow),
            (v) => {
...
// ============================================================================
// formatMeasurementResult3D — value wins over the persisted string
// ============================================================================
...
describe('formatMeasurementResult3D', () => {
    ...
    it('returns null when there is neither a value nor a stored string', () => {
