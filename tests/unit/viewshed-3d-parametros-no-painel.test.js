...
/**
 * Campo horizontal e distancia da analise de visibilidade 3D.
 *
 * O painel e reconstruido a partir da COPIA que a entidade de origem carrega
 * em `properties.viewshedData`, capturada na criacao. Quem trocava a distancia
 * ou o campo horizontal recriava o cone e a linha do store, mas deixava essa
 * copia velha: clicar fora e voltar no marcador trazia o valor antigo de volta.
 */
...
// O store real le de um `deepClone` do cache em memoria, entao cada leitura
// devolve um objeto NOVO. O duble faz o mesmo: sem isso a entidade de origem
// ficaria apontando para o proprio registro do store e o teste nunca veria a
// copia velha que o painel reabre.
...
// ===== Cesium dublado =====
// A aritmetica e crua de proposito: o ENU vira identidade, entao a rotacao dos
// sub-viewsheds roda sem quebrar. O que esta sob teste e o encanamento do dado,
// nao a geometria do cone.
...
const CesiumStub = {
    ViewShed3D: class {
        constructor(viewer, options = {}) {
            ...
            CesiumStub.ViewShed3D.criados.push(this);
        }
    ...
    ScreenSpaceEventType: { LEFT_CLICK: 'leftClick' },
...
beforeEach(async () => {
    ...
    CesiumStub.ViewShed3D.criados = [];
    vi.resetModules();
    tool = await import('../../src/js/3d_models_viewer_tool/tools/viewshed_tool_3d.js');
});
...
describe('distancia e campo horizontal da visibilidade 3D', () => {
    it('a distancia nova chega ao cone recriado e a copia que o painel reabre', async () => {
        ...
        expect(CesiumStub.ViewShed3D.criados.map(v => v.distance)).toEqual([2000]);
    ...
    it('o campo horizontal novo chega ao cone recriado e a copia que o painel reabre', async () => {
        ...
        expect(CesiumStub.ViewShed3D.criados.map(v => v.horizontalAngle)).toEqual([60]);
    ...
    it('acima de 150 graus o setor e repartido, e a copia do painel guarda o total pedido', async () => {
        ...
        expect(CesiumStub.ViewShed3D.criados).toHaveLength(2);
    ...
    it('trocar a distancia nao derruba o observador para o solo', async () => {
        ...
        expect(CesiumStub.ViewShed3D.criados[0].cameraPosition.z).toBeCloseTo(31.5, 6);
    });
