...
const srcRoot = join(repoRoot, 'src', 'js');
...
function createdSourceNames() {
    const names = new Set();
    const creator = /(?:setOrCreateSource|ensureSource|setSourceData|addSource|sourceId:)\s*\(?\s*(?:[^,()]+,\s*)?'([^']+)'/g;
...
describe('ZOOM_INVARIANT_SOURCES aponta para fontes que existem', () => {
    ...
    it('o instrumento acha as duas listas, e nao passa por vazio', () => {
        expect(declaredSourceNames().length).toBeGreaterThanOrEqual(9);
