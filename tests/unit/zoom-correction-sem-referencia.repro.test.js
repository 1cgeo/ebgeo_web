 ...
 * `icon-size` (`src/js/layers/styles/content.layers.js` reads a bare
 * `['get', 'calculatedSize']` with no coalesce) and into the selection-box geometry. A
 * NaN there raises no exception anywhere: it travels into native placement code.
 * Measured on this branch with the owner's file, 6 runs out of 6: selecting the image
 * from the features tab (the click zooms to it), pressing Esc and switching maps at high
 * zoom FREEZES the page's main thread, and the debugger cannot pause. The hypothesis
 * that the NaN is the root cause is supported, not proven.
 *
 ...
 * what those legacy features did before zoom correction existed. A base value that is not
 * finite has nothing to correct, so the function returns `config.fallbackValue`.
 *
 ...
 * only way to know this green is not vacuous.
 *
 * WHAT THIS FILE DOES NOT REACH. The freeze itself is native and needs a browser, which
 * this package has no harness for; the screen half is a manual probe against the running
 * app. Everything here is pure arithmetic and pure geometry in node.
 */
...
describe('controle positivo: a correcao real nao mudou', () => {
    ...
    // Zero is tempting to reject as a "never anchored" sentinel, since the tools stamp
    // it in DEFAULT_PROPERTIES, but a feature legitimately anchored at zoom 0 is
    // indistinguishable from those. The guard tests finiteness only.
...
// ============================================================================
// The other half of the defect: the pixel dimensions of the image
// ============================================================================

// `add_image_geometry.js` imports BaseGeometry from the `@tools` barrel, which pulls in
// DOM/MapLibre-coupled modules. Mock the barrel with trivial bases so the pure geometry
// runs in the `node` environment. The same mock serves the control below.
vi.mock('@tools', () => ({
    BaseControl: class {
...
const { default: AddImageGeometry, IMAGE_BOX_FALLBACKS } =
    await import('@js/draw_tools/image_tool/add_image_geometry.js');

const geom = new AddImageGeometry();

/**
 * Identity stub of the UI manager: the pixel-to-degree maths belongs to the caller, so
 * what is asserted here are the ARGUMENTS the funnel builds.
 * @returns {Object} uiManager double
 */
function makeUiManager() {
    return {
        calculateExpandedDimensions: vi.fn((width, height) => ({ width, height })),
        pixelsToDegrees: vi.fn((pixels) => pixels),
    };
}

describe('calculateSelectionBoxGeometry: o funil unico das dimensoes', () => {
    // The six call sites in `add_image_control.js` read `properties.width`/
    // `properties.height` straight, and a legacy `.ebgeo` carries neither.
    it('width/height ausentes caem na dimensao declarada, sem NaN', () => {
        ...
        // 100 * 1 * 0.625 = 62.5; the padding of 5 adds 10 on each axis.
    ...
    it('width/height NaN, zero, negativo e Infinity caem no mesmo fallback', () => {
    ...
    // CONTROLE POSITIVO: without it, a guard that always returned the fallback would
    // leave the three cases above green.
    ...
    it('zoom 0 explicito continua sendo usado, nao e valor falsy', () => {
        const ui = makeUiManager();
        geom.calculateSelectionBoxGeometry([0, 0], 10, 10, 1, 0, 12, ui, 0);
        expect(ui.pixelsToDegrees.mock.calls[0][2]).toBe(0);
    });

    it('effectiveZoom ausente ou null continua caindo para createdAtZoom', () => {
        const ui = makeUiManager();
        geom.calculateSelectionBoxGeometry([0, 0], 10, 10, 1, 0, 9, ui);
        expect(ui.pixelsToDegrees.mock.calls[0][2]).toBe(9);

        const ui2 = makeUiManager();
        geom.calculateSelectionBoxGeometry([0, 0], 10, 10, 1, 0, 7, ui2, null);
        expect(ui2.pixelsToDegrees.mock.calls[0][2]).toBe(7);
    });

    it('sem zoom usavel o poligono sai finito, pelo fallback em graus', () => {
        // The real `pixelsToDegrees` returns NaN for an undefined zoom; the stub imitates
        // that, to measure what the funnel does with the result.
        const ui = {
...
describe('createSelectionBoxFromDegrees: a ultima linha de defesa', () => {
    it('NaN de entrada cai no fallback declarado, por EIXO', () => {
        ...
        // The healthy axis passes through untouched: 2 degrees stay 2 degrees.
    ...
    // Only finiteness is tested here: a zero span (five vertices on the centre) and a
    // negative one (an inverted ring) are documented, harmless shapes.
    it('extensao zero e negativa NAO sao substituidas', () => {
        const zero = geom.createSelectionBoxFromDegrees([3, 4], 0, 0).coordinates[0];
        for (const p of zero) expect(p).toEqual([3, 4]);

        const negativa = geom.createSelectionBoxFromDegrees([0, 0], -2, -2).coordinates[0];
        expect(negativa[0]).toEqual([1, -1]);
        expect(negativa[2]).toEqual([-1, 1]);
    });
...
// ============================================================================
// The IMAGE control, which is where the defect was measured
// ============================================================================
...
vi.mock('@utils', () => ({
    IDUtils: { generateGeoJSONId: vi.fn(() => 'geo-1'), generateUniqueId: vi.fn(() => 'uid-1'), generateFeatureName: vi.fn() },
    showError: vi.fn(),
