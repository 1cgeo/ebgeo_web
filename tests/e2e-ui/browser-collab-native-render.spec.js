...
import { drawLineUI } from './helpers/collab-helpers.js';
...
describeOrSkip('Peer feature renders NATIVELY on the receiver — map source + layers tree, no duplicate', () => {
    test('A creates a line via the real store op; B renders it in its source + tree exactly once', async ({ browser }) => {
        ...
        // CLIENT A: draw a line through the REAL line tool (toolbar activate + canvas clicks +
        // right-click to finish), exactly like a user. The tool renders it directly into A's
        // source and the op syncs to B. The id is generated by the tool, so we read it back.
        const lineCoords = [
            [CENTER.lng - 0.02, CENTER.lat - 0.01],
            [CENTER.lng + 0.01, CENTER.lat + 0.005],
            [CENTER.lng + 0.03, CENTER.lat - 0.008],
        ];
        const lineId = await drawLineUI(pageA, lineCoords);
