...
**O guarda de tempo de um teste que espera um worker `unref` segura o laço de eventos: nunca `unref` nele.** Os workers do `SqliteBlobPool` são `unref`, então enquanto a resposta viaja o temporizador do guarda é a única coisa viva; com ele `unref`, o laço esvazia antes e o `node:test` CANCELA o caso ("Promise resolution is still pending but the event loop has already resolved"), no Linux sempre e no Windows não. Toda espera por worker passa pelo guarda, o `withEvicted` inclusive, e o `finally` o limpa. Medido em 2026-09-26: `backend/tests/unit/sqlite-blob-pool.test.js` e `backend/tests/unit/sqlite-blob-worker.test.js` cancelavam 14 casos por rodada no Linux e passaram 5 de 5 rodadas depois. Na mesma data, dois casos de pasta viraram portáveis: tamanho de pasta é 0 no Windows e 4096 no ext4, e o SQLite recusa a pasta com códigos diferentes nos dois sistemas; o alvo de tamanho zero é arquivo vazio, e o caso da pasta afirma a CLASSE do erro, não o código.
